...
namespace Moyo.Unity.Editor
{
    ...
    public static class UIFrameworkVerification
    {
        ...
        // -batchmode -executeMethod Moyo.Unity.Editor.UIFrameworkVerification.Batch (without -quit).
        public static async void Batch()
        {
            var exitCode = 0;
            string result;
            try
            {
                var verification = RunAsync();
                if (await Task.WhenAny(verification, Task.Delay(TimeSpan.FromSeconds(60))) != verification)
                    throw new TimeoutException("UI framework verification did not complete within 60 seconds; inspect the last UI FRAMEWORK CHECK in the log.");
                result = "PASS " + DateTimeOffset.Now.ToString("O") + "\n" + await verification;
            }
            catch (Exception error) { exitCode = 1; result = "FAIL " + DateTimeOffset.Now.ToString("O") + "\n" + error; }
            System.IO.Directory.CreateDirectory("Library/LandsongEcs");
            System.IO.File.WriteAllText("Library/LandsongEcs/ui-framework-verification.txt", result);
            Debug.Log(result);
        ...
        public static async Task<string> RunAsync()
        {
            ...
            try
            {
                ...
                registry.SetPanels(new UIPanelConfig(assetA), new UIPanelConfig(assetB));
                manager = CreateManager(scene, registry, owned);
                manager.Initialize();
                b = await manager.OpenAsync<UIFrameworkTestPanelB>();
                entered = NewSignal();
                UIFrameworkProbePanel.Opening = async (panel, args) =>
                { entered.TrySetResult(true); await Task.Delay(Timeout.Infinite, panel.OperationToken); };
                var interruptedOpen = manager.OpenAsync<UIFrameworkTestPanelA>();
                await entered.Task;
                var rootScope = manager.ApplicationScope;
                // PreviewScene does not dispatch normal play-mode MonoBehaviour destruction callbacks.
                // Exercise the synchronous fallback explicitly; the real scene test covers Unity dispatch.
                manager.SendMessage("OnDestroy", SendMessageOptions.RequireReceiver);
                Object.DestroyImmediate(manager.gameObject);
                await ExpectFailure<OperationCanceledException>(() => interruptedOpen);
                Require(rootScope.IsEnded && b == null && !UIManager.TryGetInstance(out _)
                    && UILoader.LiveAddressableLeaseCount == baseline,
                    "销毁回调同步取消打开任务、销毁实例并释放根所有权"); checks++;
        ...
        { if (!condition) throw new InvalidOperationException("框架验证失败：" + invariant); Debug.Log("[UI FRAMEWORK CHECK] " + invariant); }
