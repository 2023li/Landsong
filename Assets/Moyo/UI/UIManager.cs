...
namespace Moyo.Unity
{
    ...
    public sealed class UIManager : MonoBehaviour
    {
        ...
        private void OnDestroy()
        {
            ...
            // The application awaits ShutdownAsync before normal quit. Destruction is a
            // synchronous last resort and must not leave an Addressables lease waiting on a frame.
            shuttingDown = true;
            foreach (var scope in scopes) scope.Cancel();
            lifetime.Cancel();
            foreach (var entry in new List<Entry>(entries.Values))
            {
                ...
                try { entry.Lease?.Dispose(); } catch (Exception error) { Debug.LogException(error); }
                entry.Lease = null;
            }
            entries.Clear(); openOrder.Clear(); scopes.Clear(); focused = null;
            IsInitialized = false;
