...
namespace Landsong.ECS.Presentation
{
    ...
    public static class EcsSceneFlow
    {
        ...
        static Action<Request> beginTransition;
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
        static void Reset() { Pending = null; Busy = GameReady = false; MenuMessage = null; beginTransition = null; }
        internal static void Bind(Action<Request> handler)
        { if (beginTransition != null) throw new InvalidOperationException("场景流程已注册。"); beginTransition = handler; }
        internal static void Unbind(Action<Request> handler) { if (beginTransition == handler) beginTransition = null; }
        ...
        {
            if (Busy) return;
            if (beginTransition == null) throw new InvalidOperationException("应用流程未配置：场景必须绑定 ApplicationSceneEntry 与 UI 根。");
            bool previousBusy = Busy, previousReady = GameReady; var previousPending = Pending;
            Busy = true; GameReady = false; Pending = request;
            try { beginTransition(request); }
            catch { Busy = previousBusy; GameReady = previousReady; Pending = previousPending; throw; }
        }
        internal static void EnterGame() { GameReady = true; Busy = false; Pending = null; }
        internal static void PrepareMenu(string message) { GameReady = false; Busy = true; Pending = null; MenuMessage = message; }
