...
namespace Landsong.ECS.Presentation
{
    ...
    public sealed class InterfacePreferences
    {
        ...
        {
            Master = Clamp(Master, 0, 1, 1);
            Music = Clamp(Music, 0, 1, 1);
            Effects = Clamp(Effects, 0, 1, 1);
            Ambient = Clamp(Ambient, 0, 1, 1);
            UiScale = Clamp(UiScale, .8f, 1.4f, 1);
            CameraSpeed = Clamp(CameraSpeed, 5, 60, 18);
            ZoomSpeed = Clamp(ZoomSpeed, .25f, 3, 1);
            Smoothing = Clamp(Smoothing, 0, .3f, 0);
            if (Width < 800 || Width > 16384 || Height < 600 || Height > 16384)
                Width = Height = 0;
            Fullscreen = Mathf.Clamp(Fullscreen, -1, 1);
            Quality = Mathf.Clamp(Quality, -1, Mathf.Max(0, QualitySettings.names.Length - 1));
            if (string.IsNullOrWhiteSpace(Language) || Language.Length > 64 || !System.Text.RegularExpressions.Regex.IsMatch(Language, @"^[a-zA-Z0-9_.-]+$"))
                Language = "zh-Hans";
            var keys = new[]
            {
                Forward,
    ...
    {
        const string KeyName = "preferences";
        const string SettingsPath = "Landsong.UI.Preferences.es3";
        public static InterfacePreferences Current { get; private set; } = new InterfacePreferences();
        public static int Revision { get; private set; }

        public static Action<string> PersistenceOverride; // Owned test scope only; production uses ES3's PlayerPrefs backend.
        public static InterfacePreferences Decode(string json)
        {
            try
            {
                var data = JsonUtility.FromJson<InterfacePreferences>(json) ?? new InterfacePreferences();
                data.Validate();
                return data;
            }
            catch
        ...
        static void Load()
        {
            try
            {
                ...
                Current = ES3.KeyExists(KeyName, settings) ? ES3.Load<InterfacePreferences>(KeyName, settings) : new InterfacePreferences();
            }
            catch
            {
                Current = new InterfacePreferences();
            }
            Apply(Current, false);
        }
        ...
        public static void Apply(InterfacePreferences data, bool persist = true, bool display = true)
        {
            ...
            if (display)
            {
                if (data.Quality >= 0 && QualitySettings.GetQualityLevel() != data.Quality)
                    QualitySettings.SetQualityLevel(data.Quality);
                if (data.Width > 0 && (Screen.width != data.Width || Screen.height != data.Height))
                    Screen.SetResolution(data.Width, data.Height, data.Fullscreen < 0 ? Screen.fullScreen : data.Fullscreen != 0);
                else if (data.Fullscreen >= 0 && Screen.fullScreen != (data.Fullscreen != 0))
                    Screen.fullScreen = data.Fullscreen != 0;
            }
            ...
            if (persist)
            {
                ...
                if (PersistenceOverride != null)
                    PersistenceOverride(json);
                else
                    ES3.Save(KeyName, data, StorageSettings());
            }
        }

        static ES3Settings StorageSettings() => new ES3Settings(SettingsPath, ES3.Location.PlayerPrefs)
        {
