    ...
    internal static class MilitaryStateValidation
    {
        internal static void Validate(EntityManager em, Entity root, SnapshotCodec.Snapshot data)
        {
            ...
            foreach (var record in data.Records.OfType<SoldierSnapshot>())
            {
                ...
                if (soldier.PopulationCost < 0 || soldier.Experience < 0 || soldier.PendingSince < 0 || soldier.PendingSince > data.Clock.Turn || soldier.LastExperienceTurn < 0 || soldier.LastExperienceTurn > data.Clock.Turn || soldier.RecallState != 0 || soldier.Slot < 0 || soldier.Experience > UnitProgression.LevelThreshold(definition.Growth, definition.Growth.MaxLevel))
