...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        ...
        static IEnumerator InvitationExpeditionUi(EcsGameView view, EntityManager em, Entity root, string map)
        {
            ...
            view.OpenPanel("库存"); Require(view.Panel != "库存", "Inventory panel rejects direct locked entry"); view.OpenPanel("远征"); Require(view.Panel != "远征", "Expedition panel rejects direct locked entry");
            ...
            foreach (var permission in new[] { "feature.Inventory", "feature.Building", "feature.Expedition" }) Sim.Grant(em, root, Sim.FindDefinition(em, root, new FixedString128Bytes(permission)));
            ...
            view.OpenInvitations(marketId);
            yield return WaitFor(() => view.QuestPoolRows != null && view.QuestPoolRows.gameObject.activeInHierarchy && view.QuestPoolRows.GetComponentsInChildren<Button>().Any(b => b.interactable && b.GetComponentInChildren<Text>()?.text.StartsWith("立即邀约") == true), "Actual UGUI invitation source pool");
            var mainlineName = em.GetComponentData<Identity>(Sim.Find(em, QuestOps.Tracking(em, root).Target)).Name.ToString();
            Require(view.QuestListRows.GetComponentsInChildren<Text>().Any(t => t.text.Contains(mainlineName)), "Active mainline remains visible beside invitation pool");
            var bytes = SnapshotCodec.Capture(em, root); ClickContains(view.QuestPoolRows, "立即邀约"); ClickIn(view.BuildingConfirmRows, "取消"); Require(bytes.SequenceEqual(SnapshotCodec.Capture(em, root)), "Cancelled paid invitation is mutation free");
            ClickContains(view.QuestPoolRows, "立即邀约"); ClickIn(view.BuildingConfirmRows, "确认");
            ...
            Require(expectedCommand, "Invitation confirmation queues expected source " + marketId + " and slot; queue=" + commandDiagnostic + "; ready=" + EcsSceneFlow.GameReady + "; message=" + view.Message.text);
            yield return WaitFor(() => em.GetBuffer<Command>(root).Length == 0, "Paid invitation command processed");
            Require(QuestOfferOps.Offered(em, marketId, 0) != Entity.Null, "Paid invite button creates offer; " + view.Message.text + "; source=" + QuestOfferOps.Quote(em, root, market, 0).Reason + "; phase=" + em.GetComponentData<Session>(root).Phase);
            yield return WaitFor(() => view.QuestPoolRows.GetComponentsInChildren<Button>().Any(b => b.interactable && b.GetComponentInChildren<Text>()?.text.Contains("查看邀约") == true), "New invitation displayed under source");
            ClickContains(view.QuestPoolRows, "查看邀约"); yield return WaitFor(() => HasRow(view.QuestDetailRows, "签约"), "Invitation detail opens without auto acceptance");
            ClickContains((RectTransform)view.QuestWindow.transform, "取消来源筛选"); yield return new WaitForSecondsRealtime(.4f);
            for(var type=0;type<4;type++)view.QuestTypeToggle(type).isOn=type==3; yield return new WaitForSecondsRealtime(.4f);
            Require(!view.QuestPoolRows.GetComponentsInChildren<Text>().Any(t => t.text.Contains("市场（")), "Independent source filter hides nonmatching trade source");
            for(var type=0;type<4;type++)view.QuestTypeToggle(type).isOn=true;
            ...
            view.OpenPanel("远征"); yield return WaitFor(() => view.PrimaryRows.GetComponentsInChildren<Button>().Any(b => b.interactable && b.GetComponentInChildren<Text>()?.text.Contains("陆上远征所 LV") == true), "Expedition lists available source buildings");
            ...
            bytes = SnapshotCodec.Capture(em, root); ClickRow(view, "确认派遣"); ClickIn(view.BuildingConfirmRows, "取消"); Require(bytes.SequenceEqual(SnapshotCodec.Capture(em, root)), "Cancel expedition does not spend or lock workers");
            ClickRow(view, "确认派遣"); ClickIn(view.BuildingConfirmRows, "确认"); yield return WaitFor(() => EconomyOps.WorkforceLocked(em, postId), "Real UGUI dispatch locks source workforce");
            yield return WaitFor(() => HasRow(view.PrimaryRows, "放弃远征"), "Travelling party exposes abandon action");
            ClickRow(view, "放弃远征"); ClickIn(view.BuildingConfirmRows, "取消"); Require(EconomyOps.WorkforceLocked(em, postId), "Abandon cancellation retains party");
            ClickRow(view, "放弃远征"); ClickIn(view.BuildingConfirmRows, "确认"); yield return WaitFor(() => !EconomyOps.WorkforceLocked(em, postId), "Confirmed abandon returns workers without casualties");
            yield return WaitFor(() => HasRow(view.PrimaryRows, "确认派遣"), "Source can depart again after abandonment");
            ClickRow(view, "确认派遣"); ClickIn(view.BuildingConfirmRows, "确认"); yield return WaitFor(() => EconomyOps.WorkforceLocked(em, postId), "Second UGUI departure accepted");
            ulong journeyId = 0;
            ...
            bytes = SnapshotCodec.Capture(em, root); ClickRow(view, "领取远征奖励"); ClickIn(view.BuildingConfirmRows, "取消"); Require(bytes.SequenceEqual(SnapshotCodec.Capture(em, root)), "Reward cancellation retains unclaimed result and inventory");
            var wood = Sim.FindDefinition(em, root, new FixedString128Bytes("原木")); var woodBefore = InventoryOps.Count(em, root, wood);
            ClickRow(view, "领取远征奖励"); ClickIn(view.BuildingConfirmRows, "确认"); yield return WaitFor(() => em.GetBuffer<Command>(root).Length == 0, "Reward claim command processed");
            Require(Sim.Find(em, journeyId) == Entity.Null, "Confirmed reward claim consumes result; " + view.Message.text + "; paused=" + em.GetComponentData<Session>(root).Paused + "; menu=" + (view.PauseMenu != null && view.PauseMenu.IsOpen));
