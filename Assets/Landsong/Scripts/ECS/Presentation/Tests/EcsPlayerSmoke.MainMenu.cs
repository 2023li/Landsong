...
namespace Landsong.ECS.Presentation
{
    ...
    {
        IEnumerator MainMenuArchivesUi(EcsMainMenu menu, RunArchive archive)
        {
            ...
            try
            {
                ...
                var browser = FindFirstObjectByType<ArchiveBrowser>();
                string loadedRun = null, loadedSlot = null;
                browser.Load = (run, slot, backup) => { loadedRun = run; loadedSlot = slot; };
        ...
        IEnumerator MainMenuUi(EcsMainMenu menu)
        {
            var originalStore = menu.Store;
            ...
            try
            {
                ...
                finally { menu.Catalog = catalog; menu.CloseManagement(); }
                menu.LoadButton.onClick.Invoke();
                yield return null;
                var archive = FindFirstObjectByType<ArchiveBrowser>();
                Require(archive != null && ReferenceEquals(archive.Store, store), "Load game opens cross-dynasty browser using the isolated store");
                archive.Back();
                menu.SettingsButton.onClick.Invoke();
                yield return null;
                Require(FindFirstObjectByType<InterfaceSettingsPanel>() != null, "Home opens shared settings");
                menu.CloseManagement();
                menu.QuitButton.onClick.Invoke();
                yield return null;
                var cancel = menu.GetComponentInParent<Canvas>().GetComponentsInChildren<Button>()
                    .Single(b => b.GetComponentInChildren<TextMeshProUGUI>().text == "取消");
                Require(cancel.IsInteractable() && !EcsSceneFlow.Busy, "Quit waits for confirmation with an interactive cancel button");
