...
namespace Landsong.ECS.Presentation
{
    ...
    {
        IEnumerator HeroUi(EcsGameView view, EntityManager em, Entity root, string map)
        {
            ...
            view.Send(CommandKind.RecruitHero, siteId);
            ...
            yield return WaitFor(() => em.GetComponentData<Session>(root).Phase == Phase.Night || view.Panel == "入夜确认", "Hero night preparation or confirmation: " + view.Message.text);
            if (view.Panel == "入夜确认") { ClickRow(view, "确认放弃以上物资与士兵并入夜"); }
            ...
            var canvas = view.GetComponentInParent<Canvas>(); Transform hud = null;
            yield return WaitFor(() => { hud = canvas.transform.Find("Battle HUD"); return hud != null && hud.gameObject.activeInHierarchy && hud.GetComponentsInChildren<Button>().Any(button => button.interactable); }, "Independent hero HUD available");
            hud.GetComponentsInChildren<Button>().First(button => button.interactable).onClick.Invoke();
            Button WakeButton() => view.BuildingDetailsRows.GetComponentsInChildren<Button>().FirstOrDefault(button => button.interactable && button.GetComponentInChildren<Text>()?.text == "唤醒英雄");
            ...
            Require(math.distance(from, destination) > 1, "Hero test has reachable movement destination"); view.Send(CommandKind.MoveHero, position: destination);
            ...
            view.Send(CommandKind.SelectHero); yield return WaitFor(() => em.GetComponentData<Session>(root).SelectedHero == Entity.Null, "Cancel hero selection through command");
            ...
            view.BuildingDetailsClose.onClick.Invoke();
            ...
            SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, original)); view.OpenPanel("建筑"); yield return null;
        }
    }
}
#endif
