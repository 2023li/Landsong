...
namespace Landsong.ECS.Presentation
{
    ...
    {
        IEnumerator PeacefulUi(EcsGameView view, EntityManager em, Entity root, string map)
        {
            ...
            var cameraPosition = view.Camera.transform.position; float zoom = view.Camera.orthographicSize;
            ...
            var ray = new Ray(view.Camera.transform.position, view.Camera.transform.forward); var plane = new Plane(Vector3.up, (Vector3)position); if (plane.Raycast(ray, out float distance)) view.Camera.transform.position += (Vector3)position - ray.GetPoint(distance);
            view.Camera.orthographicSize = 10; view.OpenPanel("建筑");
            yield return WaitFor(() => view.GetComponentInParent<Canvas>().GetComponentsInChildren<Button>().Any(b => b.name == "Night marker" && b.interactable && b.GetComponentInChildren<TMP_Text>().text.Contains("小偷")), "13 TMP marker is visible and clickable");
            var marker = view.GetComponentInParent<Canvas>().GetComponentsInChildren<Button>().First(b => b.name == "Night marker" && b.interactable && b.GetComponentInChildren<TMP_Text>().text.Contains("小偷")); marker.onClick.Invoke();
            ...
            view.Send(CommandKind.Pause); yield return WaitFor(() => em.GetComponentData<Session>(root).Paused != 0, "13 capture can be paused"); var frozen = Sim.Position(em, visitor);
            ...
            view.Send(CommandKind.Pause); yield return WaitFor(() => em.GetComponentData<Session>(root).Paused == 0, "13 resume capture");
            ...
            em.GetBuffer<BattleReportEntry>(root).RemoveAt(privateRow); view.OpenPanel("建筑");
            ...
            yield return WaitFor(() => view.GetComponentInParent<Canvas>().GetComponentsInChildren<Button>().Any(b => b.name == "Night marker" && b.interactable && b.GetComponentInChildren<TMP_Text>().text.Contains("特殊战利品")), "13 special loot marker can be clicked during cleanup");
            view.GetComponentInParent<Canvas>().GetComponentsInChildren<Button>().First(b => b.name == "Night marker" && b.interactable && b.GetComponentInChildren<TMP_Text>().text.Contains("特殊战利品")).onClick.Invoke();
            ...
            yield return WaitFor(() => view.Advance.interactable, "13 report entry becomes available"); view.Advance.onClick.Invoke();
            ...
            checkpoint.Import(root, archive, false); SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, original)); view.Camera.transform.position = cameraPosition; view.Camera.orthographicSize = zoom; view.OpenPanel("建筑"); yield return null;
        }
    }
}
#endif
