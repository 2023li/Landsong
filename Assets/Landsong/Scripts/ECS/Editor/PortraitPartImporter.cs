...
{
    [Serializable]
    public sealed class PortraitImportLayer
    {
        public PortraitLayer Layer;
        public PortraitTint Tint;
        public string File = "";
    }

    [Serializable]
    public sealed class PortraitImportDraft
    {
        public string Id = "";
        ...
        {
            if (!PortraitPartRules.Allowed(Type).Contains(layer) || Layers.Any(item => item.Layer == layer))
                return false;
    ...
    public static class PortraitPartImporter
    {
        ...
        public static void ValidateId(string id)
        {
            ...
            if (new[]
            {
                ...
                "AUX",
                "NUL"
            }.Contains(first) || Regex.IsMatch(first, @"\A(COM|LPT)[1-9]\z") || id.StartsWith("placeholder.", StringComparison.OrdinalIgnoreCase))
                throw new InvalidOperationException("该标识为系统或程序占位保留，请换一个标识。");
        }

        public static byte[] ReadPng(string file, int resolution)
        {
            if (resolution != 32 && resolution != 64)
                throw new InvalidOperationException("项目肖像尺寸必须为 32 或 64。");
            ...
            if (info.Length < 33 || info.Length > MaximumFileBytes)
                throw new InvalidOperationException("PNG 文件无效或超过 4 MB。");
            ...
            if (!bytes.Take(8).SequenceEqual(signature) || bytes[12] != 'I' || bytes[13] != 'H' || bytes[14] != 'D' || bytes[15] != 'R')
                throw new InvalidOperationException("文件内容不是有效的 PNG 图像。");
            ...
            if (width != resolution || height != resolution)
                throw new InvalidOperationException($"{Path.GetFileName(file)} 原始尺寸是 {width}×{height}，项目要求 {resolution}×{resolution}。不允许缩放或裁切后导入。");
            var texture = new Texture2D(2, 2, TextureFormat.RGBA32, false);
            try
            {
                if (!ImageConversion.LoadImage(texture, bytes, false) || texture.width != resolution || texture.height != resolution)
                    throw new InvalidOperationException("PNG 数据损坏，无法解码：" + Path.GetFileName(file));
            }
            finally
            {
        ...
        static List<Input> Prepare(PortraitConfig config, PortraitImportDraft draft)
        {
            if (EditorApplication.isPlayingOrWillChangePlaymode || EditorApplication.isCompiling)
                throw new InvalidOperationException("请退出 Play 并等待编译完成后再导入。");
            if (config == null || string.IsNullOrEmpty(AssetDatabase.GetAssetPath(config)))
                throw new InvalidOperationException("需要已保存的项目 PortraitConfig 资产。");
            if (draft == null || draft.Layers == null || draft.Layers.Any(l => l == null))
                throw new InvalidOperationException("部件草稿无效。");
            ValidateId(draft.Id);
            if (draft.Type > PortraitPartType.Accessory)
                throw new InvalidOperationException("当前导入窗口只接入运行时使用的 13 类单选部件。");
            ...
            if (Directory.Exists(destination) || System.IO.File.Exists(destination) || System.IO.File.Exists(destination + ".meta"))
                throw new InvalidOperationException("目标路径已存在，不允许覆盖：" + destination);
            using (var current = PortraitLibraryBuilder.Build(config))
                if (PortraitOps.Find(ref current.Value, hash) >= 0)
                    throw new InvalidOperationException("部件标识与当前库中的稳定 ID 冲突。");
            var paths = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            var inputs = new List<Input>();
            foreach (var layer in selected)
            {
                if (!Enum.IsDefined(typeof(PortraitTint), layer.Tint))
                    throw new InvalidOperationException("无效的染色通道。");
                if (draft.Type == PortraitPartType.Hair && layer.Tint != PortraitTint.Hair)
                    throw new InvalidOperationException("前发和后发都必须使用发色染色。");
                if (!paths.Add(Path.GetFullPath(layer.File)))
                    throw new InvalidOperationException("添加多个渲染层时，各层不能重复选择同一张文件。");
        ...
        public static string Import(PortraitConfig config, PortraitImportDraft draft, Action<string> probe = null)
        {
            ...
            var inputs = Prepare(config, draft);
            string directory = Destination(draft);
            var original = config.Parts;
            bool created = false, registered = false;
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(folderGuid))
                    throw new IOException("无法创建部件目录。");
                string actual = AssetDatabase.GUIDToAssetPath(folderGuid);
                if (actual != directory)
                {
                    AssetDatabase.DeleteAsset(actual);
                    throw new IOException("目标目录发生变化，已取消导入：" + directory);
                }

                created = true;
                var renders = new List<PortraitRenderSource>();
                foreach (var input in inputs)
                {
                    string path = directory + "/" + input.Layer + ".png";
                    using (var stream = new FileStream(path, FileMode.CreateNew, FileAccess.Write))
                        stream.Write(input.Bytes, 0, input.Bytes.Length);
                    AssetDatabase.ImportAsset(path, ImportAssetOptions.ForceSynchronousImport);
                    var importer = AssetImporter.GetAtPath(path) as TextureImporter;
                    if (importer == null)
                        throw new IOException("无法读取 PNG 导入设置：" + path);
                    ...
                    importer.maxTextureSize = config.Resolution;
                ...
                var part = new PortraitPartSource
                {
                    Id = draft.Id,
                    Type = draft.Type,
                ...
                try
                {
                    candidate.Parts = (original ?? Array.Empty<PortraitPartSource>()).Append(part).ToArray();
                    using var library = PortraitLibraryBuilder.Build(candidate);
                }
                finally
                {
                ...
                config.Parts = (original ?? Array.Empty<PortraitPartSource>()).Append(part).ToArray();
                registered = true;
                EditorUtility.SetDirty(config);
                probe?.Invoke("registered");
            ...
            catch
            {
                if (registered)
                {
                    config.Parts = original;
                    EditorUtility.SetDirty(config);
                    AssetDatabase.SaveAssetIfDirty(config);
                }

                if (created && (AssetDatabase.AssetPathToGUID(directory) != folderGuid || !AssetDatabase.DeleteAsset(directory)))
                    throw new IOException("导入失败且临时目录归属变化或清理失败，请检查：" + directory);
                throw;
            }
        }
        ...
        static void EnsureFolder(string path)
        {
            ...
            if (Directory.Exists(path))
            {
                ...
                if (AssetDatabase.IsValidFolder(path))
                    return;
                throw new IOException("已有资源目录尚未正确导入：" + path);
            }
            ...
            {
                AssetDatabase.DeleteAsset(actual);
                throw new IOException("资源目录发生变化，已取消导入：" + path);
            }
        }
    }
}
