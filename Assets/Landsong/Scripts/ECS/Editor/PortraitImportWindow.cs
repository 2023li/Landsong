...
namespace Landsong.ECS.Editor
{
    ...
    {
        [SerializeField]
        PortraitImportDraft draft = new PortraitImportDraft();
        Vector2 scroll;
        string error = "", result = "";
        bool validate = true;
        int resolution, nextLayer;
        [SerializeField]
        bool imported;
        ...
        void OnEnable()
        {
            minSize = new Vector2(780, 620);
            if (draft == null)
                draft = new PortraitImportDraft();
            if (draft.Layers == null)
                draft.ResetLayers();
            ReloadConfig();
        }
        ...
        void ReloadConfig() => config = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Landsong/ECSContent/World/GameWorldTemplate.prefab")?.GetComponent<PortraitLibraryAuthoring>()?.Portraits;
        void ClearPreviews()
        {
        ...
        {
            ReloadConfig();
            error = imported ? "" : PortraitPartImporter.Validate(config, draft);
            validate = false;
            Repaint();
        }
        ...
        {
            Recheck();
            if (imported || error.Length != 0)
                return;
            try
            {
                result = "已导入并登记：" + PortraitPartImporter.Import(config, draft) + "\n下次进入游戏时生效。";
                imported = true;
            }
            catch (Exception e)
            {
        ...
        {
            PortraitLayer.HairBack => "后发",
            PortraitLayer.HairFront => "前发",
            PortraitLayer.FaceBase => "脸部底图",
            PortraitLayer.Ear => "耳朵",
            PortraitLayer.Eyes => "眼睛底图",
            PortraitLayer.EyeOverlay => "瞳孔／眼睛覆盖层",
            PortraitLayer.Eyebrows => "眉毛",
            PortraitLayer.Nose => "鼻子",
            PortraitLayer.Mouth => "嘴型",
            PortraitLayer.FacialHair => "胡须",
            PortraitLayer.Body => "身体",
            PortraitLayer.NeckBack => "颈部后层",
            PortraitLayer.Neck => "颈部",
            PortraitLayer.ClothesFront => "服装前层",
            PortraitLayer.ClothesBack => "服装后层",
            PortraitLayer.HeadAccessoryFront => "头饰前层",
            PortraitLayer.HeadAccessoryBack => "头饰后层",
            PortraitLayer.FaceAccessory => "面饰",
            PortraitLayer.FrontAccessory => "饰品前层",
            PortraitLayer.BackAccessory => "饰品后层",
            _ => layer.ToString()};
        void OnGUI()
        {
            if (config == null)
            ...
            {
                GUILayout.Label("肖像部件导入", EditorStyles.boldLabel);
                ...
                if (GUILayout.Button("打开素材目录", EditorStyles.toolbarButton))
                {
                    var path = Path.GetFullPath(PortraitPartImporter.OutputRoot);
                    if (Directory.Exists(path))
                        EditorUtility.RevealInFinder(path);
                    else
                        result = "首次成功导入时会自动创建：" + PortraitPartImporter.OutputRoot;
                }
            ...
            if (config == null)
            {
                EditorGUILayout.HelpBox("模拟根模板的肖像组件尚未引用 PortraitConfig，请先配置项目肖像资源。", MessageType.Error);
                return;
            }
            ...
            if (locked)
                EditorGUILayout.HelpBox("Play／编译期间禁用导入，请退出 Play 并等待编译完成。", MessageType.Warning);
                ...
                using (new EditorGUI.DisabledScope(locked))
                {
                    ...
                    using (new EditorGUILayout.HorizontalScope())
                    {
                        using (new EditorGUILayout.VerticalScope(GUILayout.MinWidth(420)))
                        {
                            ...
                            if (type != (int)draft.Type)
                            {
                                draft.Type = (PortraitPartType)type;
                                draft.ResetLayers();
                                nextLayer = 0;
                            }
                            ...
                            using (new EditorGUILayout.HorizontalScope())
                            {
                                ...
                                draft.Male = GUILayout.Toggle(draft.Male, "男用", GUILayout.Width(70));
                                draft.Female = GUILayout.Toggle(draft.Female, "女用", GUILayout.Width(70));
                            }
                            ...
                            if (draft.Type == PortraitPartType.Hair)
                                EditorGUILayout.LabelField("发型可只含前发、只含后发，或同时包含两层。", EditorStyles.wordWrappedLabel);
                            ...
                            if (available.Length > 0)
                            {
                                using (new EditorGUILayout.HorizontalScope())
                                {
                                    ...
                                    if (GUILayout.Button("添加图层", GUILayout.Width(85)))
                                    {
                                        draft.AddLayer(available[nextLayer]);
                                        nextLayer = 0;
                                        GUI.changed = true;
                                    }
                                }
                                ...
                                EditorGUILayout.LabelField("该类别的所有可用图层均已添加，可移除不需要的图层。", EditorStyles.wordWrappedMiniLabel);
                            int removeLayer = -1;
                            for (int layerIndex = 0; layerIndex < draft.Layers.Count; layerIndex++)
                            {
                                ...
                                using (new EditorGUILayout.HorizontalScope())
                                {
                                    ...
                                    if (GUILayout.Button("移除图层", GUILayout.Width(85)))
                                ...
                                using (new EditorGUILayout.HorizontalScope())
                                {
                                    ...
                                    if (GUILayout.Button("选择 PNG", GUILayout.Width(85)))
                                    {
                                        string file = EditorUtility.OpenFilePanel("选择" + LayerName(layer.Layer), "", "png");
                                ...
                                using (new EditorGUI.DisabledScope(draft.Type == PortraitPartType.Hair))
                                    layer.Tint = (PortraitTint)EditorGUILayout.Popup("染色通道", (int)layer.Tint, Tints);
                            }

                            if (removeLayer >= 0)
                            {
                                draft.Layers.RemoveAt(removeLayer);
                                nextLayer = 0;
                                GUI.changed = true;
                            }
                        }
                        ...
                        using (new EditorGUILayout.VerticalScope(GUILayout.Width(210)))
                        {
                            ...
                            EditorGUI.DrawRect(rect, new Color(.24f, .24f, .24f));
                            for (int y = 0; y < 10; y++)
                                for (int x = 0; x < 10; x++)
                                    if ((x + y) % 2 == 0)
                                        EditorGUI.DrawRect(new Rect(rect.x + x * 20, rect.y + y * 20, 20, 20), new Color(.31f, .31f, .31f));
                            foreach (var layer in draft.Layers.OrderBy(l => l.Layer))
                            {
                                var preview = Preview(layer.File);
                                if (preview != null)
                            ...
                            foreach (var layer in draft.Layers)
                                if (!string.IsNullOrWhiteSpace(layer.File))
                                    EditorGUILayout.LabelField(LayerName(layer.Layer) + "：" + Path.GetFileName(layer.File), EditorStyles.wordWrappedMiniLabel);
                        }
                    }
            ...
            if (error.Length > 0)
                ...
                EditorGUILayout.HelpBox("校验通过：" + DestinationText(), MessageType.Info);
            using (new EditorGUILayout.HorizontalScope())
            {
                if (GUILayout.Button("重新校验", GUILayout.Height(32)))
                {
                    ClearPreviews();
                    Recheck();
                }

                if (GUILayout.Button("填写下一个部件", GUILayout.Height(32)))
                {
                    draft = new PortraitImportDraft
                    {
                        Type = draft.Type
                ...
                using (new EditorGUI.DisabledScope(locked || imported || error.Length != 0))
                    if (GUILayout.Button(imported ? "已导入" : "导入并登记部件", GUILayout.Height(32)))
                        ImportCurrent();
            }
        }
        ...
        Texture2D Preview(string file)
        {
            if (string.IsNullOrWhiteSpace(file))
                return null;
            if (previews.TryGetValue(file, out var existing))
                return existing;
            Texture2D texture = null;
            try
            {
                var bytes = PortraitPartImporter.ReadPng(file, resolution);
