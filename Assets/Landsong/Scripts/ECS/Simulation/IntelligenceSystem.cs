...
namespace Landsong.ECS
{
    ...
    public partial class IntelligenceSystem : SystemBase
    {
        ...
        {
            if (SystemAPI.Time.ElapsedTime < next)
                return;
            next = SystemAPI.Time.ElapsedTime + .25;
            var root = WorldQueries.Root(EntityManager);
            if (root == Entity.Null)
                return;
            var s = EntityManager.GetComponentData<Session>(root);
            NightRuntimeState sNight = EntityManager.GetComponentData<NightRuntimeState>(root);
            ...
            if (s.Phase == Phase.Day && (projection.Revision != grid.Revision || projection.Seed != sNight.Seed))
            {
                NightPlanOps.Reproject(EntityManager, root);
                EntityState.Set(EntityManager, root, new IntelProjection { Revision = grid.Revision, Seed = sNight.Seed });
            }
    ...
    public struct IntelProjection : IComponentData
    {
        public int Revision;
        public uint Seed;
    }
    ...
    public struct IntelGeometry : IBufferElementData
    {
        public int Region, Count, Revision, Tier;
        public float3 Position;
        public IntelArea Area;
    }
}
