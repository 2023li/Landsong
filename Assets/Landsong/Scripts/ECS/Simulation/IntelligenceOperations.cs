...
namespace Landsong.ECS
{
    ...
    {
        public static int Tier(IntelligenceSettings settingsIntelligence, int value) => value >= settingsIntelligence.HighIntel ? 3 : value >= settingsIntelligence.MediumIntel ? 2 : value >= settingsIntelligence.LowIntel ? 1 : 0;
        public static List<IntelSource> Sources(EntityManager em, Entity root)
        {
            var result = new List<IntelSource>();
            var quote = IntelligenceEffects.Query(em, root);
            foreach (var source in quote.Sources)
                result.Add(new IntelSource { Building = source.Owner, Name = source.Name, Points = (int)source.Value, Effective = (int)source.Applied, Reason = source.Reason });
            return result;
        }
        ...
        public static int Current(EntityManager em, Entity root)
        {
            ...
            return s.Phase != Phase.Day && s.Phase != Phase.Settlement ? math.clamp(sNight.Intelligence, 0, 100) : (int)Math.Min(100L, Sources(em, root).Sum(x => (long)x.Effective));
        }
        ...
        public static int Known(EntityManager em, Entity root)
        {
            ...
            return math.max(Current(em, root), r.Turn == sClock.Turn ? r.KnownIntel : 0);
        }

        public static void Refresh(EntityManager em, Entity root)
        {
            EntityState.Buffer<IntelGeometry>(em, root);
            var geometry = em.GetBuffer<IntelGeometry>(root);
            var revision = em.GetComponentData<GridData>(root).Revision;
            if (geometry.Length > 0 && geometry[0].Revision != revision)
                geometry.Clear();
            foreach (var wave in em.GetBuffer<NightWave>(root))
                for (int tier = 2; tier <= 3; tier++)
                {
                    if (wave.Region < 0 || wave.Region >= em.GetBuffer<SpawnRegion>(root).Length || wave.Spawned != 0)
                        continue;
                    bool exists = false;
                    foreach (var entry in geometry)
                        if (Matches(entry, wave, tier, revision))
                        {
                            exists = true;
                            break;
                        }
                    ...
                    if (!exists)
                        geometry.Add(new IntelGeometry { Region = wave.Region, Count = wave.Count, Position = wave.Position, Revision = revision, Tier = tier, Area = ComputeSpawnArea(em, root, wave, tier) });
                }

            // The cache is disposable, never knowledge or a second authority.
            if (geometry.Length > 2048)
                geometry.Clear();
            var s = em.GetComponentData<Session>(root);
            GameClock sClock = em.GetComponentData<GameClock>(root);
            NightRuntimeState sNight = em.GetComponentData<NightRuntimeState>(root);
            IntelligenceModeState sIntelligenceMode = em.GetComponentData<IntelligenceModeState>(root);
            var r = em.HasComponent<RecoveryState>(root) ? em.GetComponentData<RecoveryState>(root) : new RecoveryState
            {
                Turn = sClock.Turn
            };
            if (r.Turn != sClock.Turn)
                r = new RecoveryState
                {
                    Turn = sClock.Turn
                };
            r.KnownIntel = math.max(r.KnownIntel, Current(em, root));
            EntityState.Set(em, root, r);
            ...
            Hash(sClock.Turn.ToString());
            Hash(view.Known.ToString());
            foreach (var line in view.Lines)
                Hash(line);
            foreach (var line in CourtLines(em, root, view.Known))
                Hash(line);
            foreach (var source in view.Sources)
            {
                Hash(source.Name);
                Hash(source.Effective.ToString());
                Hash(source.Reason);
            }

            foreach (var area in view.Areas)
            {
                Hash(area.Center.x.ToString("R", System.Globalization.CultureInfo.InvariantCulture));
                Hash(area.Center.z.ToString("R", System.Globalization.CultureInfo.InvariantCulture));
                Hash(area.Size.x.ToString("R", System.Globalization.CultureInfo.InvariantCulture));
                Hash(area.Size.z.ToString("R", System.Globalization.CultureInfo.InvariantCulture));
            }

            // Reopening a night gives a normal new report, even if the random public composition happens to match.
            Hash(sNight.Seed.ToString());
            r.IntelFingerprint = fingerprint;
            EntityState.Set(em, root, r);
        ...
        {
            if (!em.HasComponent<RecoveryState>(root))
                return;
            var r = em.GetComponentData<RecoveryState>(root);
            if (r.IntelFingerprint != fingerprint)
                return;
            r.IntelReadFingerprint = fingerprint;
            em.SetComponentData(root, r);
        }

        public static string Threat(int baseThreat) => baseThreat < 35 ? "较低" : baseThreat < 90 ? "中等" : "较高";
        public static int2 CountRange(int count)
        {
            int width = math.max(5, (int)math.ceil(count * .3f));
            return new int2(math.max(1, count - width), count + width);
        }
        ...
        public static IntelArea SpawnArea(EntityManager em, Entity root, NightWave wave, int tier)
        {
            if (em.HasBuffer<IntelGeometry>(root))
                foreach (var entry in em.GetBuffer<IntelGeometry>(root))
                    if (Matches(entry, wave, tier, em.GetComponentData<GridData>(root).Revision))
                        return entry.Area;
        ...
        {
            var region = em.GetBuffer<SpawnRegion>(root)[wave.Region];
            var grid = em.GetComponentData<GridData>(root);
            var boundsMin = wave.Position.xz;
            ...
            for (int n = 0; n < wave.Count; n++)
            {
                ...
                if (NightSpatialOps.SpawnPoint(em, root, wave.Region, preferred, out var point, space))
                {
                    boundsMin = math.min(boundsMin, point.xz);
                    boundsMax = math.max(boundsMax, point.xz);
                }
            }

            var low = region.Center.xz - region.Size.xz * .5f;
            var high = low + region.Size.xz;
            ...
            return new IntelArea
            {
                Center = new float3(center.x, region.Center.y, center.y),
                Size = new float3(max.x - min.x, .1f, max.y - min.y),
        ...
        static IntelArea TargetArea(EntityManager em, Entity root, Entity target, int tier, bool harassment)
        {
            ...
            var position = EntityState.Position(em, target);
            float quantum = math.max(grid.CellSize * (tier >= 3 ? 6 : 16), harassment ? NightPlanOps.Rules(em, root).TargetRadius : 0);
            var lo = position.xz - (float2)bPlacement.Size * grid.CellSize * .5f;
            var hi = position.xz + (float2)bPlacement.Size * grid.CellSize * .5f;
            lo = math.floor(lo / quantum) * quantum - grid.CellSize;
            hi = math.ceil(hi / quantum) * quantum + grid.CellSize;
            if (harassment)
            {
                lo -= NightPlanOps.Rules(em, root).TargetRadius;
                hi += NightPlanOps.Rules(em, root).TargetRadius;
            }

            var center = (lo + hi) * .5f;
            return new IntelArea
            {
                Center = new float3(center.x, position.y, center.y),
                Size = new float3(hi.x - lo.x, .1f, hi.y - lo.y),
                Target = true
            };
        }
        ...
        public static IntelView Read(EntityManager em, Entity root, int selectedWave = 0)
        {
            ...
            var view = new IntelView
            {
                Current = Current(em, root),
                Known = Known(em, root),
                Day = s.Phase == Phase.Day || s.Phase == Phase.Settlement
            };
            view.Tier = Tier(cfgIntelligence, view.Known);
            view.NextPoints = view.Tier == 3 ? 0 : (view.Tier == 0 ? cfgIntelligence.LowIntel : view.Tier == 1 ? cfgIntelligence.MediumIntel : cfgIntelligence.HighIntel) - view.Current;
            if (em.HasComponent<RecoveryState>(root))
            {
                var r = em.GetComponentData<RecoveryState>(root);
                view.Unread = r.IntelFingerprint != r.IntelReadFingerprint;
            }

            view.Sources.AddRange(Sources(em, root));
            if (view.Tier == 0)
            {
                view.Lines.Add("暂无可用军情");
                return view;
            }

            view.Lines.Add(sNight.Kind == NightKind.Peaceful ? "当晚平安" : sNight.Kind == NightKind.Boss ? "当晚存在大型威胁" : "当晚发现入侵迹象");
            ...
            try
            {
                ...
                var lead = view.Tier == 3 ? cfgIntelligence.HighIntelLead : cfgIntelligence.MediumIntelLead;
                var visible = groups.Where(t => NightOps.WaveAt(em, root, t) - plan.CombatElapsed <= lead).ToArray();
                view.WaveChoices = view.Day ? 0 : visible.Length;
                view.SelectedWave = math.clamp(selectedWave, 0, math.max(0, visible.Length - 1));
                ...
                if (view.Day)
                {
                    ...
                    {
                        var name = EnemyDefinitions.Get(em, root, group.Key).Metadata.Name.ToString();
                        int count = group.Sum(w => w.Count);
                        var range = CountRange(count);
                        view.Lines.Add(view.Tier == 3 ? name + " × " + count : name + " · 约 " + range.x + "～" + range.y);
                        if (view.Tier == 3 && (EnemyDefinitions.Get(em, root, group.Key).Behavior & EnemyBehaviorFlags.Boss) != 0)
                            view.Lines.Add("大型威胁方位：" + string.Join("、", group.Select(w => Direction(w.Direction)).Distinct()));
                    }
                }
                else
                    view.Lines.Add(focus < 0 ? "暂无新的来袭方向" : "来袭方向：" + string.Join("、", waves.ToArray().Where(w => w.Spawned == 0 && w.SpatiallyBlocked == 0 && w.At == focus).Select(w => Direction(w.Direction)).Distinct()));
                foreach (var w in waves)
                {
                    if (w.SpatiallyBlocked != 0 || w.Spawned != 0)
                        continue;
                    var area = SpawnArea(em, root, w, view.Tier);
                    area.Secondary = !view.Day && w.At != focus;
                    view.Areas.Add(area);
        ...
        public static List<string> CourtLines(EntityManager em, Entity root, int known)
        {
            ...
            int tier = Tier(em.GetComponentData<IntelligenceSettings>(root), known);
            if (tier == 0)
            {
                lines.Add("暂无宫廷情报");
                return lines;
            ...
            bool threat = false;
            using var royals = WorldQueries.OrderedEntities<Royal>(em);
            foreach (var e in royals)
            {
                var p = em.GetComponentData<Royal>(e);
                if (!CourtOps.Eligible(em, king, e) || CourtOps.PlotChance(em, root, king, e) <= 0 && p.Evidence == 0)
                    continue;
                threat = true;
                if (tier >= 2)
                    lines.Add(em.GetComponentData<Identity>(e).Name + "：" + (tier == 3 ? (p.Evidence != 0 ? "已掌握弑君阴谋确凿证据" : "势力接近君王，有夺权动机，暂无确凿证据") : "其势力值得关注"));
            }

            lines.Add(threat ? "宫廷存在动荡迹象" : "目前未发现明显夺权迹象");
            return lines;
        }
