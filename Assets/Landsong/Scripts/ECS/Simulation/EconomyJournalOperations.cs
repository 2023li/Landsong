...
namespace Landsong.ECS
{
    ...
    public static class EconomyJournalOps
    {
        ...
        {
            EntityState.Buffer<EconomyEntry>(em, root);
            em.GetBuffer<EconomyEntry>(root).Clear();
            EntityState.Set(em, root, new EconomyJournalState { Turn = em.GetComponentData<GameClock>(root).Turn, Recording = 1, Forecast = (byte)(forecast ? 1 : 0) });
        }
        ...
        public static void DiscardPending(EntityManager em, Entity root)
        {
            ...
            var state = prior;
            state.Recording = 1;
            state.Reason = EconomyReason.NightDiscard;
            state.Source = 0;
            state.SourceName = "入夜清空";
            em.SetComponentData(root, state);
            try
            {
                foreach (var item in em.GetBuffer<PendingItem>(root))
                    Record(em, root, item.Item, -item.Amount, true);
                em.GetBuffer<PendingItem>(root).Clear();
            }
            finally
            {
                em.SetComponentData(root, prior);
            }
        ...
        public readonly struct Scope : IDisposable
        {
            ...
            {
                if (active)
                    em.SetComponentData(root, previous);
            }
        }

        public static void Record(EntityManager em, Entity root, ItemId item, int delta, bool pending = false, FixedString128Bytes note = default)
        {
            HistoryOps.Resource(em, root, item, delta, pending, note);
            if (!em.HasComponent<EconomyJournalState>(root))
                return;
            var s = em.GetComponentData<EconomyJournalState>(root);
            if (s.Recording == 0 || delta == 0 && note.IsEmpty)
                return;
    ...
    public sealed class InventoryTransaction : IDisposable
    {
        readonly EntityManager em;
        readonly Entity root;
        readonly NativeArray<InventorySlot> slots;
        readonly NativeArray<PendingItem> pending;
        readonly int journalCount, historyCount;
        ...
        public void Reject(FixedString128Bytes note)
        {
            Rollback();
            complete = true;
            EconomyJournalOps.Note(em, root, note);
        }

        public void Rollback()
        {
            em.GetBuffer<InventorySlot>(root).CopyFrom(slots);
            em.GetBuffer<PendingItem>(root).CopyFrom(pending);
            if (em.HasBuffer<EconomyEntry>(root))
                em.GetBuffer<EconomyEntry>(root).ResizeUninitialized(journalCount);
            if (em.HasBuffer<HistoryEntry>(root))
                em.GetBuffer<HistoryEntry>(root).ResizeUninitialized(historyCount);
        }
