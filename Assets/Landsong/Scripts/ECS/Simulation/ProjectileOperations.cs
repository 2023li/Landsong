...
{
    public struct ProjectileBlock
    {
        public byte Environment;
        public ulong Owner;
    }

    public struct ImpactTarget
    {
    ...
    {
        public static float Mitigate(float amount, float armor, float penetration, float reduction) => math.max(0, amount - math.max(0, armor - math.max(0, penetration))) * (1 - math.clamp(reduction, 0, .95f));
        public static NativeArray<ProjectileBlock> Blocks(EntityManager em, Entity root, Allocator allocator)
        {
            var grid = em.GetComponentData<GridData>(root);
            var blocks = new NativeArray<ProjectileBlock>(grid.Value.Value.Cells.Length, allocator);
            for (int i = 0; i < blocks.Length; i++)
                blocks[i] = new ProjectileBlock
                {
                    Environment = grid.Value.Value.Cells[i].BlocksProjectile
                };
            ...
            foreach (var e in sites)
            {
                ...
                if (!BuildingDefinitions.Get(em, root, em.GetComponentData<BuildingDefinitionRef>(e).Definition).DefenseStats.Profile.BlocksProjectile)
                    continue;
                ...
                for (int y = 0; y < bPlacement.Size.y; y++)
                    ...
                    {
                        int at = GridOps.Index(grid, bPlacement.Cell + new int2(x, y));
                        if (at < 0)
                            continue;
                        var cell = blocks[at];
                        cell.Owner = id.Id;
        ...
        {
            var a = (from.xz - grid.Origin.xz) / grid.CellSize;
            var b = (to.xz - grid.Origin.xz) / grid.CellSize;
            var delta = b - a;
            var cell = (int2)math.floor(a);
            var last = (int2)math.floor(b);
            ...
            {
                if (CellBlocked(grid, blocks, cell, source, target))
                    return true;
                if (math.all(cell == last))
                    return false;
                if (math.abs(next.x - next.y) < .00001f)
                {
                    if (CellBlocked(grid, blocks, cell + new int2(step.x, 0), source, target) || CellBlocked(grid, blocks, cell + new int2(0, step.y), source, target))
                        return true;
                    cell += step;
                    next += stride;
                }
        ...
        static bool CellBlocked(GridData grid, NativeArray<ProjectileBlock> blocks, int2 cell, ulong source, ulong target)
        {
            ...
            if (at < 0 || grid.Value.Value.Cells[at].Exists == 0)
                return true;
            var b = blocks[at];
            return b.Environment != 0 || b.Owner != 0 && b.Owner != source && b.Owner != target;
        }

        public static float3 Closest(ImpactTarget target, float3 from)
        {
            var point = target.Position;
            point.xz = math.clamp(from.xz, point.xz - target.HalfSize, point.xz + target.HalfSize);
            return point;
        }
    }
}
