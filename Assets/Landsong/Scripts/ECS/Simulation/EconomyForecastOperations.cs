...
namespace Landsong.ECS
{
    public static class EconomyForecastOps
    {
        ...
        {
            if (em.HasComponent<EconomyForecastState>(root))
                em.SetComponentData(root, new EconomyForecastState());
            if (em.HasBuffer<EconomyForecastEntry>(root))
                em.GetBuffer<EconomyForecastEntry>(root).Clear();
        }

        public static string Fingerprint(EntityManager em, Entity root)
        {
            using var hash = SHA256.Create();
            return Convert.ToBase64String(hash.ComputeHash(SnapshotCodec.Capture(em, root)));
        }
        ...
        public static ResultCode Create(EntityManager em, Entity root, Action<string> probe = null)
        {
            ...
            if (stateSettlement.LastSettledTurn == stateClock.Turn)
            ...
            using (var transaction = new RestoreTransaction(em, root))
            {
                ...
                var s = em.GetComponentData<Session>(candidate);
                s.Phase = Phase.Settlement;
                em.SetComponentData(candidate, s);
                DailyEconomySettlement.Settle(em, candidate, true);
                probe?.Invoke("forecast-settled");
                using var rows = em.GetBuffer<EconomyEntry>(candidate).ToNativeArray(Allocator.Temp);
                entries = rows.ToArray();
            }

            // Read-model only; the real journal, RNG and entire day have been restored.
            EntityState.Buffer<EconomyForecastEntry>(em, root);
            em.GetBuffer<EconomyForecastEntry>(root).Clear();
            foreach (var entry in entries)
                em.GetBuffer<EconomyForecastEntry>(root).Add(new EconomyForecastEntry { Value = entry });
            EntityState.Set(em, root, new EconomyForecastState { Turn = stateClock.Turn, Fingerprint = fingerprint });
