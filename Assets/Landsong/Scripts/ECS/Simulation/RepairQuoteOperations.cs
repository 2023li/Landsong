...
{
    public struct RepairPayment
    {
        public ItemId Item;
        public int Required, Pending, Normal, Available, Missing;
    }
    ...
    public static partial class BuildingCostOps
    {
        ...
        public static RepairQuote QuoteRepair(EntityManager em, Entity root, Entity building)
        {
            var q = new RepairQuote
            {
                ...
                Reason = "按当前库存可支付下一期；实际在白天结算时重新核对"
            };
            ...
            {
                q.Duration = math.max(1, bConstruction.RepairDuration);
                q.Step = bConstruction.Progress;
                foreach (var r in em.GetBuffer<RepairMaterial>(building))
                    Add(q.Total, r.Item, r.Amount);
            }
            else
            {
                q.Total.AddRange(RepairTotal(em, root, building, out q.Duration));
                q.Step = 0;
            }
            ...
            foreach (var c in q.Total)
            {
                ...
                var pending = math.min(required, InventoryOps.PendingCount(em, root, c.Item));
                var available = InventoryOps.Count(em, root, c.Item);
                var normal = required - pending;
                var missing = math.max(0, normal - available);
                ...
                if (missing > 0)
                {
                    ...
                    q.Reason = "当期材料不足，整期不扣款、不推进";
                }
            }

            if (q.NeedsNetwork)
                q.Provider = ResourceNetworkOps.Provider(em, root, building);
            if (q.NeedsNetwork && q.Provider == Entity.Null)
            {
                q.Code = ResultCode.Unavailable;
                q.Reason = "需要正常库存，但没有有效资源连接；整期暂停";
            }

            if (b.RuinPending != 0)
            {
                q.Code = ResultCode.Busy;
                q.Reason = "等待黎明提交本夜荒废，尚不能修复";
            }

            if (b.Stage != LifeStage.Ruined && b.Stage != LifeStage.Repairing)
            {
                q.Code = ResultCode.InvalidTarget;
                q.Reason = "建筑不处于荒废或修复状态";
            }
        ...
        public static bool PayRepairStep(EntityManager em, Entity root, Entity building)
        {
            ...
            {
                EconomyJournalOps.Note(em, root, q.Reason);
                return false;
            }

            using var payment = new InventoryTransaction(em, root);
            if (!Pay(em, root, q.Costs, true))
                return false;
            foreach (var c in q.Payments)
                ResourceNetworkOps.Record(em, root, q.Provider, c.Item, c.Normal);
            payment.Commit();
            return true;
        }
    }
