...
namespace Landsong.ECS.Authoring
{
    ...
    {
        [LabelText("游戏内容目录")] public GameCatalogAsset Catalog;
        [LabelText("运行地图")] public MapAsset Map;
        [LabelText("初始王朝名称")] public string DynastyName = "新王朝";
        [LabelText("初始基础人口")] public int BasePopulation;
        ...
        public sealed class Baker : Baker<GameWorldAuthoring>
        {
            public override void Bake(GameWorldAuthoring authoring)
            {
                ...
                foreach (var rule in ContentModuleCompiler.CompileRewards(catalog.StartingRewards,new ContentReferenceResolver(catalog))) grants.Add(new StartingGrant { Rule = rule });
        ...
        {
            var compiled = new ContentCompilation(catalog);
            BuildingModuleValidation.Validate(catalog,compiled);
            ValidateInventory(catalog,compiled);
            ValidateWorkforce(catalog,compiled);
            TechnologyContentValidation.Validate(catalog,compiled);
            QuestContentValidation.Validate(catalog,compiled);
            InvitationExpeditionValidation.Validate(catalog,compiled);
            CourtContentValidation.Validate(catalog,compiled);
            NightContentValidation.Validate(catalog,compiled);
            IntelligenceValidation.Validate(catalog,compiled);
            PeacefulContentValidation.Validate(catalog,compiled);
            foreach (var source in catalog.Content)
            {
                ...
                foreach (var cost in compiled.For(source)) if (cost.Kind == RuleKind.RecruitCost && (cost.Amount < 0 || cost.Level != 0 && cost.Level != 1 || cost.Target < 0 || catalog.Content[cost.Target].Kind != ContentKind.Item))
            ...
            NightContentValidation.Bake(catalog, builder, ref root,compiled);
            var content = catalog.Content; var compiledRules = new Rule[content.Length][]; var total = 0;
            for (int i = 0; i < content.Length; i++) { compiledRules[i] = compiled.For(content[i]); total += compiledRules[i].Length; }
            var definitions = builder.Allocate(ref root.Definitions, content.Length);
            var rules = builder.Allocate(ref root.Rules, total);
            var authoredTiers = new System.Collections.Generic.List<WorkerEfficiencyTier>();
            for (int i = 0; i < content.Length; i++)
            {
                var source = content[i];
                if (source.Kind != ContentKind.Building || source.Modules?.Workforce?.Enabled != true) continue;
                foreach (var tier in source.Modules.Workforce.EfficiencyTiers)
                    authoredTiers.Add(new WorkerEfficiencyTier { Definition = i, Level = tier.Level, MinimumWorkers = tier.MinimumWorkers, MaximumWorkers = tier.MaximumWorkers });
            }
            var workerTiers = builder.Allocate(ref root.WorkerTiers, authoredTiers.Count);
            for (int i = 0; i < authoredTiers.Count; i++) workerTiers[i] = authoredTiers[i];
            var at = 0;
            for (var i = 0; i < content.Length; i++)
            {
                var c = content[i];
                definitions[i] = new ContentDefinition { Id = new FixedString128Bytes(c.Id), Name = new FixedString128Bytes(c.Name), Kind = c.Kind, RuleStart = at, RuleCount = compiledRules[i].Length, Group = catalog.Find(c.Group), Level = c.Level, Capacity = c.Capacity, Duration = c.Duration, Value = c.Value, Limit = c.Limit, Size = new int2(c.Size.x, c.Size.y), Health = c.Health, Damage = c.Damage, Range = c.Range, Interval = c.Interval, Speed = c.Speed, ProjectileSpeed = c.ProjectileSpeed, Chance = c.Chance, Loss = c.Loss, Population = c.Population, Cost = c.Cost, Flags = c.Flags, SoldierGrowth = c.SoldierGrowth };
                ...
                entry.BuildingPolicy = (c.Building ?? new BuildingPolicySource()).Bake(c.Modules?.Maintenance?.Enabled==true?c.Modules.Maintenance.RepairTurns:0,c.Modules?.Garrison?.Enabled==true?c.Modules.Garrison.RecruitmentLimitPerTurn:0);
                ...
                definitions[i] = entry;
                foreach (var r in compiledRules[i])
                {
                    var rule = r;
        ...
        public static void ValidateWorkforce(GameCatalogAsset catalog,ContentCompilation compiled=null)
        {
            compiled??=new ContentCompilation(catalog);
            foreach (var d in catalog.Content) foreach (var r in compiled.For(d))
            {
        ...
        public static void ValidateInventory(GameCatalogAsset catalog,ContentCompilation compiled=null)
        {
            compiled??=new ContentCompilation(catalog);
            ...
            foreach (var d in content)
            {
                ...
                foreach (var r in compiled.For(d))
                {
                    ...
                    var at = r.Target; if (at < 0) throw new InvalidOperationException("Missing storage rule target: " + d.Id);
