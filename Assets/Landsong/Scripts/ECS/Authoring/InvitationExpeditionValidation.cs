        ...
        public static void Validate(GameCatalogAsset catalog, ContentCompilation compiled=null)
        {
            ...
            foreach (var d in catalog.Content)
            {
                if (d.Kind == ContentKind.Quest)
                {
                    ...
                    foreach (var r in compiled.For(d)) if (r.Kind == RuleKind.SubmitItem || r.Kind == RuleKind.RequireItem || r.Kind == RuleKind.RewardItem || r.Kind == RuleKind.FailureItem)
                ...
                foreach (var r in compiled.For(d))
                {
                    ...
                    {
                        var item = r.Target;
                        if (d.Kind != ContentKind.Expedition || item < 0 || catalog.Content[item].Kind != ContentKind.Item || r.Amount < 0 || r.Amount > 1000000 || r.B < 0 || r.B > r.Amount / 2 || !math.isfinite(r.Value) || r.Value < 0 || r.Value > 1 || !math.isfinite(r.Extra) || r.Extra < 0 || r.Extra > 1 || !supplyItems.Add(compiled.Id(r.Target)) || supplyItems.Count > 8) throw new InvalidOperationException("Invalid expedition supply (at most 8 unique items): " + d.Id);
                    }
                    if (r.Kind == RuleKind.VisiblePrerequisite && (d.Kind != ContentKind.Expedition || r.Target < 0 || r.Amount < 1)) throw new InvalidOperationException("Invalid destination visibility: " + d.Id);
                }
