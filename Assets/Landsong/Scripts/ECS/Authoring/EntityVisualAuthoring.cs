...
namespace Landsong.ECS.Authoring
{
    ...
    {
        [Sirenix.OdinInspector.LabelText("所属实体对象")]
        ...
        sealed class Baker : Baker<EntityVisualAuthoring>
        {
            public override void Bake(EntityVisualAuthoring authoring)
            {
                ...
                if (renderer != null)
                {
                    ...
                    var slot = GetComponentInParent<BuildingVisualSlotAuthoring>();
                    var part = GetComponentInParent<BuildingVisualPartAuthoring>();
    ...
    public partial struct EntityVisualSystem : ISystem
    {
        public void OnUpdate(ref SystemState state)
        {
            ...
            foreach (var(_, selected, entity)in SystemAPI.Query<RefRO<Building>, RefRW<BuildingVisualSelection>>().WithEntityAccess())
                selected.ValueRW.Slot = BuildingVisualResolver.Select(em, entity);
            foreach (var(owner, color, entity)in SystemAPI.Query<RefRO<VisualOwner>, RefRW<URPMaterialPropertyBaseColor>>().WithEntityAccess())
            {
                var root = owner.ValueRO.Owner;
                if (!em.Exists(root))
                    continue;
                var visible = true;
                var ruined = false;
                if (em.HasComponent<VisualState>(root))
                    visible = em.GetComponentData<VisualState>(root).Visible != 0;
                if (em.HasComponent<ExternalVisual>(root) && em.GetComponentData<ExternalVisual>(root).Active != 0)
                    visible = false;
                if (em.HasComponent<Building>(root))
                {
                    var b = em.GetComponentData<Building>(root);
                    BuildingFarmingState bFarming = em.GetComponentData<BuildingFarmingState>(root);
                    BuildingGatheringState bGathering = em.GetComponentData<BuildingGatheringState>(root);
                    var visual = owner.ValueRO;
                    ruined = b.Stage == LifeStage.Ruined || b.Stage == LifeStage.Repairing;
                    if (visual.Slot != Entity.Null)
                        visible &= (em.HasComponent<BuildingVisualSelection>(root) ? em.GetComponentData<BuildingVisualSelection>(root).Slot : BuildingVisualResolver.Select(em, root)) == visual.Slot;
                    if (visual.OperationalOnly != 0)
                        visible &= b.Stage == LifeStage.Operational;
                    if (visual.HarvestRemaining > 0)
                        visible &= bGathering.RemainingUses == visual.HarvestRemaining;
                    if (!visual.CropId.IsEmpty)
                    {
                        var simulation = em.GetComponentData<SimulationOwner>(root).Root;
                        if (!CropDefinitions.IsValid(em, simulation, bFarming.Crop))
                            visible = false;
                        else
                        {
                ...
                if (em.HasComponent<MaterialMeshInfo>(entity))
                    em.SetComponentEnabled<MaterialMeshInfo>(entity, visible);
            }
        }
    }
}
