...
namespace Landsong.ECS.Authoring
{
    ...
    public partial class BuildingVisualBakingSystem : SystemBase
    {
        protected override void OnUpdate()
        {
            ...
            {
                var parent = em.GetComponentData<AdditionalEntityParent>(e).Parent;
                if (!em.HasComponent<VisualOwner>(parent))
                    continue;
                var owner = em.GetComponentData<VisualOwner>(parent);
                Material material = em.GetComponentData<RenderMeshUnmanaged>(e).materialForSubMesh;
                var color = material != null && material.HasProperty("_BaseColor") ? material.GetColor("_BaseColor") : Color.white;
                owner.OriginalColor = new float4(color.r, color.g, color.b, color.a);
                EntityState.Set(em, e, owner);
                EntityState.Set(em, e, new URPMaterialPropertyBaseColor { Value = owner.OriginalColor });
            }
        }
    }
}
