...
{
    public static class TalentHeroJobEffectCompiler
    {
        public static void Compile(ref BlobBuilder builder, TalentHeroJobEffectSource source, ref global::Landsong.ECS.Definitions.TalentHeroJobEffect target, BuildingCatalogIndex buildingIndex, HeroCatalogIndex heroIndex, ItemCatalogIndex itemIndex)
        {
            if (source == null)
                throw new InvalidOperationException("缺少 TalentHeroJobEffect 配置。");
            if (source.Order < 0)
            ...
            target.Recipient = heroIndex.Resolve(source.Recipient, true);
