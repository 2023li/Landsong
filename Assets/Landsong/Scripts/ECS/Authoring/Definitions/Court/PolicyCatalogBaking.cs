...
namespace Landsong.ECS.Authoring.Definitions
{
    ...
    public static class PolicyCatalogBaking
    {
        public static BlobAssetReference<PolicyCatalogBlob> Compile(GameContentSetAsset content) => PolicyCatalogCompiler.Build(content.Get<PolicyCatalogAsset>(), new BuffCatalogIndex(content.Get<BuffCatalogAsset>()), new BuildingCatalogIndex(content.Get<BuildingCatalogAsset>()), new ExpeditionCatalogIndex(content.Get<ExpeditionCatalogAsset>()), new FeatureCatalogIndex(content.Get<FeatureCatalogAsset>()), new HeroCatalogIndex(content.Get<HeroCatalogAsset>()), new ItemCatalogIndex(content.Get<ItemCatalogAsset>()), new PolicyGroupCatalogIndex(content.Get<PolicyGroupCatalogAsset>()), new QuestCatalogIndex(content.Get<QuestCatalogAsset>()), new SoldierCatalogIndex(content.Get<SoldierCatalogAsset>()), new TalentCatalogIndex(content.Get<TalentCatalogAsset>()), new TechnologyCatalogIndex(content.Get<TechnologyCatalogAsset>()));
        public sealed class Baker : Baker<GameContentSetAuthoring>
        {
            public override void Bake(GameContentSetAuthoring authoring)
            {
                ...
                if (authoring.Content.Get<PolicyCatalogAsset>() == null)
                    throw new InvalidOperationException("缺少 政策领域目录。");
                DependsOn(authoring.Content.Get<PolicyCatalogAsset>());
                foreach (var asset in authoring.Content.Get<PolicyCatalogAsset>().Definitions)
                    DependsOn(asset);
                ...
                foreach (var asset in authoring.Content.Get<PolicyGroupCatalogAsset>().Definitions)
                    DependsOn(asset);
                var policyGroupIndex = new PolicyGroupCatalogIndex(authoring.Content.Get<PolicyGroupCatalogAsset>());
                ...
                var blob = PolicyCatalogCompiler.Build(authoring.Content.Get<PolicyCatalogAsset>(), buffIndex, buildingIndex, expeditionIndex, featureIndex, heroIndex, itemIndex, policyGroupIndex, questIndex, soldierIndex, talentIndex, technologyIndex);
                AddBlobAsset(ref blob, out _);
                var entity = GetEntity(TransformUsageFlags.None);
                AddComponent(entity, new PolicyCatalog { Value = blob });
            }
        }
    }
}
