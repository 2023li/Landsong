...
namespace Landsong.ECS.Authoring.Definitions
{
    ...
    public static class RoyalTraitCatalogBaking
    {
        public static BlobAssetReference<RoyalTraitCatalogBlob> Compile(GameContentSetAsset content) => RoyalTraitCatalogCompiler.Build(content.Get<RoyalTraitCatalogAsset>(), new BuffCatalogIndex(content.Get<BuffCatalogAsset>()), new BuildingCatalogIndex(content.Get<BuildingCatalogAsset>()), new ExpeditionCatalogIndex(content.Get<ExpeditionCatalogAsset>()), new FeatureCatalogIndex(content.Get<FeatureCatalogAsset>()), new HeroCatalogIndex(content.Get<HeroCatalogAsset>()), new ItemCatalogIndex(content.Get<ItemCatalogAsset>()), new QuestCatalogIndex(content.Get<QuestCatalogAsset>()), new SoldierCatalogIndex(content.Get<SoldierCatalogAsset>()), new TalentCatalogIndex(content.Get<TalentCatalogAsset>()), new TechnologyCatalogIndex(content.Get<TechnologyCatalogAsset>()));
        public sealed class Baker : Baker<GameContentSetAuthoring>
        {
            public override void Bake(GameContentSetAuthoring authoring)
            {
                ...
                if (authoring.Content.Get<RoyalTraitCatalogAsset>() == null)
                    throw new InvalidOperationException("缺少 王室特性领域目录。");
                ...
                var blob = RoyalTraitCatalogCompiler.Build(authoring.Content.Get<RoyalTraitCatalogAsset>(), buffIndex, buildingIndex, expeditionIndex, featureIndex, heroIndex, itemIndex, questIndex, soldierIndex, talentIndex, technologyIndex);
                AddBlobAsset(ref blob, out _);
                var entity = GetEntity(TransformUsageFlags.None);
                AddComponent(entity, new RoyalTraitCatalog { Value = blob });
            }
        }
    }
}
