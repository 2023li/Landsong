...
namespace Landsong.ECS.Authoring.Definitions
{
    ...
    public static class TalentCatalogBaking
    {
        public static BlobAssetReference<TalentCatalogBlob> Compile(GameContentSetAsset content) => TalentCatalogCompiler.Build(content.Get<TalentCatalogAsset>(), new BuffCatalogIndex(content.Get<BuffCatalogAsset>()), new BuildingCatalogIndex(content.Get<BuildingCatalogAsset>()), new ExpeditionCatalogIndex(content.Get<ExpeditionCatalogAsset>()), new FeatureCatalogIndex(content.Get<FeatureCatalogAsset>()), new HeroCatalogIndex(content.Get<HeroCatalogAsset>()), new ItemCatalogIndex(content.Get<ItemCatalogAsset>()), new QuestCatalogIndex(content.Get<QuestCatalogAsset>()), new RoyalTraitCatalogIndex(content.Get<RoyalTraitCatalogAsset>()), new SoldierCatalogIndex(content.Get<SoldierCatalogAsset>()), new TechnologyCatalogIndex(content.Get<TechnologyCatalogAsset>()));
        public sealed class Baker : Baker<GameContentSetAuthoring>
        {
            public override void Bake(GameContentSetAuthoring authoring)
            {
                ...
                if (authoring.Content.Get<TalentCatalogAsset>() == null)
                    throw new InvalidOperationException("缺少 人才领域目录。");
                ...
                var blob = TalentCatalogCompiler.Build(authoring.Content.Get<TalentCatalogAsset>(), buffIndex, buildingIndex, expeditionIndex, featureIndex, heroIndex, itemIndex, questIndex, royalTraitIndex, soldierIndex, technologyIndex);
                AddBlobAsset(ref blob, out _);
                var entity = GetEntity(TransformUsageFlags.None);
                AddComponent(entity, new TalentCatalog { Value = blob });
            }
        }
    }
}
