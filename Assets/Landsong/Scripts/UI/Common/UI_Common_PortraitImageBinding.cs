...
namespace Landsong.ECS.Presentation
{
    public sealed class UI_Common_PortraitImageBinding : MonoBehaviour
    {
        ...
        [Sirenix.OdinInspector.LabelText("画像覆盖目录"), Sirenix.OdinInspector.Required]
        ...
        {
            if (Target == null || Cache == null || Portraits == null)
        ...
        void Rebind()
        {
            if (!configured || !isActiveAndEnabled || Target.canvasRenderer.cull || world == null || !world.IsCreated)
                return;
            var em = manager;
            var person = WorldQueries.Find(em, id);
            ...
            dead = em.HasComponent<Royal>(person) ? !CourtOps.Alive(em, person) : em.HasComponent<Health>(person) && !EntityState.Alive(em, person);
            var dna = preview ?? em.GetComponentData<PortraitDNA>(person);
            string definition = "";
            if (em.HasComponent<TalentDefinitionRef>(person))
                definition = TalentDefinitions.Get(em, root, em.GetComponentData<TalentDefinitionRef>(person).Definition).Metadata.Id.ToString();
            else if (em.HasComponent<SoldierDefinitionRef>(person))
                definition = SoldierDefinitions.Get(em, root, em.GetComponentData<SoldierDefinitionRef>(person).Definition).Metadata.Id.ToString();
            else if (em.HasComponent<HeroDefinitionRef>(person))
                definition = HeroDefinitions.Get(em, root, em.GetComponentData<HeroDefinitionRef>(person).Definition).Metadata.Id.ToString();
            var fixedPortrait = Portraits.Face(definition, id);
