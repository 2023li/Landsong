    ...
    public sealed class UI_GamePanel_Marriage : Moyo.Unity.UIViewBase
    {
        ...
        internal void RefreshMarriageEvents()
        {
            ...
            if (MarriageOpen)
            {
                ...
                if (MarriageOpen)
                {
                    bool enabled = courtController.CourtDay && !intelligence.IsOpen && sessionController.em.GetComponentData<SimulationControl>(sessionController.root).Paused == 0;
            ...
            MarriageEventButton.gameObject.SetActive(count > 0 && !intelligence.IsOpen && (inputContext.PauseMenu == null || !inputContext.PauseMenu.IsOpen));
            if (count == 0)
                return;
            var pfirst = sessionController.em.GetComponentData<Royal>(WorldQueries.Find(sessionController.em, first));
        ...
        public void ShowMarriage(ulong id)
        {
            ...
            MarriageDetails(WorldQueries.Find(sessionController.em, marriageMate), MarriagePanel.Mate);
        ...
        internal void DecideMarriage(int option)
        {
            ...
            CloseMarriage();
            commandsController.TryQueue(new ResolveMarriageRequest { Person = id, Mate = mate, ExpectedRequestTurn = turn, Decision = option });
        }
        ...
        internal void MarriageDetails(Entity person, UI_GamePanel_RoyalPersonSummary view)
        {
            ...
            if (sessionController.em.HasComponent<Talent>(person))
            {
                ...
                lines.Add(t.Slot.IsValid ? "岗位：" + TalentSlotDefinitions.Get(sessionController.em, sessionController.root, t.Slot).Metadata.Name.ToString() + " · " + (t.Paid != 0 ? "已付薪" : "未付薪") : "当前未任职");
            ...
            foreach (var trait in sessionController.em.GetBuffer<TraitEntry>(person))
                ...
                {
                    traits = true;
                    lines.Add("特性：" + RoyalTraitDefinitions.Get(sessionController.em, sessionController.root, trait.Definition).Metadata.Name.ToString() + (trait.Active != 0 ? "（已激活）" : "（尚未激活）"));
                }

            if (!traits)
                lines.Add("暂无已知特性");
            if (p.FateUntil > 0)
                lines.Add("知天命：自然寿限还剩 " + System.Math.Max(0, p.FateUntil - sessionController.em.GetComponentData<GameClock>(sessionController.root).Turn) + " 回合");
        ...
        public void OpenMarriagePicker(ulong id)
        {
            ...
            if (!courtController.CourtDay || sessionController.em.GetComponentData<SimulationControl>(sessionController.root).Paused != 0 || !RoyalFamilyOps.CanArrange(sessionController.em, sessionController.root, person))
                return;
            commandsController.TryQueue(new PrepareMarriageRequest { Person = id });
        ...
        internal void RefreshMarriagePicker(Entity person)
        {
            ...
            if (marriageMate != 0 && !candidates.Contains(WorldQueries.Find(sessionController.em, marriageMate)))
            {
                marriageMate = 0;
                BuildMarriagePicker();
            }
        ...
        internal void BuildMarriagePicker()
        {
            ...
            BindMarriageButton(MarriagePanel.Approve, MarriagePanel.ApproveLabel, "确认赐婚", () =>
            {
                ...
                CloseMarriage();
                commandsController.TryQueue(new ArrangeMarriageRequest { Person = id, Mate = mate });
            });
