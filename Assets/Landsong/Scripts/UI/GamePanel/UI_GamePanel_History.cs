...
{
    public sealed class UI_GamePanel_History : GameFeatureViewBase
    {
        public override void Render() => HistoryRows();
        ...
        {
            historyCategory = -1;
            historyPage = historyFilterTurn = 0;
            historySearch = "";
            if (HistoryFilter != null)
                HistoryFilter.SetTextWithoutNotify("");
        }

        internal void InitializeInterface()
        {
            if (worldController.WorldPresentation == null || NavigationPanel == null || HistoryTools == null || HistoryFilter == null)
        ...
        internal void HistoryRows()
        {
            ...
            {
                historyFilterTurn = historyFilterTurn == 0 ? sessionController.em.GetComponentData<GameClock>(sessionController.root).Turn : 0;
                historyPage = 0;
                refresh.NextPanel = 0;
            });
            historyTools.gameObject.SetActive(true);
            var entries = new List<(int turn, string text, Action locate)>();
            var totals = new SortedDictionary<ItemId, (long income, long expense)>(Comparer<ItemId>.Create((a, b) => a.Index.CompareTo(b.Index)));
            if (sessionController.em.HasBuffer<HistoryEntry>(sessionController.root))
                foreach (var h in sessionController.em.GetBuffer<HistoryEntry>(sessionController.root))
                {
                    ...
                    if (h.Item.IsValid)
                        line += $" · {ItemDefinitions.Get(sessionController.em, sessionController.root, h.Item).Metadata.Name.ToString()} {(h.Delta > 0 ? "+" : "")}{h.Delta}（{(h.Pending != 0 ? "待存放" : "库存")}）";
                    if (h.Count > 1)
                        line += " × " + h.Count;
                    if (!string.IsNullOrWhiteSpace(historySearch) && line.IndexOf(historySearch, StringComparison.OrdinalIgnoreCase) < 0)
                        continue;
                    if (h.Item.IsValid && h.Transfer == 0)
                    {
            ...
            if (totals.Count > 0)
            {
                ...
                foreach (var t in totals)
                    rowsController.Row(ItemDefinitions.Get(sessionController.em, sessionController.root, t.Key).Metadata.Name.ToString() + " · 收入 +" + t.Value.income + " / 支出 -" + t.Value.expense + " / 净额 " + UI_GamePanel_Economy.Signed(t.Value.income - t.Value.expense));
            }
