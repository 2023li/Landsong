...
namespace Landsong.ECS.Presentation
{
    public sealed class TechnologyNodeModel
    {
        ...
        public List<TechnologyId> Prerequisites;
        public string Name;
    ...
    public sealed class UI_GamePanel_TechnologyTree : MonoBehaviour
    {
        ...
        public TechnologyId Selected;
        public int NodeCount => buttons.Count;
        public int EdgeCount => Lines.Edges.Count;

        readonly Dictionary<TechnologyId, UI_GamePanel_TechnologyNode> buttons = new Dictionary<TechnologyId, UI_GamePanel_TechnologyNode>();
        readonly Dictionary<TechnologyId, TechnologyNodeModel> models = new Dictionary<TechnologyId, TechnologyNodeModel>();
        float zoom = 1;
        Action<TechnologyId> selected;
        public Button NodeButton(TechnologyId definition) => buttons.TryGetValue(definition, out var b) ? b.Select : null;
        public void BindActions(Action<TechnologyId> select, Action close)
        {
        ...
        public void Bind(List<TechnologyNodeModel> nodes, TechnologyId selection, int points, TechnologyId current)
        {
            Selected = selection;
            Header.text = "科技树 · 可用研究点 " + points;
            CurrentButton.onClick.RemoveAllListeners();
            CurrentButton.interactable = current.IsValid;
            if (current.IsValid)
        ...
        void Layout()
        {
            ...
            foreach (var m in models.Values)
            {
                ...
                foreach (var parent in m.Prerequisites)
        ...
        public void Focus(TechnologyId definition)
        {
