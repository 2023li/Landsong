...
namespace Landsong.ECS.Presentation
{
    public sealed class UI_GamePanel_Intelligence : UI_GamePanel_List
    {
        ...
        internal void ResetSession() => ResetView();
        ...
        public override void Render()
        {
            ...
            Row("退出情报模式", Navigation.ClosePanel);
            ...
            foreach (var line in IntelOps.CourtLines(em, root, view.Known))
                Row(line);
            ...
            foreach (var source in view.Sources)
            {
                ...
                {
                    var e = WorldQueries.Find(em, owner);
                    if (e == Entity.Null)
                        return;
                    World.LocateHistory(owner, EntityState.Position(em, e));
                });
            }

            if (view.Unread && em.HasComponent<RecoveryState>(root))
                Commands.TryQueue(new ReadIntelligenceRequest { ExpectedFingerprint = em.GetComponentData<RecoveryState>(root).IntelFingerprint });
        }
    }
}
