    ...
    public class UI_GamePanel_List : UI_GamePanel_View
    {
        ...
        public override void Bind(GameUiSessionHandle session, GameUiCommandWriter commands, IGameUiNavigation navigation, GameUiRefreshScheduler refresh)
        {
            ...
            if (UsesConfiguredPresenter)
                ((IGameFeatureRenderer)Presenter).BindFeature(session, commands, navigation, this, refresh);
        }
        ...
        internal UI_GamePanel_Row Row(string label, Action action = null, bool right = false, RectTransform parent = null, string key = null, [System.Runtime.CompilerServices.CallerFilePath] string caller = "", [System.Runtime.CompilerServices.CallerLineNumber] int line = 0) => Rows.Row(label, action, parent ?? (right ? SecondaryRows : PrimaryRows), key, caller, line);
        internal T Item<T>(T template, string label, Action action = null, RectTransform parent = null, string key = null, [System.Runtime.CompilerServices.CallerFilePath] string caller = "", [System.Runtime.CompilerServices.CallerLineNumber] int line = 0)
            where T : UI_GamePanel_Row => Rows.Item(template, label, action, parent, key, null, caller, line);
