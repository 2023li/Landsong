...
namespace Landsong.ECS.Presentation
{
    ...
    public abstract class UI_GamePanel_View : MonoBehaviour
    {
        ...
        protected GameUiSessionHandle Session { get; private set; }
        protected GameUiRefreshScheduler Refresh { get; private set; }
        ...
        public bool IsFeatureUnlocked(EntityManager manager, Entity owner)
        {
            if (owner == Entity.Null || !manager.Exists(owner))
                return false;
            if (string.IsNullOrEmpty(RequiredFeatureId))
                return true;
            var definition = FeatureDefinitions.Find(manager, owner, new FixedString128Bytes(RequiredFeatureId));
            return !definition.IsValid ? AllowMissingFeature : FeatureUnlocks.Has(manager, owner, definition);
        }

        public bool CanOpen(EntityManager manager, Entity owner) => owner != Entity.Null && manager.Exists(owner) && (AllowLockedOpen || IsFeatureUnlocked(manager, owner));
        public virtual void ValidateConfiguration()
        {
            ...
            if (PanelId == GamePanelId.None || !Enum.IsDefined(typeof(GamePanelId), PanelId) || ContentRoot == null || Title == null || CloseButton == null)
                throw new InvalidOperationException(name + " 的功能面板引用不完整。");
        }

        public virtual void Bind(GameUiSessionHandle session, GameUiCommandWriter commands, IGameUiNavigation navigation, GameUiRefreshScheduler refresh)
        {
            if (session == null || commands == null || navigation == null)
                throw new ArgumentNullException("面板会话服务");
