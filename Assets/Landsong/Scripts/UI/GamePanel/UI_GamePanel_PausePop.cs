...
namespace Landsong.ECS.Presentation
{
    public sealed class UI_GamePanel_PausePop : MonoBehaviour
    {
        ...
        bool CanWrite => Valid && manager.GetComponentData<Session>(root).Phase == Phase.Day && manager.GetComponentData<PersistenceGate>(root).CheckpointPending == 0;

        public void Open()
        {
            if (IsOpen || !Valid || !View.InputPolicy.Capture().CanTogglePause)
                return;
            SimulationControl stateControl = manager.GetComponentData<SimulationControl>(root);
            previouslyPaused = stateControl.Paused != 0;
            closing = false;
            ...
            if (stateControl.Paused == 0)
                Queue(new PauseRequest { Decision = true ? PauseDecision.Pause : PauseDecision.Resume });
        }
        ...
        void Update()
        {
            ...
            SimulationControl stateControl = manager.GetComponentData<SimulationControl>(root);
            PersistenceGate statePersistence = manager.GetComponentData<PersistenceGate>(root);
            if (closing)
            {
                if (statePersistence.CheckpointPending != 0)
                    return;
                if (state.Phase != Phase.Ended && stateControl.Paused != (previouslyPaused ? 1 : 0))
                    Queue(new PauseRequest { Decision = previouslyPaused ? PauseDecision.Pause : PauseDecision.Resume });
                SetVisible(false);
                BackgroundGroup.interactable = previousInteraction;
                closing = false;
                return;
            ...
            if (stateControl.Paused == 0 && state.Phase != Phase.Ended && statePersistence.CheckpointPending == 0)
                Queue(new PauseRequest { Decision = true ? PauseDecision.Pause : PauseDecision.Resume });
        ...
        void OpenSaves()
        {
            ...
            navigation.OpenArchives(new ArchiveOpenRequest { Mode = ArchiveOpenMode.Save, Store = store, CurrentRun = run, IsSessionValid = () => Valid, CanWrite = () => CanWrite, Save = (slot, stamp, name) =>
            {
                if (slot == null)
                    Queue(new CreateSaveRequest { Label = new Unity.Collections.FixedString128Bytes(name) });
                else
                    Queue(new OverwriteSaveRequest { SlotId = new Unity.Collections.FixedString128Bytes(slot), ExpectedStamp = stamp });
            }, Load = (targetRun, slot, backup) =>
            {
                if (!CanWrite)
                    throw new InvalidOperationException("当前阶段不能载入。");
                if (targetRun == run && slot != null)
                {
                    Queue(new LoadSaveRequest { SlotId = new Unity.Collections.FixedString128Bytes(slot), Backup = backup });
                }
                else
