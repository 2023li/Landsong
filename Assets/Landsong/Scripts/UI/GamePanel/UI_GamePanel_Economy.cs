...
namespace Landsong.ECS.Presentation
{
    ...
    {
        [Sirenix.OdinInspector.LabelText("账单滚动视图")]
        public ScrollRect BillScroll;
        [Sirenix.OdinInspector.LabelText("回合账单模板")]
        public UI_GamePanel_BillTurn TurnTemplate;
        [Sirenix.OdinInspector.LabelText("无记录提示")]
        public TMP_Text EmptyState;
        [Sirenix.OdinInspector.LabelText("账单范围")]
        public TMP_Text ScopeLabel;
        [Sirenix.OdinInspector.LabelText("查看全城")]
        public Button AllSources;
        ...
        readonly Dictionary<(int turn, ItemId item), UI_GamePanel_BillRow> rows = new Dictionary<(int, ItemId), UI_GamePanel_BillRow>();
        ...
        public override void ValidateConfiguration()
        {
            ...
            if (BillScroll == null || BillScroll.content == null || TurnTemplate == null || TurnTemplate.TurnLabel == null || TurnTemplate.Rows == null || TurnTemplate.RowTemplate == null || EmptyState == null || ScopeLabel == null || AllSources == null)
        ...
        public override void Bind(GameUiSessionHandle session, GameUiCommandWriter commands, IGameUiNavigation navigation, GameUiRefreshScheduler refresh)
        {
            ...
            AllSources.onClick.AddListener(() => OpenEconomy());
        }

        public void OpenEconomy(ulong source = 0)
        {
            if (economySource != source)
                ClearAllRows();
        ...
        public override void Render()
        {
            ...
            if (em.HasBuffer<EconomyBillEntry>(root))
                foreach (var bill in em.GetBuffer<EconomyBillEntry>(root))
                {
                    if (bill.Source != economySource)
                        continue;
                    if (!groups.TryGetValue(bill.Turn, out var list))
                        groups.Add(bill.Turn, list = new List<EconomyBillEntry>());
                    if (bill.Item.IsValid)
                        list.Add(bill);
                }

            EmptyState.gameObject.SetActive(groups.Count == 0);
            EmptyState.text = "暂无账单记录。完成回合结算后显示。";
            var keys = new HashSet<(int, ItemId)>();
            int index = 1;
            foreach (var group in groups)
            {
                if (!turns.TryGetValue(group.Key, out var turn))
                {
                    turn = Instantiate(TurnTemplate, BillScroll.content);
                    turn.RowTemplate.gameObject.SetActive(false);
                    turn.gameObject.SetActive(true);
                    turns.Add(group.Key, turn);
                }

                turn.TurnLabel.text = group.Key + "回合";
                turn.transform.SetSiblingIndex(index++);
                foreach (var entry in group.Value.OrderBy(r => r.Item))
                {
                    var key = (group.Key, entry.Item);
                    keys.Add(key);
                    if (!rows.TryGetValue(key, out var row))
                    {
                        row = Instantiate(turn.RowTemplate, turn.Rows);
                        row.gameObject.SetActive(true);
                        rows.Add(key, row);
                    }

                    row.Show(ItemDefinitions.Get(em, root, entry.Item).Metadata.Name.ToString(), entry);
                }
            }

            foreach (var key in rows.Keys.Where(k => !keys.Contains(k)).ToArray())
            {
                Destroy(rows[key].gameObject);
                rows.Remove(key);
            }

            foreach (var key in turns.Keys.Where(k => !groups.ContainsKey(k)).ToArray())
            {
                Destroy(turns[key].gameObject);
                turns.Remove(key);
            }
        }
