...
namespace Landsong.ECS.Presentation
{
    ...
    public sealed class GameUiInputPolicy
    {
        ...
        public GameUiInputSnapshot Capture()
        {
            ...
            if (soldier.SoldierDetailsOpen)
                owners |= GameUiInputOwner.SoldierDetails;
            if (portrait.PortraitOpen)
                owners |= GameUiInputOwner.Portrait;
            if (requests.PersonRequestsOpen)
                owners |= GameUiInputOwner.PersonRequests;
            if (marriage.MarriageOpen)
        ...
        public static GameUiInputSnapshot Evaluate(bool sessionBound, bool gameReady, GameUiInputOwner owners, bool intelligence, bool textFocused, GamePanelId panel, bool hasPauseMenu) => new GameUiInputSnapshot(sessionBound, gameReady, owners, intelligence, textFocused, panel, hasPauseMenu);
    }
    ...
    public readonly struct GameUiInputSnapshot
    {
        ...
        internal GameUiInputSnapshot(bool sessionBound, bool gameReady, GameUiInputOwner owners, bool intelligence, bool textFocused, GamePanelId panel, bool hasPauseMenu)
        {
        ...
        {
            if (requested != GameUiInputOwner.SoldierDetails && requested != GameUiInputOwner.Portrait && requested != GameUiInputOwner.PersonRequests && requested != GameUiInputOwner.Marriage && requested != GameUiInputOwner.BuildingConfirmation && requested != GameUiInputOwner.InventoryDetails)
        ...
        public GameUiInputOwner BackOwner(bool includeClosedPause = false)
        {
            ...
            {
                if (rule.Owner == GameUiInputOwner.Application || rule.Owner == GameUiInputOwner.BuildingConfirmation)
                    continue;
        ...
        {
            if (!SessionBound || !GameReady)
                return false;
            var access = Intelligence ? CommandAccess.Pause | CommandAccess.Archive | CommandAccess.Intelligence | CommandAccess.Camera : CommandAccess.All;
            foreach (var rule in Rules)
