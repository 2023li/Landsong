...
namespace Landsong.EditorTools
{
    public static class GameMapPaths
    {
        ...
        public const string LayeredRules = Root + "/分层地形规则.asset";
        public const string LayeredTemplate = Root + "/分层模板_TWC配置.asset";
        ...
        public static string[] BakedScenes() => AssetDatabase.FindAssets("t:MapAsset", new[] { Root }).Select(g => AssetDatabase.LoadAssetAtPath<MapAsset>(AssetDatabase.GUIDToAssetPath(g))).Where(m => m != null && m.IsBaked).Select(m => AssetDatabase.GUIDToAssetPath(m.EntitySceneGuid)).OrderBy(p => p, StringComparer.Ordinal).ToArray();
        public static string[] BakedMapPaths() => AssetDatabase.FindAssets("t:MapAsset", new[] { Root }).Select(AssetDatabase.GUIDToAssetPath).Where(p => AssetDatabase.LoadAssetAtPath<MapAsset>(p).IsBaked).ToArray();
    }
}
