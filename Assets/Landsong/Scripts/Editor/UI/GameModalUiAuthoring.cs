...
namespace Landsong.ECS.Editor
{
    ...
    {
        const string Views = "Assets/Landsong/UI/Prefabs/GamePanel/Views/";
        ...
        public static void Configure()
        {
            ...
            Edit("Soldier", contents =>
            {
                ...
                binding.Target = panel.Portrait;
                panel.PortraitBinding = binding;
            ...
            var gameContents = PrefabUtility.LoadPrefabContents(ApplicationUiAuthoring.GamePath);
            try
            {
                ...
                PrefabUtility.SaveAsPrefabAsset(gameContents, ApplicationUiAuthoring.GamePath);
            }
            finally
            {
                PrefabUtility.UnloadPrefabContents(gameContents);
            }
        ...
        static void Edit(string name, Action<GameObject> apply)
        {
            if (EditorApplication.isPlayingOrWillChangePlaymode)
                throw new InvalidOperationException("请退出运行模式后修复弹窗布局。");
            ...
            try
            {
                ...
                if (PrefabUtility.SaveAsPrefabAsset(contents, path) == null)
                    throw new InvalidOperationException("无法保存弹窗配置：" + name);
            }
            finally
            {
        ...
        static void Actions(params Button[] buttons)
        {
            ...
            parent.anchorMin = new Vector2(.06f, 0);
            parent.anchorMax = new Vector2(.94f, 0);
            parent.pivot = new Vector2(.5f, 0);
            parent.anchoredPosition = new Vector2(0, 12);
            parent.sizeDelta = new Vector2(0, 42);
            ...
            foreach (var button in buttons)
            {
                ...
                element.minHeight = element.preferredHeight = 42;
        ...
        static void PortraitSample(UIViewBase owner, Image target)
        {
            ...
            if (sprite == null)
            {
                var config = AssetDatabase.LoadAssetAtPath<PortraitConfig>("Assets/Landsong/ECSContent/Presentation/Portraits/PortraitConfig.asset");
                ...
                texture.name = "SoldierPortraitSample";
            ...
            if (sample == null)
                sample = target.transform.Find("EditorPortraitSample");
            if (sample == null)
            {
                var created = new GameObject("PreviewOnly_SoldierPortrait", typeof(RectTransform), typeof(Image));
                created.transform.SetParent(target.transform, false);
                sample = created.transform;
            }

            sample.name = "PreviewOnly_SoldierPortrait";
            sample.gameObject.tag = "EditorOnly";
            var rect = (RectTransform)sample;
            rect.anchorMin = Vector2.zero;
            rect.anchorMax = Vector2.one;
            rect.offsetMin = rect.offsetMax = Vector2.zero;
            var image = sample.GetComponent<Image>();
            image.sprite = sprite;
