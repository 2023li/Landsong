...
namespace Landsong.ECS.Editor
{
    ...
    public static class SimulationQueryBenchmark
    {
        ...
        const string Source = "Assets/Landsong/Scripts/ECS/Simulation/Services/WorldQueries.cs";
        ...
        public static string Run()
        {
            ...
            var report = new RunReport
            {
                ...
                entitiesAssemblyVersion = typeof(World).Assembly.GetName().Version.ToString(),
                sourceSha256 = Sha256(Source),
                coreAssemblySha256 = Sha256(typeof(WorldQueries).Assembly.Location),
                packageLockSha256 = Sha256("Packages/packages-lock.json"),
                seedHex = Seed.ToString("X8"),
                warmupBatches = WarmupBatches,
                measuredBatches = Samples,
                findCallsPerBatch = FindBatch,
                orderedCallsPerBatch = OrderBatch,
        ...
        static void SampleSize(RunReport report, int count)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                em.SetComponentData(created[i], new Identity { Id = Id(ranks[i]) });
            }
            ...
            using (var actual = WorldQueries.Entities<Identity>(em))
            {
                if (actual.Length != count)
                    throw new InvalidOperationException("性能夹具实体数量不符。");
                first = actual[0];
                middle = actual[count / 2];
                last = actual[count - 1];
                random = Seed ^ 0x9E3779B9;
                for (int i = 0; i < uniform.Length; i++)
                    uniform[i] = actual[(int)(Next(ref random) % (uint)count)];
            }
            ...
            var missing = new ulong[FindBatch];
            for (int i = 0; i < missing.Length; i++)
                missing[i] = ulong.MaxValue;
            ...
            report.results.Add(Measure(count, "Harness.find-checksum", "Loop/checksum reference; reported separately, never subtracted", FindBatch, () => EntitySequenceHash(firstSequence), expectedFind, 0));
            ulong expectedOrdered = HashStart;
            for (int i = 0; i < OrderBatch; i++)
                expectedOrdered = OrderHash(expectedOrdered, count, sorted[0], sorted[count - 1]);
            report.results.Add(Measure(count, "OrderedEntities.Building", "Shuffled unique IDs; full ascending identity sort", OrderBatch, () =>
            {
                ...
                for (int i = 0; i < OrderBatch; i++)
                {
                    using var ordered = WorldQueries.OrderedEntities<Building>(em);
        ...
        static void AddFind(RunReport report, EntityManager em, int count, string name, string distribution, Entity[] expected, ulong[] ids, long payload = -1)
        {
            for (int i = 0; i < ids.Length; i++)
                if (WorldQueries.Find(em, ids[i]) != expected[i])
                    throw new InvalidOperationException("Find 夹具结果不符：" + name);
            report.results.Add(Measure(count, name, distribution, FindBatch, () =>
            {
                ulong checksum = HashStart;
                for (int i = 0; i < ids.Length; i++)
                    checksum = EntityHash(checksum, WorldQueries.Find(em, ids[i]));
                return checksum;
        ...
        static Result Measure(int count, string operation, string distribution, int calls, Func<ulong> invoke, ulong expected, long nativePayload)
        {
            for (int i = 0; i < WarmupBatches; i++)
                if (invoke() != expected)
                    throw new InvalidOperationException("预热结果校验失败：" + operation);
            ...
            for (int i = 0; i < Samples; i++)
            {
                ...
                if (checksum != expected || allocated[i] < 0)
                    throw new InvalidOperationException("采样结果或分配计数校验失败：" + operation);
            ...
            var result = new Result
            {
                ...
                validatedChecksum = expected.ToString("X16"),
                minimumNativePayloadBytesPerCall = nativePayload
            };
            ...
            for (int i = 0; i < Samples; i++)
            {
                ...
                result.meanMicroseconds += times[i] / Samples;
                result.meanManagedBytesPerCall += bytes[i] / Samples;
            }

            Array.Sort(times);
            Array.Sort(bytes);
            result.minMicroseconds = times[0];
            result.maxMicroseconds = times[Samples - 1];
            result.medianMicroseconds = times[Samples / 2];
            result.p95Microseconds = times[(int)Math.Ceiling(Samples * .95) - 1];
            result.minManagedBytesPerCall = bytes[0];
            result.maxManagedBytesPerCall = bytes[Samples - 1];
            return result;
        }
        ...
        static void ValidateOrdered(EntityManager em, Entity[] expected)
        {
            ...
            if (ordered.Length != expected.Length)
                throw new InvalidOperationException("排序数量不符。");
        ...
        static Entity[] Repeat(Entity entity)
        {
            var values = new Entity[FindBatch];
            for (int i = 0; i < values.Length; i++)
                values[i] = entity;
            return values;
        }

        static ulong[] Ids(EntityManager em, Entity[] values)
        {
            var ids = new ulong[values.Length];
            for (int i = 0; i < values.Length; i++)
                ids[i] = em.GetComponentData<Identity>(values[i]).Id;
            return ids;
        }

        static ulong Id(int rank) => 100001UL + (ulong)rank * 17UL;
        static uint Next(ref uint value)
        {
            value ^= value << 13;
            value ^= value >> 17;
            value ^= value << 5;
        ...
        static ulong EntitySequenceHash(Entity[] values)
        {
            ulong hash = HashStart;
            for (int i = 0; i < values.Length; i++)
                hash = EntityHash(hash, values[i]);
            return hash;
        }
        ...
        static string Markdown(RunReport report)
        {
            ...
            text.Append("Unity ").Append(report.unityVersion).Append("；").Append(report.os).Append("；").Append(report.processor).Append("；逻辑处理器 ").Append(report.processorCount).Append("；内存 ").Append(report.memoryMegabytes).Append(" MB；Collections 检查 ").Append(report.collectionChecks).Append("。\n\n");
