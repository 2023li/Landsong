...
namespace Landsong.ECS.Editor
{
    public static class InventoryUiVerification
    {
        public static void PreparePreview(UI_GamePanel_Inventory view, string mode)
        {
            ...
            view.BuildingsScroll.gameObject.SetActive(buildings);
            view.ResourcesScroll.gameObject.SetActive(!buildings);
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                pending.Icon.sprite = woodIcon;
                pending.Label.text = i == 0 ? "原木 × 20" : "石头 × 5";
            ...
            if (buildings)
                for (int i = 0; i < 3; i++)
                {
                    ...
                    for (int j = 0; j < 8; j++)
                    {
                        ...
                        slot.Icon.sprite = woodIcon;
                        slot.Label.text = j < 3 ? (20 + j * 5).ToString() : "空";
                        slot.Icon.gameObject.SetActive(j < 3);
                ...
                for (int i = 0; i < 4; i++)
                {
                    ...
                    row.Icon.sprite = woodIcon;
                    row.Information.text = new[]
                    {
                        "原木",
                        "石头",
                        "木板",
        ...
        public static string Run()
        {
            ...
            try
            {
                var prefab = AssetDatabase.LoadAssetAtPath<GameObject>(InventoryUiAssets.Path);
                var view = prefab.GetComponent<UI_GamePanel_Inventory>();
                ...
                Check(!view.BuildingTemplate.gameObject.activeSelf && !view.ResourceTemplate.gameObject.activeSelf && !view.PendingTemplate.gameObject.activeSelf && !view.ResourceDetailsRoot.activeSelf, "Templates and ledger start hidden");
                ...
                var rootView = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Landsong/UI/Prefabs/GamePanel/UI_GamePanel.prefab").GetComponent<UI_GamePanel>();
                ...
                using var world = new World("Inventory read models");
                var em = world.EntityManager;
                var root = em.CreateEntity();
                em.AddBuffer<InventorySlot>(root).Add(new InventorySlot { Provider = 1, Index = 0, Item = ItemId.FromIndex(4), Count = 10 });
                em.GetBuffer<InventorySlot>(root).Add(new InventorySlot { Provider = 2, Index = 0, Item = ItemId.FromIndex(4), Count = 7 });
                em.GetBuffer<InventorySlot>(root).Add(new InventorySlot { Provider = 3, Index = 0, Item = ItemId.FromIndex(4), Count = 30, Unavailable = 1 });
                em.AddBuffer<PendingItem>(root).Add(new PendingItem { Item = ItemId.FromIndex(4), Amount = 5 });
                em.GetBuffer<PendingItem>(root).Add(new PendingItem { Item = ItemId.FromIndex(6), Amount = 2 });
                var resources = InventoryReadModel.Resources(em, root);
                Check(resources[ItemId.FromIndex(4)].Stored == 17 && resources[ItemId.FromIndex(4)].Pending == 5 && resources[ItemId.FromIndex(6)].Stored == 0 && resources[ItemId.FromIndex(6)].Pending == 2, "Resources combine providers, exclude unavailable stock, and retain pending-only kinds");
                em.AddBuffer<EconomyForecastEntry>(root);
                void Entry(int delta, EconomyReason reason, byte pending = 0, int item = 4, int turn = 9) => em.GetBuffer<EconomyForecastEntry>(root).Add(new EconomyForecastEntry { Value = new EconomyEntry { Turn = turn, Item = ItemId.FromIndex(item), Delta = delta, Pending = pending, Reason = reason } });
                Entry(20, EconomyReason.Production);
                Entry(-8, EconomyReason.Production);
                Entry(-2, EconomyReason.NaturalLoss);
                Entry(-3, EconomyReason.CapacityTransfer);
                Entry(3, EconomyReason.CapacityTransfer, 1);
                Entry(4, EconomyReason.Tax);
                Entry(100, EconomyReason.Production, item: 6);
                ...
                Check(!input.CanNavigate && !input.CanWorldInput && !input.CanQueue(CommandKind.MoveInventoryToPending) && input.BackOwner() == GameUiInputOwner.InventoryDetails, "Ledger owns back input and blocks gameplay underneath");
                log.AppendLine("PASS " + DateTime.Now.ToString("O") + " Assertions: " + assertions);
                return log.ToString();
            }
            ...
            finally
            {
                ...
                File.WriteAllText("Library/LandsongEcs/inventory-ui-configuration-verification.txt", log.ToString());
            }
        }
    }
}
