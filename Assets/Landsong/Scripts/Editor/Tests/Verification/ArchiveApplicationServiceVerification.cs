...
namespace Landsong.ECS.Editor
{
    public static class ArchiveApplicationServiceVerification
    {
        ...
        static void Reject(Action action, string label)
        {
            ...
            catch (Exception error)when (error is InvalidOperationException || error is InvalidDataException || error is ArgumentException)
            {
                rejected = true;
            }
        ...
        public static string Run()
        {
            ...
            if (Path.GetDirectoryName(directory) != parent || Directory.Exists(directory))
                throw new InvalidOperationException("Expected an unused owned fixture directory.");
            try
            {
                var store = new RunArchiveStore(directory);
                var archive = CreateArchive();
                archive.RunId = Guid.NewGuid().ToString("N");
                var roundTrip = RunArchiveCodec.Decode(RunArchiveCodec.Encode(archive));
                Check(roundTrip.Dusk == null && roundTrip.Manual == null && roundTrip.ManualDay == null,
                    "ES3 archive DTO round-trips absent optional checkpoints");
                store.Write(archive);
                store.Write(archive);
        ...
        static void LoadMode(RunArchiveStore store, RunArchive archive, string slot)
        {
            ...
            var service = new ArchiveApplicationService(new ArchiveOpenRequest { Mode = ArchiveOpenMode.Load, Store = store, Load = (run, selected, backup) => loads.Add((run, selected, backup)) });
            var automatic = service.Describe(archive.RunId);
            var selected = service.Describe(archive.RunId, slot);
            Check(service.IsValid && !service.CanWrite && automatic.Valid && selected.Valid,
                "Load mode reads valid automatic and manual nodes without gameplay save permission; automatic=" + automatic.Error + "; selected=" + selected.Error);
            ...
            service.Load(automatic, false);
            service.Load(selected, true);
        ...
        static void SaveMode(RunArchiveStore store, RunArchive archive, RunArchive other, string slot, string otherSlot)
        {
            ...
            int loads = 0;
            var request = new ArchiveOpenRequest
            {
                Mode = ArchiveOpenMode.Save,
                Store = store,
                CurrentRun = archive.RunId,
                CanWrite = () => canWrite,
                Save = (target, stamp, label) => saves.Add((target, stamp, label)),
                ManageSlot = (target, label, stamp, delete) => management.Add((target, label, stamp, delete)),
                Load = (_, _, _) => loads++
            };
            var service = new ArchiveApplicationService(request);
            var selected = service.Describe(archive.RunId, slot);
            var automatic = service.Describe(archive.RunId);
            Check(service.CanWrite, "Live save mode has explicit save capability");
            service.Create("新建验收槽");
            service.Overwrite(selected);
            service.Rename(selected, "重命名请求");
            service.Delete(selected);
            service.Load(selected, false);
        ...
        static void StaleConfirmation(RunArchiveStore store, RunArchive archive, string slot)
        {
            ...
            {
                store.RunPath(archive.RunId),
                store.RunPath(archive.RunId) + RunArchiveStore.BackupSuffix,
                store.SlotPath(archive.RunId, slot)
            }
            ...
            )
            {
                ...
                changedBytes[changedBytes.Length / 2] ^= 0x01;
                File.WriteAllBytes(path, changedBytes);
                File.SetLastWriteTimeUtc(path, originalTime);
                Check(new FileInfo(path).Length == originalBytes.Length && File.GetLastWriteTimeUtc(path) == originalTime, "Stale-content fixture retains file length and timestamp: " + Path.GetFileName(path));
        ...
        static void SessionExpiry(RunArchiveStore store, RunArchive archive, string slot)
        {
            bool valid = true;
            int dispatched = 0;
            var service = new ArchiveApplicationService(new ArchiveOpenRequest { Mode = ArchiveOpenMode.Save, Store = store, CurrentRun = archive.RunId, IsSessionValid = () => valid, CanWrite = () => true, Save = (_, _, _) => dispatched++, ManageSlot = (_, _, _, _) => dispatched++, Load = (_, _, _) => dispatched++ });
            var selected = service.Describe(archive.RunId, slot);
            valid = false;
