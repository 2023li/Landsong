...
namespace Landsong.ECS.Editor
{
    public static class SessionBoundaryVerification
    {
        ...
        static void Lifetime()
        {
            ...
            Check(!em.Exists(nullOwner) && !em.Exists(retiredOwner) && !em.Exists(invalidOwner), "Cleanup removes null, destroyed and non-session owners including disabled instances");
            Check(em.Exists(active) && em.Exists(disabled) && em.Exists(prefab), "Cleanup retains valid active and disabled instances and baked templates");
            em.AddComponent<Disabled>(root);
            SimulationLifetimeSystem.Cleanup(em);
            Check(!SimulationLifetimeSystem.IsReleased(em) && em.Exists(active) && em.Exists(disabled), "A disabled Session remains a valid owner and blocks release");
            em.RemoveComponent<Disabled>(root);
            using (var transaction = new RestoreTransaction(em, root))
            {
                Check(em.HasComponent<Disabled>(root) && WorldQueries.Root(em) == transaction.Root, "Restore stages a candidate while hiding the original root");
                SimulationLifetimeSystem.Cleanup(em);
                Check(em.Exists(root) && em.Exists(active) && em.Exists(disabled) && !SimulationLifetimeSystem.IsReleased(em), "Ownership cleanup does not delete the originals isolated by synchronous restore");
            }

            Check(WorldQueries.Root(em) == root && em.Exists(active) && em.Exists(disabled) && em.HasComponent<Disabled>(disabled), "Aborted staging restores original visibility and preserves pre-existing disabled state");
            ...
            SimulationLifetimeSystem.Cleanup(em);
            Check(!em.Exists(active) && !em.Exists(disabled) && SimulationLifetimeSystem.IsReleased(em), "All active and disabled orphans are released after map ownership ends");
            ...
            SimulationLifetimeSystem.Cleanup(em);
            Check(!em.Exists(disabledOrphan) && SimulationLifetimeSystem.IsReleased(em), "Cleanup makes the disabled-only orphan world releasable");
        }
        ...
        static void QuestEntry(string path)
        {
            ...
            try
            {
                ...
                var definition = QuestDefinitions.Find(em, root, "random_supply_wood");
                Check(definition.IsValid, "Fixture uses a registered ordinary quest with a real failure penalty");
                ...
                var importedQuest = strictImport.Records.OfType<QuestSnapshot>().FirstOrDefault(r => r.Quest.Status == QuestStatus.Active || r.Quest.Status == QuestStatus.Completed);
                Check(importedQuest != null, "Strict import fixture contains an existing bound quest");
                var invalidImportQuest = importedQuest.Quest;
                invalidImportQuest.ContainerSlot = int.MaxValue;
                ...
                var quest = QuestLifecycle.CreateQuest(em, root, definition, coreId);
                var questState = em.GetComponentData<Quest>(quest);
                questState.Status = QuestStatus.Active;
                questState.Container = coreId;
                questState.ContainerSlot = em.GetComponentData<BuildingQuestStats>(core).Capacity + 100;
                questState.StartTurn = em.GetComponentData<GameClock>(root).Turn;
                questState.Deadline = questState.StartTurn + QuestDefinitions.Get(em, root, definition).DeadlineTurns;
                ...
                statePopulation.BasePopulation += 10;
                {
                ...
                SoldierCombatants.Configure(em, root, soldier, false, 0, EntityState.Position(em, core));
                EntityState.Set(em, soldier, new Soldier { PopulationCost = 1 });
                SoldierLifeOps.InitializePerson(em, root, soldier);
                ...
                {
                    Check(snapshot.SequenceEqual(SnapshotCodec.Capture(em, root)) && em.Exists(quest) && questState.Equals(em.GetComponentData<Quest>(quest)), label + " preserves live task, penalty resources, RNG and day");
                    Check(originalEntities.SequenceEqual(Entities(em)) && originalEvents.SequenceEqual(Buffer<GameEvent>(em, root)) && originalHistory.SequenceEqual(Buffer<HistoryEntry>(em, root)), label + " preserves entity handles, events and history");
                }
                ...
                var corruptProgress = questState;
                corruptProgress.StartTurn = em.GetComponentData<GameClock>(root).Turn + 1;
                ...
                var otherQuest = WorldQueries.Find(em, importedQuest.Identity.Id);
                var otherState = em.GetComponentData<Quest>(otherQuest);
                var duplicate = otherState;
                duplicate.Container = questState.Container;
                duplicate.ContainerSlot = questState.ContainerSlot;
                ...
                )
                {
                    ...
                    try
                    {
                        NightEntryOps.Begin(em, root, false, 0, stage =>
                        {
                            ...
                            {
                                var candidate = WorldQueries.Root(em);
                                Check(candidate != root && WorldQueries.Find(em, questId) == Entity.Null && InventoryOps.Count(em, candidate, penalty.Item) == stock - penalty.Amount, "Invalid task and its penalty are reconciled only on the staged candidate");
                            }
                ...
                Check(NightEntryOps.Begin(em, root, false, 0) == ResultCode.ConfirmationRequired, "A pending soldier forces review after candidate task reconciliation");
                Check(Buffer<NightEntryLoss>(em, root).Any(loss => loss.Soldier == soldierId), "Review names the actual pending soldier");
                Unchanged(before, "Unconfirmed entry/cancel");
                var review = em.GetComponentData<NightEntryReview>(root);
                Check(review.Fingerprint.ToString() == EconomyForecastOps.Fingerprint(em, root), "Review fingerprint describes the unchanged live day before task reconciliation");
                ...
                Check(NightEntryOps.Begin(em, root, true, review.Token) == ResultCode.ConfirmationRequired, "A stale confirmation cannot commit task removal or its penalty");
                Unchanged(changed, "Stale confirmation");
                var refreshed = em.GetComponentData<NightEntryReview>(root);
                Check(refreshed.Token != review.Token && refreshed.Fingerprint.ToString() == EconomyForecastOps.Fingerprint(em, root), "Changed state receives a fresh consent token and matching live fingerprint");
                Check(NightEntryOps.Begin(em, root, true, refreshed.Token) == ResultCode.Success, "Exact consent commits the candidate day");
                Check(WorldQueries.Find(em, questId) == Entity.Null && WorldQueries.Find(em, soldierId) == Entity.Null && em.GetComponentData<Session>(root).Phase == Phase.Deployment, "Successful commit removes the invalid task and approved pending soldier");
                int PenaltyEvents() => Buffer<GameEvent>(em, root).Count(e => e.Kind == EventKind.Message && e.Target == questId && e.Message.ToString() == "任务惩罚已扣除" && e.Amount == penalty.Amount);
                ...
                Check(NightEntryOps.Begin(em, root, true, refreshed.Token) == ResultCode.WrongPhase && committed.SequenceEqual(SnapshotCodec.Capture(em, root)) && PenaltyEvents() == 1, "Repeated confirmation cannot charge the committed task again");
            }
            finally
            {
