...
namespace Landsong.ECS.Editor
{
    public static partial class CourtVerification
    {
        static void FamilyRules(EntityManager em, Entity root)
        {
            ...
            void Gender(Entity e, PersonGender gender)
            {
            ...
            var child = DynastyOps.CreateRoyal(em, root, "赐婚测试子女", 2, 24, Id(king));
            ...
            Gender(child, PersonGender.Female);
            Gender(mate, PersonGender.Male);
            Gender(grandchild, PersonGender.Male);
            Gender(grandMate, PersonGender.Female);
            ...
            var pending = SnapshotCodec.Capture(em, root);
            var decoded = SnapshotCodec.Decode(em, root, pending);
            Check(decoded.Records.OfType<PersonSnapshot>().Single(r => r.Identity.Id == Id(child)).Royal.RequestedSpouse == Id(mate), "Pending request survives codec round trip");
            var bad = SnapshotCodec.Decode(em, root, pending);
            var index = Array.FindIndex(bad.Records, r => r.Identity.Id == Id(child));
            ((PersonSnapshot)bad.Records[index]).Royal.RequestedSpouse = ulong.MaxValue;
            bool rejected = false;
            try
            {
            ...
            ResultCode Decide(int option, ulong mateId, int turn) => GameRequestExecution.Execute(em, root, new ResolveMarriageRequest { Person = Id(child), Mate = mateId, ExpectedRequestTurn = turn, Decision = option });
            ...
            session.Phase = Phase.Deployment;
            ...
            var forced = settings;
            forced.BirthChance = 1;
            em.SetComponentData(root, forced);
            int Children(ulong id)
            {
                using var all = WorldQueries.OrderedEntities<Royal>(em);
                return all.Count(e =>
                {
                    var r = em.GetComponentData<Royal>(e);
                    return r.Parent == id || r.SecondParent == id;
                });
            }

            try
            {
                int before = Children(Id(child));
            ...
            var requester = DynastyOps.CreateRoyal(em, root, "拒婚测试", 2, 22, Id(child));
            var target = DynastyOps.CreateRoyal(em, root, "拒婚对象", 4, 22);
            Gender(requester, PersonGender.Female);
            Gender(target, PersonGender.Male);
            var rp = em.GetComponentData<Royal>(requester);
            rp.Influence = 100;
            rp.Ambition = 1;
            em.SetComponentData(requester, rp);
            float plot = CourtOps.PlotChance(em, root, child, requester), usurp = CourtOps.UsurpChance(em, root, requester);
            bool resented = false;
            for (int attempt = 0; attempt < 40 && !resented; attempt++)
            {
                rp = em.GetComponentData<Royal>(requester);
                rp.MarriageCooldownUntil = 0;
                em.SetComponentData(requester, rp);
                Check(RoyalFamilyOps.Request(em, root, requester, target), "Rejected request can recur after cooldown");
                rp = em.GetComponentData<Royal>(requester);
                Check(RoyalFamilyOps.Resolve(em, root, requester, 0, Id(target), rp.MarriageRequestTurn) == ResultCode.Success, "Refusal resolves once");
                resented = em.GetComponentData<Royal>(requester).Grievance > 0;
            }
            ...
            rp.MarriageCooldownUntil = 0;
            em.SetComponentData(requester, rp);
            RoyalFamilyOps.Request(em, root, requester, target);
            CourtOps.Die(em, root, target, 1);
            Check(!RoyalFamilyOps.RequestValid(em, root, requester) && em.GetComponentData<Royal>(requester).RequestedSpouse == 0, "Participant death cancels pending request without refusal grievance");
            ...
            var prepared = SnapshotCodec.Capture(em, root);
            RoyalFamilyOps.Prepare(em, root, requester);
            ...
            Check(GameRequestExecution.Execute(em, root, new ArrangeMarriageRequest { Person = Id(requester), Mate = Id(arranged) }) == ResultCode.Success, "Player can proactively arrange an adult child's marriage through ECS");
            Check(RoyalFamilyOps.Arrange(em, root, requester, arranged) == ResultCode.Unavailable, "Repeated arranged marriage is unavailable");
            var baby = DynastyOps.CreateRoyal(em, root, "父母映射", 2, 0, Id(requester));
            var bp = em.GetComponentData<Royal>(baby);
            bp.SecondParent = Id(arranged);
            em.SetComponentData(baby, bp);
            ...
            var final = SnapshotCodec.Capture(em, root);
            SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, final));
