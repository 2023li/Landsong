...
namespace Landsong.GridSystem
{
    ...
    public struct GridFootprint
    {
        ...
        public GridFootprint(GridPosition origin, Vector2Int size) : this(origin, size, BuildingOrientation.North)
        {
        }

        public GridFootprint(GridPosition origin, Vector2Int baseSize, BuildingOrientation orientation, int elevationLevel = 0, int surfaceLayer = 0)
        {
        ...
        {
            return new GridFootprint(newOrigin, baseSize, Orientation, elevationLevel, SurfaceLayer);
        }

        public bool Contains(GridPosition position)
        {
            return position.X >= MinX && position.X < MaxXExclusive && position.Z >= MinZ && position.Z < MaxZExclusive;
        }
