...
namespace Landsong.GridSystem
{
    public sealed class GridLayoutService
    {
        ...
        public static GridPlaneMode GetPlaneMode(UnityEngine.Grid grid)
        {
            ...
            switch (grid.cellLayout)
            {
                ...
                case GridLayout.CellLayout.IsometricZAsY:
                    return isWorldXZ ? GridPlaneMode.IsometricDiamondXZ : GridPlaneMode.IsometricDiamondXY;
        ...
        public static Vector3 GetPlaneNormal(UnityEngine.Grid grid)
        {
            ...
            if (Mathf.Abs(Vector3.Dot(normal, Vector3.up)) >= 0.999f && Vector3.Dot(normal, Vector3.up) < 0f)
            {
                normal = -normal;
            }
        ...
        public bool TryRaycastToGridPlane(Ray ray, float heightOffset, out Vector3 worldPosition, out float distance)
        {
            var plane = new UnityEngine.Plane(PlaneNormal, Origin + PlaneNormal * heightOffset);
