...
namespace Landsong.GridSystem
{
    ...
    /// </summary>
    [CreateAssetMenu(menuName = "Landsong/地图/TWC 烘焙配置", fileName = "地图_烘焙配置")]
    public sealed class TileWorldCreatorMapBakeProfile : ScriptableObject
    {
        [SerializeField, LabelText("使用蓝图层规则"), FormerlySerializedAs("useLayerPresetRules")]
        private bool useLayerBlueprintRules;
        [SerializeField, LabelText("蓝图地形规则")]
        private List<BlueprintTerrainRule> blueprintRules = new List<BlueprintTerrainRule>();
        public bool UseLayerBlueprintRules => useLayerBlueprintRules;
        public IReadOnlyList<BlueprintTerrainRule> BlueprintRules => blueprintRules;

        [SerializeField, LabelText("地形重叠规则")]
        private TerrainOverlapRules overlapRules;
        public TerrainOverlapRules OverlapRules => overlapRules;
        ...
        [SerializeField, HideIf(nameof(UseLayerBlueprintRules)), TitleGroup("基础逻辑层"), LabelText("旧地图默认地形来源层")]
        [PropertyTooltip("仅用于旧地图的地形输入；地图范围始终来自 TWC Settings 宽高。")]
        ...
        private List<TileWorldCreatorLayerBinding> layerBindings = new List<TileWorldCreatorLayerBinding>();
        ...
        [SerializeField, LabelText("边缘区层名称")]
        private string edgeLayerName = "边缘区";
        public string EdgeLayerName => edgeLayerName;

        [SerializeField, LabelText("边缘（格）")]
        ...
        public IReadOnlyList<TileWorldCreatorLayerBinding> LayerBindings => layerBindings != null ? layerBindings : EmptyBindings;

        private static readonly IReadOnlyList<TileWorldCreatorLayerBinding> EmptyBindings = Array.Empty<TileWorldCreatorLayerBinding>();
...
#if UNITY_EDITOR
        public void ApplyContent(MapContentAuthoring content)
        {
            if (content.UsesLegacyTerrainInput)
            {
                if (content.LegacyTerrainInput == null)
                    throw new InvalidOperationException("旧地图缺少地形输入配置。");
                JsonUtility.FromJsonOverwrite(JsonUtility.ToJson(content.LegacyTerrainInput), this);
                useLayerBlueprintRules = false;
                blueprintRules.Clear();
                overlapRules = null;
                baseLayerGuid = string.Empty;
                foreach (var binding in layerBindings)
                    binding?.SetResolvedLayer(string.Empty, binding.LayerName);
            }
            else
            {
                if (content.TerrainRules == null || !content.TerrainRules.useLayerBlueprintRules)
                    throw new InvalidOperationException("请选择跨 Layer 地形映射规则。");
                ApplyRules(content.TerrainRules);
            }

            baseLayerName = content.UsesLegacyTerrainInput ? content.LegacyBaseLayerName : string.Empty;
            baseLayerGuid = string.Empty;
            edgeWidth = content.EdgeWidth;
        ...
        public void ApplyRules(MapTerrainRules rules)
        {
            ...
            if (useLayerBlueprintRules)
                elevationWorldStep = 1f;
            baseLayerGuid = string.Empty;
            foreach (var binding in layerBindings)
                binding?.SetResolvedLayer(string.Empty, binding.LayerName);
            regenerateTileWorldBeforeBake = false;
