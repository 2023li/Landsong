...
namespace Landsong.BuildingSystem
{
    ...
    public sealed class BuildingPlacementController : MonoBehaviour
    {
        ...
        public void BeginPlacement(BuildingDefinition definition)
        {
            ...
            if (!definition.HasPlacementGhostPrefab)
            {
                Debug.LogWarning($"Building '{definition.DisplayName}' has no placement ghost prefab.", definition);
                return;
            }
        ...
        private IEnumerator ConfirmPlacementRoutine(BuildingDefinition definition, GridPosition origin)
        {
            ...
            var placedObject = Instantiate(definition.BuildingPrefab, placementPosition, Quaternion.identity, placedBuildingRoot);
            var gridOccupancyId = CreateGridOccupancyId(definition);
            if (!gridMap.TryOccupy(origin, definition.Size, gridOccupancyId, out failureReason))
            {
                Debug.LogWarning($"Cannot occupy grid for building '{definition.DisplayName}' at {origin}: {failureReason}.", this);
            ...
            var building = placedObject.GetComponent<BuildingBase>();
            if (building != null)
            {
                building.SetPlacement(origin, gridOccupancyId);
            }
            else
            {
        ...
        private void CreateGhost(BuildingDefinition definition)
        {
            ...
            ghostInstance = Instantiate(definition.PlacementGhostPrefab, parent);
