...
namespace Landsong.VisualSystem
{
    ...
    /// </summary>
    [CreateAssetMenu(menuName = "Landsong/Building/View Naming Config", fileName = "BuildingViewNamingConfig")]
    public sealed class LS_BuildingViewNamingConfig : ScriptableObject
    {
        public const string DefaultAssetPath = "Assets/Landsong/Art/BuildingViewNamingConfig.asset";
        public const string DefaultViewOutputRoot = "Assets/Landsong/ECSContent/Buildings";
        [Serializable]
        public sealed class Entry
        {
            ...
            public string ConstructionAuthoringName => string.IsNullOrWhiteSpace(ConstructionAuthoringNameOverride) ? NormalizedAuthoringPrefix + "_建造阶段" : ConstructionAuthoringNameOverride.Trim();
        ...
        public readonly struct ResolvedView
        {
            ...
            public string PrefabBaseName => Purpose == LS_BuildingViewPurpose.Construction ? $"{Entry.NormalizedPrefabPrefix}_建造阶段_Optimized" : $"{Entry.NormalizedPrefabPrefix}_LV{Level}_Optimized";
            public string BuildingFolderName => Entry.Definition != null && !string.IsNullOrWhiteSpace(Entry.Definition.Metadata.Id) ? Entry.Definition.Metadata.Id : Entry.NormalizedPrefabPrefix;
        }
        ...
        public string ViewOutputRoot => string.IsNullOrWhiteSpace(viewOutputRoot) ? DefaultViewOutputRoot : viewOutputRoot.Trim().Replace('\\', '/');
        public IReadOnlyList<Entry> Entries => entries ?? Array.Empty<Entry>();

        public bool TryResolveAuthoringName(string gameObjectName, out ResolvedView result, out string error)
        {
            result = default;
            error = string.Empty;
            gameObjectName = string.IsNullOrWhiteSpace(gameObjectName) ? string.Empty : gameObjectName.Trim();
            ...
            {
                var entry = Entries[i];
                if (entry == null || string.IsNullOrEmpty(entry.NormalizedAuthoringPrefix) || string.IsNullOrEmpty(entry.NormalizedPrefabPrefix) || !TryMatchEntry(entry, gameObjectName, out var purpose, out var level))
                {
                    continue;
                }
                ...
                {
                    error = $"名称“{gameObjectName}”同时命中多个固定映射：“{matchedEntry.NormalizedAuthoringPrefix}”与“{entry.NormalizedAuthoringPrefix}”。";
                    return false;
                }
            ...
            if (matchedEntry == null)
            {
                error = $"名称“{gameObjectName}”不符合任何固定映射。请在 {DefaultAssetPath} 中显式添加前缀，不会进行模糊猜测。";
                return false;
            }

            if (matchedPurpose == LS_BuildingViewPurpose.Operational && matchedEntry.Definition != null && matchedLevel > matchedEntry.Definition.MaximumLevel && Array.IndexOf(matchedEntry.ExpectedOperationalLevelsOverride ?? Array.Empty<int>(), matchedLevel) < 0)
            {
                error = $"固定映射“{matchedEntry.NormalizedAuthoringPrefix}”关联的 ECS 定义 不包含 LV{matchedLevel}。";
                return false;
            }
        ...
        {
            error = string.Empty;
            if (!ViewOutputRoot.StartsWith("Assets/", StringComparison.Ordinal) && !string.Equals(ViewOutputRoot, "Assets", StringComparison.Ordinal))
            {
            ...
            var families = new HashSet<BuildingDefinitionAsset>();
            for (var i = 0; i < Entries.Count; i++)
            {
                var entry = Entries[i];
                if (entry == null)
                ...
                if (string.IsNullOrEmpty(entry.NormalizedAuthoringPrefix) || string.IsNullOrEmpty(entry.NormalizedPrefabPrefix))
                {
                ...
                if (entry.ExpectsConstructionView && !constructionNames.Add(entry.ConstructionAuthoringName))
                {
        ...
        private static bool TryMatchEntry(Entry entry, string objectName, out LS_BuildingViewPurpose purpose, out int level)
        {
            ...
            if (entry.BareNameIsLevelOne && string.Equals(objectName, entry.NormalizedAuthoringPrefix, StringComparison.Ordinal))
            {
                return true;
            }
            ...
            return int.TryParse(levelText, out level) && level > 0 && string.Equals(objectName, levelPrefix + level, StringComparison.Ordinal);
        }
    }
}
