...
namespace Landsong.VisualSystem
{
    ...
    public sealed class BuildingCropPlantVisual : MonoBehaviour
    {
        ...
        public IReadOnlyList<GrowthStage> GrowthStages => growthStages ?? Array.Empty<GrowthStage>();
        public int MaximumPlantCount => Mathf.Clamp(maximumPlantCount, 1, MaximumSupportedPlantCount);
        public float SpawnDensityPercent => Mathf.Clamp(spawnDensityPercent, 1f, 100f);
        public int TargetPlantCount => Mathf.Clamp(Mathf.RoundToInt(MaximumPlantCount * SpawnDensityPercent * 0.01f), 1, MaximumPlantCount);

        private void OnValidate()
        {
            maximumPlantCount = Mathf.Clamp(maximumPlantCount, 1, MaximumSupportedPlantCount);
        ...
        public bool TryValidateConfiguration(out string error)
        {
            ...
            if (GetComponentInChildren<Collider>(true) != null || GetComponentInChildren<Collider2D>(true) != null)
            {
        ...
        private bool TryValidateStages(bool requireAuthoringDefaults, out string error)
        {
            ...
            for (var i = 0; i < growthStages.Length; i++)
            {
                ...
                if (stageTransform.localPosition.sqrMagnitude > 0.000001f || Quaternion.Angle(stageTransform.localRotation, Quaternion.identity) > 0.001f || (stageTransform.localScale - Vector3.one).sqrMagnitude > 0.000001f)
                {
                    error = $"作物 {CropId} 的阶段子对象 Transform 必须为 Position=0、Rotation=0、Scale=1：{stage.StageRoot.name}。";
                    return false;
                }

                if (requireAuthoringDefaults && stage.StageRoot.activeSelf != (i == 0))
                {
