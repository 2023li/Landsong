...
namespace Landsong.VisualSystem
{
    ...
    public sealed class BuildingCropVisualController : MonoBehaviour
    {
        ...
        public IReadOnlyList<BuildingCropPlantVisual> CropPrefabs => cropPrefabs ?? Array.Empty<BuildingCropPlantVisual>();
        public int PlantPointCount => CountChildPlantPoints();
        ...
        public bool TryResolveCropPrefab(string requestedCropId, out BuildingCropPlantVisual cropPrefab)
        {
            var normalizedCropId = string.IsNullOrWhiteSpace(requestedCropId) ? string.Empty : requestedCropId.Trim();
            if (cropPrefabs != null)
            {
                for (var i = 0; i < cropPrefabs.Length; i++)
                {
                    var candidate = cropPrefabs[i];
                    if (candidate != null && string.Equals(candidate.CropId, normalizedCropId, StringComparison.Ordinal))
                    {
        ...
        private List<PlantPlacement> BuildPlantPlacements(int targetPlantCount)
        {
            ...
            {
                var pointIndex = Mathf.Min(availablePlacements.Count - 1, Mathf.FloorToInt((plantIndex + 0.5f) * availablePlacements.Count / targetPlantCount));
        ...
        private int ResolveGeneratedPlantCount(BuildingCropPlantVisual cropPrefab)
        {
            ...
            return Mathf.Min(cropPrefab.TargetPlantCount, CountChildPlantPoints());
        }

        private List<PlantPlacement> GetAvailablePlantPlacements()
        {
            var result = new List<PlantPlacement>();
            foreach (var point in GetChildPlantPoints())
                result.Add(new PlantPlacement(point));
            return result;
        }
        ...
        private void PreviewFixedPoints()
        {
            ...
            if (!TryValidatePlantPoints(out var error) || previewCropPrefab == null || !previewCropPrefab.TryValidateConfiguration(out error))
            {
                var previewError = string.IsNullOrWhiteSpace(error) ? "没有可用的作物 Prefab。" : error;
                Debug.LogWarning($"无法预览农田作物：{previewError}", this);
                return;
            }

            var configuredPointCount = CountChildPlantPoints();
            ...
            editorPreviewRoot.transform.SetLocalPositionAndRotation(transform.localPosition, transform.localRotation);
            ...
            {
                var instanceObject = PrefabUtility.InstantiatePrefab(previewCropPrefab.gameObject, editorPreviewRoot.transform) as GameObject;
                if (instanceObject == null)
                {
                    instanceObject = Instantiate(previewCropPrefab.gameObject, editorPreviewRoot.transform, false);
                }
                ...
                instanceObject.transform.SetLocalPositionAndRotation(placement.LocalPosition, placement.LocalRotation);
                instanceObject.transform.localScale = Vector3.one * placement.UniformScale;
                if (instance == null || !instance.ApplyGrowthPercent(editorPreviewGrowthPercent, out error))
                {
                    ClearEditorPreview();
            ...
            Debug.Log($"已预览作物 {previewCropPrefab.CropId}：{editorPreviewGrowthPercent:0.##}% 阶段，" + $"使用 {placements.Count}/{configuredPointCount} 个固定点。", this);
        }
        ...
        private void ClearEditorPreview()
        {
            ...
            if (previewParent != null)
            {
                for (var i = previewParent.childCount - 1; i >= 0; i--)
                {
                    ...
                    if (child != transform && child.name.StartsWith(EditorPreviewRootPrefix, StringComparison.Ordinal))
                    {
        ...
        public BuildingCropPlantPoint AddEditorPlantPoint()
        {
            ...
            var createdPoint = CreateEditorPlantPointObject(position, rotation, scale, points.Count + 1, "Add Crop Plant Point");
            EditorUtility.SetDirty(this);
            PrefabUtility.RecordPrefabInstancePropertyModifications(this);
            return createdPoint;
        }

        private BuildingCropPlantPoint CreateEditorPlantPointObject(Vector3 localPosition, Quaternion localRotation, float uniformScale, int pointNumber, string undoName)
        {
