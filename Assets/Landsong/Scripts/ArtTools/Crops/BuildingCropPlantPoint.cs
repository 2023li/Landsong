...
namespace Landsong.VisualSystem
{
    ...
    public sealed class BuildingCropPlantPoint : MonoBehaviour
    {
        ...
        public bool TryValidateConfiguration(BuildingCropVisualController expectedController, out string error)
        {
            ...
            if (!IsFinite(position.x) || !IsFinite(position.y) || !IsFinite(position.z) || !IsFinite(rotation.x) || !IsFinite(rotation.y) || !IsFinite(rotation.z) || !IsFinite(rotation.w) || !IsFinite(scale.x) || !IsFinite(scale.y) || !IsFinite(scale.z))
            {
