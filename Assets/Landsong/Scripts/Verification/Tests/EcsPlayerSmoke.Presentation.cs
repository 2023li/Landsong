...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator PresentationUi(UI_GamePanel view, EntityManager em, Entity root)
        {
            ...
            string saved = null;
            InterfaceSettings.PersistenceOverride = json => saved = json;
            ...
            var hit = audio.Find(PresentationCue.Hit);
            var clickClip = click.Clip;
            var hitClip = hit.Clip;
            float cooldown = click.Cooldown;
            int concurrent = click.Concurrency;
            var ownedClip = AudioClip.Create("Owned 15 audio probe", 44100, 1, 44100, false);
            var ownedPixels = new Texture2D(2, 2);
            ownedPixels.SetPixels(new[] { Color.cyan, Color.cyan, Color.cyan, Color.cyan });
            ownedPixels.Apply();
            var ownedPortrait = Sprite.Create(ownedPixels, new Rect(0, 0, 2, 2), new Vector2(.5f, .5f));
            ...
            try
            {
                ...
                yield return null;
                Require(FindObjectsByType<AudioRuntime>(FindObjectsSortMode.None).Length == 1 && FindObjectsByType<AudioListener>(FindObjectsSortMode.None).Length == 1 && runtime.SourceCount == 18, "15 exactly one runtime/listener and bounded 18 audio sources");
                click.Clip = hit.Clip = ownedClip;
                click.Cooldown = 1;
                click.Concurrency = 2;
                Require(runtime.Play(PresentationCue.Click) && !runtime.Play(PresentationCue.Click), "15 cue cooldown rejects duplicate dispatch");
                runtime.StopEffects();
                click.Cooldown = 0;
                Require(runtime.Play(PresentationCue.Click) && runtime.Play(PresentationCue.Click) && !runtime.Play(PresentationCue.Click) && runtime.VoiceCount == 2, "15 per-cue concurrency cap enforced");
                runtime.StopEffects();
                Require(runtime.Play(PresentationCue.Hit), "15 effects bus playback");
                ...
                yield return null;
                yield return null;
                Require(runtime.VoiceCount == 1 && !runtime.Play(PresentationCue.Hit) && runtime.Play(PresentationCue.Click), "15 pause freezes world audio while UI stays usable");
                runtime.StopEffects();
                sControl.Paused = 0;
                {
                    em.SetComponentData(root, sControl);
                ...
                var p = InterfaceSettings.Current.Copy();
                p.Muted = true;
                p.Music = .23f;
                p.Effects = .34f;
                p.Ambient = .45f;
                InterfaceSettings.Apply(p, false, false);
                yield return null;
                ...
                audio.DayMusic = ownedClip;
                yield return new WaitForSecondsRealtime(1.6f);
                Require(runtime.Mode == "Day" && runtime.GetComponentsInChildren<AudioSource>().Count(a => a.loop && a.clip == ownedClip && a.isPlaying) == 1, "15 BGM state selects and loops exactly one source");
                p.Muted = false;
                InterfaceSettings.Apply(p, false, false);
                view.PauseMenu.Open();
                yield return null;
                ...
                settingsPanel.Language.onClick.Invoke();
                ...
                var translated = settingsPanel.ApplyButton.GetComponentInChildren<TMP_Text>();
                translated.ForceMeshUpdate(false, true);
                ...
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/presentation-language.png");
                ...
                var kid = DynastyOps.CreateRoyal(em, root, "表现验收继承人", 2, 8, em.GetComponentData<Identity>(king).Id);
                var kidId = em.GetComponentData<Identity>(kid).Id;
                PortraitOps.Ensure(em, root, kid);
                ...
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/presentation-family.png");
                    yield return new WaitForEndOfFrame();
                }
                ...
                yield return null;
                Require(bridge.EffectCount > 0, "15 spatial cue spawns bounded visual effect");
                {
                ...
                yield return new WaitForSecondsRealtime(1);
                Require(bridge.EffectCount == 0, "15 completed effect released by owner");
                Require(authority.SequenceEqual(SnapshotCodec.Capture(em, root)), "15 models/audio/FX do not alter authority or rewards");
                var probePosition = EntityState.Position(em, building);
                var native = SoldierEntities.Spawn(em, root, SoldierDefinitions.Find(em, root, "militia"), probePosition, false);
                SoldierCombatants.Configure(em, root, native, true, id.Id, probePosition);
                var nativeId = em.GetComponentData<Identity>(native).Id;
                yield return null;
                yield return null;
                ...
                bridge.ClearViews();
                Require(bridge.EffectCount == 0 && bridge.ModelCount == 0, "15 explicit owner cleanup releases all models/effects");
            }
            finally
            {
                ...
                hit.Clip = hitClip;
                click.Cooldown = cooldown;
                click.Concurrency = concurrent;
                runtime.StopEffects();
                // Detach the owned test music before destroying its clip; production assets were never saved.
                foreach (var source in runtime.GetComponentsInChildren<AudioSource>())
                    if (source.clip == ownedClip)
                    {
                        source.Stop();
                        source.clip = null;
                    }
                ...
                Destroy(ownedClip);
                Destroy(ownedPortrait);
                Destroy(ownedPixels);
                bridge.ClearViews();
                if (view.PauseMenu.IsOpen)
                    view.PauseMenu.Close();
        ...
        {
            Require(FindObjectsByType<AudioRuntime>(FindObjectsSortMode.None).Length == 1 && FindObjectsByType<AudioListener>(FindObjectsSortMode.None).Length == 1, "15 scene transition retains exactly one runtime/listener");
