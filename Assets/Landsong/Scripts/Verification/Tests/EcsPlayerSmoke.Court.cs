...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator CourtUi(UI_GamePanel view,EntityManager em,Entity root,string map)
        {
            ...
            view.OpenPanel(GamePanelId.Talent); yield return WaitFor(()=>HasRow(view.PrimaryRows,"打开交际页面"),"Native talent panel opens"); ClickRow(view,"打开交际页面");
            ...
            Require(System.Math.Abs(EffectOps.Modifier(em,root,RuleKind.SoldierAttackBonus,-1)-.1f)<.0001,"UI appointment pays exact soldier bonus");
            ...
            view.OpenPanel(GamePanelId.Royal); yield return WaitFor(()=>Object.FindFirstObjectByType<UI_GamePanel_CourtGraph>()?.Node(hid)!=null,"Underage royal visible in family graph"); Object.FindFirstObjectByType<UI_GamePanel_CourtGraph>().Node(hid).onClick.Invoke();
            yield return WaitFor(()=>view.Court.RoyalDetails!=null&&view.Court.RoyalDetails.PersonId==hid&&view.Court.RoyalDetails.Designate.interactable,"Underage designation enabled in right details"); var before=SnapshotCodec.Capture(em,root);
            view.Court.RoyalDetails.Designate.onClick.Invoke(); ClickIn(view.Buildings.BuildingConfirmRows,"取消"); Require(before.SequenceEqual(SnapshotCodec.Capture(em,root)),"Cancelling designation preserves exact day");
            view.Court.RoyalDetails.Designate.onClick.Invoke(); ClickIn(view.Buildings.BuildingConfirmRows,"确认"); yield return WaitFor(()=>CourtOps.State(em,root).Crown==hid,"Real UI crown command");
            yield return WaitFor(()=>view.Court.RoyalDetails.Execute.interactable,"Execution button available for deliberate choice"); before=SnapshotCodec.Capture(em,root); view.Court.RoyalDetails.Execute.onClick.Invoke();
            Require(view.Buildings.BuildingConfirmRows.GetComponentsInChildren<Text>().Any(t=>t.text.Contains("储君死亡")),"Execution confirmation explains succession consequences"); ClickIn(view.Buildings.BuildingConfirmRows,"取消"); Require(before.SequenceEqual(SnapshotCodec.Capture(em,root)),"Cancelled execution cannot kill or penalize crown");
            ...
            var s=em.GetComponentData<Session>(root); s.PublicOpinion=50; em.SetComponentData(root,s); view.OpenPanel(GamePanelId.Policy);
            ...
            view.OpenPanel(GamePanelId.Intelligence); yield return WaitFor(()=>view.PrimaryRows.GetComponentsInChildren<Text>().Any(t=>t.text.Contains("宫廷情报")),"Court intelligence appears in dedicated intelligence panel");
            ...
            view.OpenPanel(GamePanelId.Building); SnapshotCodec.Restore(em,root,SnapshotCodec.Decode(em,root,original)); Require(original.SequenceEqual(SnapshotCodec.Capture(em,root)),"Wave-nine UI fixtures restore original day without player save IO");
        }
    }
}
#endif
