    ...
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator MainMenuArchivesUi(UI_StartPanel menu, RunArchive archive)
        {
            ...
            try
            {
                ...
                File.WriteAllText(store.RunPath(archive.RunId) + RunArchiveStore.BackupSuffix, "damaged backup fixture");
                menu.RefreshContinue();
                ...
                if (Application.isEditor)
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/main-menu-with-save.png");
                yield return null;
                ...
                {
                    Navigation = FindFirstObjectByType<ApplicationUiRoot>(),
                    Mode = ArchiveOpenMode.Load,
                    Store = store,
                    Load = (run, slot, backup) =>
                    {
                        loadedRun = run;
                        loadedSlot = slot;
                    }
                };
                void Click(string text) => browser.GetComponentsInChildren<Button>().Single(b => b.gameObject.activeInHierarchy && b.GetComponentInChildren<TextMeshProUGUI>().text.Contains(text)).onClick.Invoke();
                ...
                {
                    yield return WaitTask(archiveManager.OpenAsync<UI_SavePanel>(request));
        ...
        IEnumerator MainMenuUi(UI_StartPanel menu)
        {
            ...
            try
            {
                ...
                var labels = menu.StartButton.transform.parent.GetComponentsInChildren<Button>().Select(b => b.GetComponentInChildren<TextMeshProUGUI>().text).ToArray();
                Require(labels.SequenceEqual(new[] { "开始新王朝", "加载游戏", "设置", "退出到桌面" }), "Home actions appear in requested order without saves");
                yield return null;
                if (Application.isEditor)
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/main-menu.png");
                yield return null;
                ...
                Require(!menu.NewDynastyPanel.GetComponentsInChildren<Button>().Any(b => b.GetComponentInChildren<TMP_Text>()?.text == "不刷新小偷（暂未开放）"), "Unimplemented special-rule controls are absent");
                if (Application.isEditor)
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/new-dynasty.png");
                yield return null;
                menu.CloseManagement();
                menu.StartButton.onClick.Invoke();
                ...
                {
                    menu.NewDynasty.Catalog = catalog;
                    menu.CloseManagement();
                }
