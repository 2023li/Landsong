...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        static void ClickContains(RectTransform rows, string text)
        {
            ...
            Require(button != null, "Usable row contains " + text);
        ...
        static IEnumerator InvitationExpeditionUi(UI_GamePanel view, EntityManager em, Entity root, string map)
        {
            ...
            Require(view.Panel != GamePanelId.Inventory, "Inventory panel rejects direct locked entry");
            view.OpenPanel(GamePanelId.Expedition);
            Require(view.Panel != GamePanelId.Expedition, "Expedition panel rejects direct locked entry");
            view.OpenPanel(GamePanelId.Building);
            Require(!view.IsPanelOpen || view.Panel != GamePanelId.Building, "Building catalog rejects direct locked entry");
            foreach (var permission in new[]
            {
                ...
                FeatureOps.Unlock(em, root, FeatureDefinitions.Find(em, root, new FixedString128Bytes(permission)));
            Entity Create(string name)
            {
                var definition = BuildingDefinitions.Find(em, root, new FixedString128Bytes(name));
                var g = em.GetComponentData<GridData>(root);
                for (var i = 0; i < g.Value.Value.Cells.Length; i++)
                {
                    var cell = g.Value.Value.Min + new int2(i % g.Value.Value.Size.x, i / g.Value.Value.Size.x);
                    if (!GridOps.CanPlace(em, root, definition, cell, 0))
                        continue;
                    var e = BuildingCreation.Create(em, root, definition, cell, 0, 1, true);
                    BuildingWorkforceState bWorkforce = em.GetComponentData<BuildingWorkforceState>(e);
                    bWorkforce.Workers = bWorkforce.StableWorkers = em.GetComponentData<BuildingWorkforceStats>(e).Capacity;
                    {
            ...
            var market = Create("b市场");
            var marketId = em.GetComponentData<Identity>(market).Id;
            InventoryOps.Add(em, root, em.GetComponentData<CurrencySettings>(root).Gold, 50);
            ...
            var mainlineName = em.GetComponentData<Identity>(WorldQueries.Find(em, QuestOps.Tracking(em, root).Target)).Name.ToString();
            ...
            ClickContains(view.Quests.QuestPoolRows, "立即邀约");
            ClickIn(view.Buildings.BuildingConfirmRows, "取消");
            Require(bytes.SequenceEqual(SnapshotCodec.Capture(em, root)), "Cancelled paid invitation is mutation free");
            ClickContains(view.Quests.QuestPoolRows, "立即邀约");
            ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            var expectedCommand = false;
            var commandDiagnostic = "";
            foreach (var queued in em.GetBuffer<QueuedGameplayRequest>(root))
            {
                commandDiagnostic += queued.Kind + " target=" + queued.Target + "; ";
                if (queued.Kind == CommandKind.RecruitQuest && em.HasComponent<RecruitQuestRequest>(queued.Payload))
                {
                    var request = em.GetComponentData<RecruitQuestRequest>(queued.Payload);
                    if (request.Provider == marketId && request.OfferSlot == 0)
            ...
            yield return WaitFor(() => em.GetBuffer<QueuedGameplayRequest>(root).Length == 0, "Paid invitation command processed");
            ...
            ClickContains(view.Quests.QuestPoolRows, "查看邀约");
            yield return WaitFor(() => HasRow(view.Quests.QuestDetailRows, "签约"), "Invitation detail opens without auto acceptance");
            ClickContains((RectTransform)view.Quests.QuestWindow.transform, "取消来源筛选");
            yield return new WaitForSecondsRealtime(.4f);
            for (var type = 0; type < 4; type++)
                view.Quests.QuestTypeToggle(type).isOn = type == 3;
            yield return new WaitForSecondsRealtime(.4f);
            ...
            for (var type = 0; type < 4; type++)
                view.Quests.QuestTypeToggle(type).isOn = true;
            if (Application.isEditor)
            {
                yield return new WaitForSecondsRealtime(.4f);
                ScreenCapture.CaptureScreenshot("Library/LandsongEcs/invitations-expeditions-invitations-" + map + ".png");
                yield return new WaitForEndOfFrame();
            }

            var post = Create("b陆上远征所");
            var postId = em.GetComponentData<Identity>(post).Id;
            view.OpenPanel(GamePanelId.Expedition);
            yield return WaitFor(() => view.PrimaryRows.GetComponentsInChildren<Button>().Any(b => b.interactable && b.GetComponentInChildren<Text>()?.text.Contains("陆上远征所 LV") == true), "Expedition lists available source buildings");
            ClickContains(view.PrimaryRows, "陆上远征所 LV");
            ClickContains(view.PrimaryRows, "近郊侦察 · 需驻地");
            yield return WaitFor(() => HasRow(view.PrimaryRows, "确认派遣"), "Crew and destination preview enables departure");
            ClickRow(view, "按可用人数填满");
            yield return WaitFor(() => view.PrimaryRows.GetComponentsInChildren<Text>().Any(t => t.text.Contains("出征人数 15")), "Expedition crew selector uses actual stable workforce cap");
            ...
            if (Application.isEditor)
            {
                ScreenCapture.CaptureScreenshot("Library/LandsongEcs/invitations-expeditions-expedition-" + map + ".png");
                yield return new WaitForEndOfFrame();
            }
            ...
            ClickRow(view, "确认派遣");
            ClickIn(view.Buildings.BuildingConfirmRows, "取消");
            Require(bytes.SequenceEqual(SnapshotCodec.Capture(em, root)), "Cancel expedition does not spend or lock workers");
            ClickRow(view, "确认派遣");
            ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            yield return WaitFor(() => WorkforceSettlement.Locked(em, postId), "Real UGUI dispatch locks source workforce");
            yield return WaitFor(() => HasRow(view.PrimaryRows, "放弃远征"), "Travelling party exposes abandon action");
            ClickRow(view, "放弃远征");
            ClickIn(view.Buildings.BuildingConfirmRows, "取消");
            Require(WorkforceSettlement.Locked(em, postId), "Abandon cancellation retains party");
            ClickRow(view, "放弃远征");
            ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            yield return WaitFor(() => !WorkforceSettlement.Locked(em, postId), "Confirmed abandon returns workers without casualties");
            yield return WaitFor(() => HasRow(view.PrimaryRows, "确认派遣"), "Source can depart again after abandonment");
            ClickRow(view, "确认派遣");
            ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            yield return WaitFor(() => WorkforceSettlement.Locked(em, postId), "Second UGUI departure accepted");
            ulong journeyId = 0;
            using (var all = WorldQueries.OrderedEntities<Expedition>(em))
                foreach (var e in all)
                {
                    ...
                    j.SuccessChance = 1;
                    j.Arrival = em.GetComponentData<GameClock>(root).Turn + 1;
                    em.SetComponentData(e, j);
                    journeyId = em.GetComponentData<Identity>(e).Id;
                }
            ...
            bytes = SnapshotCodec.Capture(em, root);
            ClickRow(view, "领取远征奖励");
            ClickIn(view.Buildings.BuildingConfirmRows, "取消");
            Require(bytes.SequenceEqual(SnapshotCodec.Capture(em, root)), "Reward cancellation retains unclaimed result and inventory");
            var wood = ItemDefinitions.Find(em, root, "原木");
            var woodBefore = InventoryOps.Count(em, root, wood);
            ClickRow(view, "领取远征奖励");
            ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            yield return WaitFor(() => em.GetBuffer<QueuedGameplayRequest>(root).Length == 0, "Reward claim command processed");
            Require(WorldQueries.Find(em, journeyId) == Entity.Null, "Confirmed reward claim consumes result; " + view.Hud.Message.text + "; paused=" + em.GetComponentData<SimulationControl>(root).Paused + "; menu=" + (view.PauseMenu != null && view.PauseMenu.IsOpen));
