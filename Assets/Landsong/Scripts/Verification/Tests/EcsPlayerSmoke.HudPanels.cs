    ...
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator HudPanelsUi(UI_GamePanel view, EntityManager em, Entity root)
        {
            ...
            try
            {
                ...
                Require(!view.Technology.CurrentResearchDefinition.IsValid && view.Technology.ResearchHudName.text == "尚未选择科技", "Empty research queue has a clear selectable HUD state");
                ...
                yield return WaitFor(() => em.GetComponentData<IntelligenceModeState>(root).Enabled != 0, "Intelligence mode entered through command");
                view.PanelCloseButton.onClick.Invoke();
                yield return WaitFor(() => em.GetComponentData<IntelligenceModeState>(root).Enabled == 0, "Closing intelligence exits authoritative read-only map mode");
                ...
                ResearchState sessionResearchState = em.GetComponentData<ResearchState>(root);
                sessionResearchState.Points = 2;
                {
                    em.SetComponentData(root, sessionResearchState);
                }
                ...
                {
                    sessionResearchState = em.GetComponentData<ResearchState>(root);
                }

                sessionResearchState.Points = 3;
                {
                    em.SetComponentData(root, sessionResearchState);
                }
                ...
                sessionResearchState.Points = 8;
                {
                    em.SetComponentData(root, sessionResearchState);
                }

                ResearchOps.Settle(em, root);
                yield return WaitFor(() => !view.Technology.CurrentResearchDefinition.IsValid, "Finishing the queue restores empty research HUD");
            }
            finally
            {
            ...
            yield return QuestUi(view, em, root, VerificationMap.Id);
            ...
            yield return TechnologyUi(view, em, root, VerificationMap.Id);
        }
    }
}
#endif
