...
namespace Landsong.ECS.Presentation
{
    ...
    public sealed partial class EcsPlayerSmoke : MonoBehaviour
    {
        ...
        {
            if (kind == LogType.Error && expectedErrors.Remove(message)) return;
        ...
        IEnumerator Run()
        {
            ...
            for (var map = 0; map < ids.Length; map++)
            {
                ...
                menu = FindFirstObjectByType<UI_StartPanel>(); menu.MapSelection.value = map;
                ...
                view.PauseMenu.OpenButton.onClick.Invoke(); view.PauseMenu.MenuButton.onClick.Invoke(); yield return ConfirmShared(true);
                ...
                var restoredMapSettings = em.GetComponentData<GameSettings>(root); settings = restoredMapSettings; settings.InvasionChance = 1; settings.FirstInvasion = 1; settings.FirstBoss = 99999; em.SetComponentData(root, settings);
                Sim.Set(em, root, new NightPlanState { BossDefinition = -1 }); NightOps.Plan(em, root, false); em.SetComponentData(root, restoredMapSettings); Sim.Emit(em, root, EventKind.DayCheckpoint, default); checkpoints.Update();
            ...
            expectedErrors.Add("场景切换失败：找不到地图：missing-map-test");
            ...
            yield return null;
            expectedErrors.Add("场景切换失败：不是 ECS 存档。");
        ...
        {
            var original = SnapshotCodec.Capture(em, root); var catalog = view.Buildings.BuildingCatalog;
            ...
            view.OpenPanel(GamePanelId.Technology); Require(view.Panel != GamePanelId.Technology, "Direct panel entry cannot bypass technology license");
            var quest = Array.FindIndex(catalog.Definitions, d => d.Data.Kind == ContentKind.Quest && d.Data.Configuration.Rewards.Features.Any(r => r.Feature.Data.Id == ResearchOps.FeatureId));
            Require(quest >= 0 && RewardOps.ApplyDefinition(em, root, quest), "Actual mainline reward grants technology access");
            ...
            var tree = view.Technology.TechnologyTree; Require(tree.EdgeCount > 55 && tree.GraphScroll.horizontal && tree.GraphScroll.vertical, "Full prerequisite connections and two-axis scrolling");
            ...
            ClickIn(view.Buildings.BuildingConfirmRows, "取消"); Require(before.SequenceEqual(SnapshotCodec.Capture(em, root)), "Cancel path preview changes no simulation state");
            ClickIn(tree.DetailRows, "规划至此科技（预览前置路径）"); ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            ...
            ClickIn(tree.DetailRows, "前往建造目录 · 木材加工厂"); Require(view.Panel == GamePanelId.Building && !tree.gameObject.activeSelf, "Blueprint link returns to building menu");
        ...
        {
            var original = SnapshotCodec.Capture(em, root); view.OpenPanel(GamePanelId.Building);
            ...
            var mainId=QuestOps.Tracking(em,root).Target;view.Quests.SelectQuest(mainId);
            ...
            Require(em.GetComponentData<Quest>(Sim.Find(em,mainId)).Status==QuestStatus.Completed || !view.Quests.FindQuestCard(mainId).Action.gameObject.activeSelf,"Active mainline has no abandon action");
            ...
            view.Quests.OpenInvitations();yield return WaitFor(()=>view.Quests.FindQuestCard(offeredIds[2])!=null,"All invitation values appear under their sources");
            ...
            for(var type=0;type<4;type++)view.Quests.QuestTypeToggle(type).isOn=false;
            yield return new WaitForSecondsRealtime(.4f);Require(view.Quests.QuestPoolRows.Cast<Transform>().All(t=>!t.gameObject.activeSelf),"No checked category displays no invitations or empty slots");
            view.Quests.QuestTypeToggle(0).isOn=true;view.Quests.QuestTypeToggle(3).isOn=true;
            yield return new WaitForSecondsRealtime(.4f);Require(view.Quests.QuestTypeToggle(0).isOn && view.Quests.QuestTypeToggle(3).isOn && !view.Quests.QuestTypeToggle(1).isOn && view.Quests.FindQuestCard(offeredIds[2])!=null,"Category toggles remain independently selected across refresh");
            view.Quests.QuestTypeToggle(0).isOn=false;yield return new WaitForSecondsRealtime(.4f);Require(view.Quests.FindQuestCard(offeredIds[2])==null,"Unchecking trade hides trade invitations");
            ...
            ClickIn(view.Quests.QuestDetailRows, "签约"); Require(view.Buildings.BuildingConfirmPanel.activeSelf, "Accept displays confirmation and deadline warning"); ClickIn(view.Buildings.BuildingConfirmRows, "取消");
            Require(em.GetComponentData<Quest>(task).Status == QuestStatus.Offered, "Cancelled acceptance leaves offer unchanged");
            ClickIn(view.Quests.QuestDetailRows, "签约"); ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            ...
            var card=view.Quests.FindQuestCard(id); Require(card.Source.text.Contains("提供"),"Accepted slot displays capacity provider");
            ...
            view.Quests.FindQuestCard(id).Expand.onClick.Invoke(); yield return new WaitForSecondsRealtime(.4f);
            Require(view.Quests.FindQuestCard(id).Body.gameObject.activeSelf,"Task click expands inline details");
            view.Quests.FindQuestCard(id).Action.onClick.Invoke(); Require(view.Buildings.BuildingConfirmPanel.activeSelf,"X asks before abandoning task"); ClickIn(view.Buildings.BuildingConfirmRows,"取消");
            ...
            yield return new WaitForSecondsRealtime(.35f);Require(!view.Quests.FindQuestCard(id).Tracking.isOn,"Refresh does not re-enable unchecked task");
            view.Quests.FindQuestCard(id).Tracking.isOn=true;view.Quests.FindQuestCard(id).Tracking.isOn=false;
            yield return WaitFor(()=>em.GetBuffer<Command>(root).Length==0,"Rapid check and uncheck commands processed");
            yield return new WaitForSecondsRealtime(.35f);Require(QuestOps.Tracking(em,root).Mode==2 && !view.Quests.FindQuestCard(id).Tracking.isOn,"Rapid check then uncheck retains final player intent");
            view.Quests.FindQuestCard(id).Tracking.isOn=true;
            ...
            var first = progress[0]; view.Quests.ConfirmQuestSubmission(id, first.Key.ToString());
            ...
            view.Quests.QuestAmountInput.text = "1"; ClickIn(view.Buildings.BuildingConfirmRows, "取消");
            Require(em.GetBuffer<QuestProgress>(task)[0].Amount == 0, "Cancelled submission does not consume or progress");
            if (Application.isEditor) { view.Quests.ConfirmQuestSubmission(id, first.Key.ToString()); ScreenCapture.CaptureScreenshot("Library/LandsongEcs/quests-submit-" + map + ".png"); yield return new WaitForEndOfFrame(); ClickIn(view.Buildings.BuildingConfirmRows, "取消"); }
            view.Quests.ConfirmQuestSubmission(id, first.Key.ToString()); view.Quests.QuestAmountInput.text = "1"; ClickIn(view.Buildings.BuildingConfirmRows, "确认");
            ...
            foreach (var p in progress) { view.Quests.ConfirmQuestSubmission(id, p.Key.ToString()); ClickIn(view.Buildings.BuildingConfirmRows, "确认"); yield return WaitFor(() => em.GetBuffer<Command>(root).Length == 0, "remaining UI submission"); }
            ...
            view.Quests.FindQuestCard(id).Action.onClick.Invoke(); ClickIn(view.Buildings.BuildingConfirmRows, "取消"); Require(em.Exists(task), "Cancelled reward leaves completed task");
            view.ClosePanel();yield return WaitFor(()=>HasRow(view.Quests.QuestHudRows,"领取奖励"),"Completed tracked task offers direct HUD reward claim");
