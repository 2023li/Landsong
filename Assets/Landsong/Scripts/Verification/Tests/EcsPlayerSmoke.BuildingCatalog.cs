...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        ...
        {
            var snapshot = SnapshotCodec.Capture(em, root); var bar = view.Buildings.BuildingBar;
            try
            {
                ...
                var warehouse = Sim.FindDefinition(em, root, "b仓库"); BlueprintOps.Grant(em, root, warehouse);
                FeatureOps.Unlock(em, root, Sim.FindDefinition(em, root, "feature.Building"));
                ...
                for (var i = 0; i < blob.Value.Definitions.Length; i++) if (blob.Value.Definitions[i].Kind == ContentKind.Building && BlueprintOps.Has(em, root, i)) { expected++; union |= blob.Value.Definitions[i].BuildingPolicy.Category; }
                ...
                bar.CardButton(warehouse).onClick.Invoke(); Require(view.WorldInteraction.HasBuildingPlacement, "Building icon enters original map placement workflow");
            ...
            {
                SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, snapshot)); view.Buildings.CancelBuildingInteraction();
                bar.gameObject.SetActive(false); view.OpenPanel(GamePanelId.Economy);
            }
        }
    }
}
