...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        ...
        GameUiSessionHandle trackedGameSession;
        ...
        static bool MenuReady => !EcsSceneFlow.Busy && !EcsSceneFlow.GameReady && UIManager.TryGetInstance(out var manager) && !manager.IsBusy && manager.TryGetActivePanel<UI_StartPanel>(out var menu) && menu.StartButton.IsInteractable();
        ...
        IEnumerator CaptureSharedUiContract()
        {
            ...
            yield return WaitTask(settings);
            originalSettingsPanel = settings.Result;
            ...
            yield return WaitTask(archives);
            originalArchivePanel = archives.Result;
        ...
        {
            Require(originalUiManager != null && UIManager.TryGetInstance(out var manager) && manager == originalUiManager && manager.RootCanvas == originalCanvas && manager.EventSystem == originalEventSystem, "Same Canvas, UIManager and EventSystem survive: " + context);
            Require(FindObjectsByType<UIManager>(FindObjectsInactive.Include, FindObjectsSortMode.None).Length == 1 && FindObjectsByType<EventSystem>(FindObjectsInactive.Include, FindObjectsSortMode.None).Length == 1, "Exactly one application manager and input system: " + context);
            ...
            foreach (var panel in originalCanvas.GetComponentsInChildren<UIPanelBase>())
            {
                ...
                Require(Enumerable.Range(0, 4).All(index => Vector3.Distance(expected[index], actual[index]) < .1f), "Loaded panel bounds match the actual application Canvas: " + panel.name + " / " + context);
            }
        }
        ...
        void ObserveSharedSettings(string context)
        {
            ...
            {
                Require(settingsOpenedInStart, "Game settings reuse follows an actual Start settings open");
                settingsOpenedInGame = true;
            }
            else
                settingsOpenedInStart = true;
        ...
        void ObserveSharedArchives(string context)
        {
            ...
            if (EcsSceneFlow.GameReady)
            {
                Require(archivesOpenedInStart, "Game archive reuse follows an actual Start archive open");
                archivesOpenedInGame = true;
            }
            else
                archivesOpenedInStart = true;
            RequirePreviewCleared(panel, context);
            RequireApplicationUiStable(context);
        }
        ...
        static void RequirePreviewCleared(UIViewBase panel, string context, bool beforeOpen = false)
        {
            ...
            foreach (var preview in previews)
            {
                ...
                if (!beforeOpen)
                    continue; // Open/refresh is allowed to replace the reset text with real view data.
        ...
        void ObserveGameLifetime(UI_GamePanel view, string context)
        {
            ...
            trackedGameView = view;
            trackedGameSession = view.Session;
            trackedGameScope = view.Scope;
        ...
        void RequireGameLifetimeReleased(string context)
        {
            ...
            Require(trackedGameSession != null && !trackedGameSession.IsBound && trackedGameScope.IsEnded, "Old Game UI binding and scope are invalidated: " + context);
        ...
        static IEnumerator WaitTask(Task operation)
        {
            ...
            while (!operation.IsCompleted)
            {
                if (Time.realtimeSinceStartup >= deadline)
                    throw new TimeoutException("Shared UI navigation did not finish within 150 seconds.");
                yield return null;
            }
        ...
        static IEnumerator CloseShared<T>()
            where T : UIPanelBase
        {
        ...
        {
            browser.Rows.GetComponentsInChildren<Button>().Single(button => button.interactable && button.GetComponentInChildren<TMP_Text>()?.text.Contains(label) == true).onClick.Invoke();
        }
