            ...
            try
            {
                ...
                using (var sites = WorldQueries.OrderedEntities<Building>(em))
                    foreach (var site in sites)
                        if (em.GetComponentData<BuildingHousingStats>(site).IsCore != 0)
                        {
                            palace = site;
                            break;
                        }

                Require(palace != Entity.Null, "New game has a palace");
                ulong palaceId = em.GetComponentData<Identity>(palace).Id;
                Require(GarrisonOps.GarrisonCount(em, palaceId) == 2 && GarrisonOps.AtSlot(em, palaceId, 1) != Entity.Null && GarrisonOps.AtSlot(em, palaceId, 2) != Entity.Null && GarrisonOps.AtSlot(em, palaceId, 3) == Entity.Null, "New game grants exactly two palace soldiers in slots one and two");
                var initialSoldier = GarrisonOps.AtSlot(em, palaceId, 1);
                Require(em.HasComponent<SoldierPerson>(initialSoldier) && PortraitOps.Age(em, initialSoldier) >= 18 && em.GetComponentData<BuildingRecruitmentState>(palace).Count == 0, "Initial soldiers have persistent person data and consume no recruitment quota");
                var initialId = em.GetComponentData<Identity>(initialSoldier).Id;
                SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, original));
                Require(WorldQueries.Find(em, initialId) != Entity.Null && GarrisonOps.GarrisonCount(em, palaceId) == 2, "Restoring starting snapshot preserves identity without duplicating garrison");
                ...
                Require(home != Entity.Null, "Garrison fixture has an operational home");
                ulong homeId = em.GetComponentData<Identity>(home).Id;
                BuildingRecruitmentState bRecruitment = em.GetComponentData<BuildingRecruitmentState>(home);
                bRecruitment.Count = 0;
                {
                    em.SetComponentData(home, bRecruitment);
                }

                var definition = SoldierId.FromIndex(0);
                var quote = SoldierOps.RecruitQuote(em, root, homeId, definition, 1, true);
                ...
                garrison.Slots.GetComponentInChildren<Button>().onClick.Invoke();
                yield return WaitFor(() => view.IsPanelOpen && view.Panel == GamePanelId.Garrison, "Empty slot opens garrison management");
                ...
                yield return WaitFor(() => Recruit() != null, "Garrison panel provides affordable recruitment");
                ...
                var confirm = view.Buildings.BuildingConfirmPanel.GetComponentsInChildren<Button>().First(x => x.interactable && x.GetComponentInChildren<TMP_Text>() != null && x.GetComponentInChildren<TMP_Text>().text == "确认");
                ...
                yield return WaitFor(() => Pending() != Entity.Null, "Recruitment places soldier in pending pool");
                ulong id = em.GetComponentData<Identity>(Pending()).Id;
                ...
                yield return WaitFor(() => Pool() != null, "Pending soldier is selectable");
                Pool().onClick.Invoke();
                Require(!view.soldierController.SoldierDetailsOpen, "Selecting a pending card never opens details");
                PoolCard().Attention.isOn = true;
                ...
                yield return WaitFor(() => Slot() != null, "Selecting pending soldier enables empty slot");
                Slot().onClick.Invoke();
                yield return WaitFor(() => em.GetComponentData<Soldier>(WorldQueries.Find(em, id)).Garrison != 0, "Clicking destination assigns selected soldier");
                ulong assigned = em.GetComponentData<Soldier>(WorldQueries.Find(em, id)).Garrison;
                ...
                AssignedCard().Select.onClick.Invoke();
                Require(!view.soldierController.SoldierDetailsOpen, "Occupied card body does not open details");
                AssignedCard().Remove.onClick.Invoke();
                yield return WaitFor(() => PoolCard() != null && em.GetComponentData<Soldier>(WorldQueries.Find(em, id)).Garrison == 0, "Remove returns soldier to pending exactly once");
                Pool().onClick.Invoke();
                yield return WaitFor(() => Slot() != null, "Removed soldier can be selected again");
                Slot().onClick.Invoke();
                yield return WaitFor(() => em.GetComponentData<Soldier>(WorldQueries.Find(em, id)).Garrison == assigned, "Removed soldier reassigns to selected slot");
                var extraQuote = SoldierOps.RecruitQuote(em, root, homeId, definition, 2, true);
                foreach (var cost in extraQuote.Costs)
                    InventoryOps.Add(em, root, cost.Item, cost.Amount);
                Require(GameRequestExecution.Execute(em, root, new RecruitSoldiersRequest { Garrison = homeId, Soldier = definition, Quantity = 2, Pending = true }) == ResultCode.Success, "Sorting fixture recruits two distinct pending soldiers");
                ulong[] sortingIds;
                using (var all = WorldQueries.OrderedEntities<Soldier>(em))
                    sortingIds = all.ToArray().Where(e => em.GetComponentData<Soldier>(e).Garrison == 0).Select(e => em.GetComponentData<Identity>(e).Id).ToArray();
                for (int i = 0; i < sortingIds.Length; i++)
                {
                    var soldierEntity = WorldQueries.Find(em, sortingIds[i]);
                    var soldier = em.GetComponentData<Soldier>(soldierEntity);
                    soldier.Experience = UnitProgression.LevelThreshold(SoldierDefinitions.Get(em, root, definition).Growth, 2) + i;
                    em.SetComponentData(soldierEntity, soldier);
                }
                ...
                var wounded = WorldQueries.Find(em, sortingIds[0]);
                float total = UI_GamePanel_Garrison.SoldierAttributeTotal(em, root, wounded);
                var woundedHealth = em.GetComponentData<Health>(wounded);
                woundedHealth.Current = 1;
                em.SetComponentData(wounded, woundedHealth);
                ...
                var stronger = WorldQueries.Find(em, sortingIds[1]);
                var strongerHealth = em.GetComponentData<Health>(stronger);
                strongerHealth.Maximum += 40;
                em.SetComponentData(stronger, strongerHealth);
                ...
                var title = AssignedCard().Title;
                title.ForceMeshUpdate();
                Require(title.textInfo.characterInfo.Take(title.textInfo.characterCount).Any(c => c.isVisible), "Soldier name actually renders within its reserved line");
                ...
                yield return new WaitForEndOfFrame();
                ScreenCapture.CaptureScreenshot("Library/LandsongEcs/building-garrison-portraits.png");
                yield return null;
                ...
                yield return new WaitForEndOfFrame();
                ScreenCapture.CaptureScreenshot("Library/LandsongEcs/soldier-details.png");
                yield return null;
                view.soldierController.CloseSoldierDetails();
                Require(!view.soldierController.SoldierDetailsOpen, "Soldier detail can close without losing underlying building");
                ...
                yield return WaitFor(() => view.PrimaryRows.GetComponentsInChildren<UI_GamePanel_SoldierItem>().Any(c => c.PersonId == id), "Return to grouped soldier cards");
                var dismissalUnit = WorldQueries.Find(em, id);
                var unitState = em.GetComponentData<Soldier>(dismissalUnit);
                int employed = PopulationOps.Employed(em);
                view.PrimaryRows.GetComponentsInChildren<UI_GamePanel_SoldierItem>().Single(c => c.PersonId == id).Dismiss.onClick.Invoke();
                yield return WaitFor(() => WorldQueries.Find(em, id) == Entity.Null, "Immediate dismissal removes only selected soldier");
                Require(GarrisonOps.AtSlot(em, assigned, 1) == Entity.Null && PopulationOps.Employed(em) == employed - unitState.PopulationCost, "Immediate dismissal frees actual slot and population");
                var sort = view.SecondaryRows.GetComponentsInChildren<Button>().First(x => x.GetComponentInChildren<TMP_Text>()?.text.StartsWith("排序：") == true);
                sort.onClick.Invoke();
                ...
                yield return new WaitForEndOfFrame();
                ScreenCapture.CaptureScreenshot("Library/LandsongEcs/building-garrison.png");
                yield return null;
            }
            finally
            {
                view.soldierController.CloseSoldierDetails();
                if (view.Buildings.BuildingConfirmPanel != null)
                    view.Buildings.BuildingConfirmPanel.SetActive(false);
                view.BuildingDetails.gameObject.SetActive(false);
                view.ClosePanel();
                SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, original));
            }
        }
