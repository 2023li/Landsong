...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator InterfaceUi(UI_GamePanel view, EntityManager em, Entity root)
        {
            ...
            var priorStore = checkpoint.Store;
            var preferences = InterfaceSettings.Current.Copy();
            var cameraPosition = view.WorldInteraction.Camera.transform.position;
            var cameraRotation = view.WorldInteraction.Camera.transform.rotation;
            float zoom = view.WorldInteraction.Camera.orthographicSize;
            var priorPersistence = InterfaceSettings.PersistenceOverride;
            string savedPreferences = null;
            InterfaceSettings.PersistenceOverride = json => savedPreferences = json;
            ...
            string directory = Path.Combine(Path.GetTempPath(), "Landsong-14-UI-" + Guid.NewGuid().ToString("N"));
            ...
            try
            {
                // This fixture exercises navigation after permission acquisition; the snapshot restores the original grants.
                ...
                Require(scroll.content.GetComponentsInChildren<Button>().First().GetInstanceID() == identity && Mathf.Abs(scroll.verticalNormalizedPosition - normalized) < .05f, "14 family node identity and scroll survive timed refresh");
                view.OpenPanel(GamePanelId.History);
                SimulationEvents.Emit(em, root, EventKind.Message, "可定位验收消息");
                yield return new WaitForSecondsRealtime(.35f);
                ...
                var filter = view.GetComponentInParent<Canvas>().GetComponentsInChildren<TMP_InputField>().First(i => i.gameObject.name == "筛选来源 / 内容");
                filter.SetTextWithoutNotify("历史焦点");
                filter.ActivateInputField();
                yield return null;
                var before = view.WorldInteraction.Camera.transform.position;
                yield return KeyPress(Key.W);
                Require(view.WorldInteraction.Camera.transform.position == before, "14 focused TMP input owns keyboard and camera");
                filter.DeactivateInputField();
                UnityEngine.EventSystems.EventSystem.current.SetSelectedGameObject(null);
                view.BackPanel();
                yield return new WaitForSecondsRealtime(.3f);
                Require(view.Panel == GamePanelId.Royal, "14 panel return stack restores previous panel");
                view.OpenPanel(GamePanelId.Building);
                yield return null;
                before = view.WorldInteraction.Camera.transform.position;
                yield return KeyPress(Key.W);
                Require(view.WorldInteraction.Camera.transform.position != before, "14 default camera movement remains usable");
                var rotation = view.WorldInteraction.Camera.transform.rotation;
                yield return KeyPress(Key.E);
                Require(view.WorldInteraction.Camera.transform.rotation != rotation, "14 keyboard camera rotation works");
                ...
                var center = mapGrid.Origin + new Unity.Mathematics.float3((mapGrid.Value.Value.Min.x + mapGrid.Value.Value.Size.x * .5f) * mapGrid.CellSize, 0, (mapGrid.Value.Value.Min.y + mapGrid.Value.Value.Size.y * .5f) * mapGrid.CellSize);
                ...
                )
                {
                    ...
                    {
                        var candidate = new Vector2(Screen.width * x, Screen.height * y);
                        hits.Clear();
                ...
                Require(worldPointFound, "14 drag fixture starts outside UI ownership");
                InputSystem.QueueStateEvent(mouse, new MouseState { position = position });
                yield return null;
                var questRect = (RectTransform)view.Quests.QuestTracking.transform;
                var blocked = RectTransformUtility.WorldToScreenPoint(null, questRect.TransformPoint(questRect.rect.center));
                hits.Clear();
                ...
                before = view.WorldInteraction.Camera.transform.position;
                InputSystem.QueueStateEvent(mouse, new MouseState { position = blocked, buttons = 4 });
                yield return null;
                InputSystem.QueueStateEvent(mouse, new MouseState { position = position, buttons = 4 });
                yield return null;
                InputSystem.QueueStateEvent(mouse, new MouseState { position = position });
                yield return null;
                ...
                InputSystem.QueueStateEvent(mouse, new MouseState { position = position, buttons = 4 });
                yield return null;
                InputSystem.QueueStateEvent(mouse, new MouseState { position = position + new Vector2(90, 0), buttons = 4 });
                yield return null;
                InputSystem.QueueStateEvent(mouse, new MouseState { position = position + new Vector2(90, 0) });
                yield return null;
                Require(view.WorldInteraction.Camera.transform.position != before, "14 middle-button drag pans without world clicks");
                float size = view.WorldInteraction.Camera.orthographicSize;
                ...
                InputSystem.QueueStateEvent(touch, new TouchState { touchId = 2, phase = UnityEngine.InputSystem.TouchPhase.Began, position = position + new Vector2(90, 0) });
                yield return null;
                InputSystem.QueueStateEvent(touch, new TouchState { touchId = 2, phase = UnityEngine.InputSystem.TouchPhase.Moved, position = position + new Vector2(180, 30) });
                yield return null;
                ...
                InputSystem.QueueStateEvent(touch, new TouchState { touchId = 2, phase = UnityEngine.InputSystem.TouchPhase.Ended, position = position + new Vector2(180, 30) });
                yield return null;
                ...
                view.PauseMenu.SettingsButton.onClick.Invoke();
                ...
                settingsPanel.Master.value = .42f;
                settingsPanel.ApplyButton.onClick.Invoke();
                ...
                settingsPanel.KeyButtons[0].onClick.Invoke();
                yield return KeyPress(Key.UpArrow);
                settingsPanel.ApplyButton.onClick.Invoke();
                ...
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/interface-settings.png");
                    yield return new WaitForEndOfFrame();
                }

                yield return CloseShared<UI_SettingPanel>();
                view.PauseMenu.SaveButton.onClick.Invoke();
                yield return WaitFor(() => Shared<UI_SavePanel>() != null, "14 shared save panel opens");
                var browser = Shared<UI_SavePanel>();
                browser.RenameInput.SetTextWithoutNotify("界面验收原始槽");
                ClickArchive(browser, "创建独立存档");
                var run = em.GetComponentData<RunPersistence>(root).RunId.ToString();
                yield return WaitFor(() => store.Slots(run).Length == 1, "14 isolated save created");
                string slot = store.Slots(run)[0];
                yield return WaitFor(() => File.Exists(store.PreviewPath(run, slot)), "14 map thumbnail attached to saved slot");
                browser.Refresh();
                ClickArchive(browser, store.Describe(run, slot).Name);
                browser.RenameInput.SetTextWithoutNotify("验收独立槽");
                ClickArchive(browser, "重命名");
                yield return WaitFor(() => Shared<UI_ConfirmPanel>() != null, "14 rename requests shared confirmation");
                yield return ConfirmShared(true);
                Require(store.Describe(run, slot).Name == "验收独立槽", "14 confirmed rename uses slot metadata only");
                ClickArchive(browser, "验收独立槽");
                ClickArchive(browser, "覆盖此槽");
                yield return ConfirmShared(false);
                Require(!File.Exists(store.SlotPath(run, slot) + RunArchiveStore.BackupSuffix), "14 cancelling overwrite preserves original and creates no backup");
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/interface-slots.png");
                    yield return new WaitForEndOfFrame();
                }

                ClickArchive(browser, "永久删除此槽及其备份");
                yield return ConfirmShared(false);
                ...
                view.PauseMenu.Close();
                yield return WaitFor(() => !view.PauseMenu.IsOpen, "14 modal closes and releases input");
            }
            finally
            {
                ...
                InterfaceSettings.Apply(preferences, false, false);
                view.WorldInteraction.Camera.transform.SetPositionAndRotation(cameraPosition, cameraRotation);
                view.WorldInteraction.Camera.orthographicSize = zoom;
                InterfaceSettings.PersistenceOverride = priorPersistence;
                checkpoint.Store = priorStore;
