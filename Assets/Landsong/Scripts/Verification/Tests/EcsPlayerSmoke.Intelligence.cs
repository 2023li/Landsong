...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator IntelligenceUi(UI_GamePanel view, EntityManager em, Entity root, string map)
        {
            ...
            var position = view.WorldInteraction.Camera.transform.position; float zoom = view.WorldInteraction.Camera.orthographicSize;
            ...
            var ray = new Ray(view.WorldInteraction.Camera.transform.position, view.WorldInteraction.Camera.transform.forward); var plane = new Plane(Vector3.up, (Vector3)area.Center);
            if (plane.Raycast(ray, out float distance)) view.WorldInteraction.Camera.transform.position += (Vector3)area.Center - ray.GetPoint(distance);
            ...
            yield return WaitFor(() => !view.Hud.InIntelligenceMode && em.GetComponentData<Session>(root).IntelligenceMode == 0, "12 explicit exit releases both input gates");
            em.SetComponentData(root, settings); checkpoints.Import(root, archive, false); SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, original));
            view.WorldInteraction.Camera.transform.position = position; view.WorldInteraction.Camera.orthographicSize = zoom; view.OpenPanel(GamePanelId.Building); yield return null;
        }
        ...
        IEnumerator FontReloadProbe()
        {
            ...
            for (int cycle = 0; cycle < 3; cycle++)
            {
                ...
                yield return null; var view = FindFirstObjectByType<UI_GamePanel>();
                ObserveGameLifetime(view, "repeated load " + cycle);
                view.OpenPanel(GamePanelId.Intelligence); yield return new WaitForSecondsRealtime(.3f); view.OpenPanel(GamePanelId.Building); yield return new WaitForSecondsRealtime(.3f);
                EcsSceneFlow.ReturnToMenu(); yield return WaitFor(() => MenuReady, "TMP probe returns to menu");
