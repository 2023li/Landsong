    ...
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator PortraitsUi(UI_GamePanel view, EntityManager em, Entity root)
        {
            ...
            try
            {
                ...
                ulong id = em.GetComponentData<Identity>(king).Id;
                var royal = em.GetComponentData<Royal>(king);
                royal.Age = 16;
                em.SetComponentData(king, royal);
                var beauty = RoyalTraitDefinitions.Find(em, root, "gene.beauty");
                var traits = em.GetBuffer<TraitEntry>(king);
                for (int i = traits.Length - 1; i >= 0; i--)
                    if (traits[i].Definition == beauty)
                        traits.RemoveAt(i);
                traits.Add(new TraitEntry { Definition = beauty, Revealed = 1, Active = 1 });
                PortraitOps.Ensure(em, root, king);
                var dna = em.GetComponentData<PortraitDNA>(king);
                dna.Customized = 0;
                dna.InvitationAnnounced = 0;
                em.SetComponentData(king, dna);
                ...
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/portraits-family.png");
                    yield return null;
                }

                view.ClosePanel();
                view.OpenPanel(GamePanelId.Royal);
                yield return WaitFor(() => view.Court.RoyalDetails.Portrait.sprite != null, "Reopening a panel rebinds its cached portrait");
                var before = SnapshotCodec.Capture(em, root);
                view.portraitController.OpenPortrait(id);
                ...
                view.portraitController.PortraitCloseButton.onClick.Invoke();
                Require(!view.portraitController.PortraitOpen && PortraitOps.CanCustomize(em, root, king), "Later preserves the one-use opportunity");
                view.portraitController.OpenPortrait(id);
                customization.ColorSliders[0].value = 177;
                ...
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/beauty-customization.png");
                    yield return null;
                }
                ...
                view.portraitController.OpenPortrait(id);
                Require(!view.portraitController.PortraitOpen, "Completed beauty cannot be customized twice");
                var saved = SnapshotCodec.Capture(em, root);
                SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, saved));
                king = WorldQueries.Find(em, id);
                view.ClosePanel();
                view.OpenPanel(GamePanelId.Royal);
                ...
                var adultSprite = view.Court.RoyalDetails.Portrait.sprite;
                royal = em.GetComponentData<Royal>(king);
                royal.Age = 75;
                em.SetComponentData(king, royal);
                ...
                royal.Age = 15;
                em.SetComponentData(king, royal);
                var aged = view.Court.RoyalDetails.Portrait.sprite;
                view.Refresh();
                ...
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/portrait-swaddle.png");
                    yield return null;
                }

                view.ClosePanel();
                var definition = SoldierId.FromIndex(0);
                var soldier = SoldierEntities.Spawn(em, root, definition, Unity.Mathematics.float3.zero, true);
                EntityState.Set(em, soldier, new Soldier { Experience = 39 });
                SoldierCombatants.Configure(em, root, soldier, false, 0, Unity.Mathematics.float3.zero);
                ...
                var soldierImage = SoldierCard().Portrait;
                var corners = new Vector3[4];
                soldierImage.rectTransform.GetWorldCorners(corners);
                ...
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/soldier-portraits.png");
                    yield return null;
                }
            }
            finally
