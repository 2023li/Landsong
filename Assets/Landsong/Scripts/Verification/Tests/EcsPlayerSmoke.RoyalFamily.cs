    ...
    public sealed partial class EcsPlayerSmoke
    {
        IEnumerator RoyalFamilyUi(UI_GamePanel view, EntityManager em, Entity root)
        {
            ...
            try
            {
                ...
                var mate = DynastyOps.CreateRoyal(em, root, "苏清和", 4, 23);
                var childId = Id(child);
                var mateId = Id(mate);
                Gender(child, PersonGender.Male);
                Gender(mate, PersonGender.Female);
                ...
                view.marriageController.MarriageEventButton.onClick.Invoke();
                ...
                king = WorldQueries.Find(em, oldId);
                child = WorldQueries.Find(em, childId);
                mate = WorldQueries.Find(em, mateId);
                ...
                view.marriageController.ShowMarriage(childId);
                yield return new WaitForSecondsRealtime(.3f);
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/marriage-request.png");
                    yield return null;
                }

                view.marriageController.MarriageApproveButton.onClick.Invoke();
                yield return WaitFor(() => em.GetComponentData<Royal>(WorldQueries.Find(em, childId)).Spouse == mateId, "Approve button dispatches ECS marriage decision");
                Require(!view.marriageController.MarriageOpen && em.GetComponentData<Royal>(WorldQueries.Find(em, mateId)).Spouse == childId, "Marriage modal closes and spouses become reciprocal");
                var grandson = DynastyOps.CreateRoyal(em, root, "沈景明", 2, 3, childId);
                var g = em.GetComponentData<Royal>(grandson);
                g.SecondParent = mateId;
                em.SetComponentData(grandson, g);
                ...
                graph.Node(childId).onClick.Invoke();
                yield return new WaitForSecondsRealtime(.3f);
                ...
                graph.Node(Id(grandson)).onClick.Invoke();
                yield return new WaitForSecondsRealtime(.3f);
                ...
                Require(view.Buildings.BuildingConfirmPanel.activeSelf, "Detail designation opens existing confirmation");
                view.Buildings.CancelBuildingInteraction();
                graph.Node(oldId).onClick.Invoke();
                yield return new WaitForSecondsRealtime(.3f);
                ...
                view.Court.RoyalDetails.OverviewTab.onClick.Invoke();
                yield return new WaitForSecondsRealtime(.3f);
                ...
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/royal-family.png");
                    yield return null;
                }
                ...
                Gender(proposed, PersonGender.Male);
                Require(RoyalFamilyOps.Request(em, root, request, proposed), "Second marriage request belongs to new monarch's child");
                view.marriageController.ShowMarriage(Id(request));
                yield return WaitFor(() => view.marriageController.MarriageOpen, "New dynasty generation can open marriage request");
                view.marriageController.MarriageRefuseButton.onClick.Invoke();
                ...
                view.OpenPanel(GamePanelId.Royal);
                yield return WaitFor(() => graph.Node(Id(request)) != null, "New child has a portrait");
                graph.Node(Id(request)).onClick.Invoke();
                ...
                view.Court.RoyalDetails.Marriage.onClick.Invoke();
                yield return WaitFor(() => view.marriageController.MarriageOpen && RoyalFamilyOps.Candidates(em, root, request).Count >= 3, "Arranged marriage prepares persistent candidates");
                yield return new WaitForSecondsRealtime(.3f);
                var candidate = RoyalFamilyOps.Candidates(em, root, request)[0];
                var candidateId = Id(candidate);
                var candidateName = em.GetComponentData<Identity>(candidate).Name.ToString();
                var candidateButton = view.marriageController.MarriageWindow.GetComponentsInChildren<UnityEngine.UI.Button>().First(b => b.GetComponentInChildren<TMPro.TMP_Text>().text.StartsWith(candidateName + " · "));
                candidateButton.onClick.Invoke();
                yield return new WaitForSecondsRealtime(.3f);
                Require(view.marriageController.MarriageApproveButton.interactable, "Chosen spouse details allow final confirmation");
                if (Application.isEditor)
                {
                    ScreenCapture.CaptureScreenshot("Library/LandsongEcs/arrange-marriage.png");
                    yield return null;
                }

                view.marriageController.MarriageApproveButton.onClick.Invoke();
                yield return WaitFor(() => em.GetComponentData<Royal>(request).Spouse == candidateId, "Active marriage confirmation commits chosen spouse");
            }
            finally
            {
