...
namespace Landsong.ECS.Presentation
{
    public sealed partial class EcsPlayerSmoke
    {
        ...
        {
            var original = SnapshotCodec.Capture(em, root); var cameraPosition = view.WorldInteraction.Camera.transform.position; var cameraSize = view.WorldInteraction.Camera.orthographicSize;
            ...
            var ray = new Ray(view.WorldInteraction.Camera.transform.position, view.WorldInteraction.Camera.transform.forward); var plane = new Plane(Vector3.up, (Vector3)point); if (plane.Raycast(ray, out float distance)) view.WorldInteraction.Camera.transform.position += (Vector3)point - ray.GetPoint(distance);
            view.WorldInteraction.Camera.orthographicSize = 10; view.OpenPanel(GamePanelId.Building); Entity WarningShot() { using var shots = Sim.Entities<Projectile>(em); foreach (var e in shots) { var p = em.GetComponentData<Projectile>(e); if (p.Mode == ProjectileMode.Ground && p.Warning > .2f) return e; } return Entity.Null; }
            yield return WaitFor(() => WarningShot() != Entity.Null, "Actual DBP acquires unit and launches telegraphed boss projectile");
            view.Commands.Send(CommandKind.Pause); yield return WaitFor(() => em.GetComponentData<Session>(root).Paused != 0, "11C pause command accepted");
            ...
            view.Commands.Send(CommandKind.Pause); yield return WaitFor(() => em.GetComponentData<Session>(root).Paused == 0, "11C resume command accepted");
            ...
            SnapshotCodec.Restore(em, root, SnapshotCodec.Decode(em, root, original)); view.WorldInteraction.Camera.transform.position = cameraPosition; view.WorldInteraction.Camera.orthographicSize = cameraSize; view.OpenPanel(GamePanelId.Building); yield return null;
        }
    }
}
#endif
