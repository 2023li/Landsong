...
namespace Landsong.ECS.Presentation
{
    ...
    public sealed class ApplicationCleanupRecovery
    {
        ...
        {
            if (BlocksNewSession)
                throw new InvalidOperationException("场景清理尚未完成，请先完成清理重试。");
        }

        public bool RequestRetry()
        {
            if (State != ApplicationCleanupState.Failed || retryRequested)
                return false;
            retryRequested = true;
            return true;
        }

        public async Task RunAsync(Exception initialFailure, Func<Task> cleanup, Func<Task> returnToMenu, Func<Task> nextFrame, Action<ApplicationCleanupRecovery> stateChanged)
        {
            ...
            while (true)
            {
                ...
                retryRequested = false;
                AttemptCount++;
                State = ApplicationCleanupState.Cleaning;
                stateChanged(this);
                try
                {
                    await cleanup();
                    State = ApplicationCleanupState.ReturningToMenu;
                    stateChanged(this);
                    await returnToMenu();
                    State = ApplicationCleanupState.Completed;
                    LastFailure = null;
                    stateChanged(this);
                    return;
                }
