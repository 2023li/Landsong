...
namespace Landsong.ECS.Presentation
{
    ...
    public sealed class GameApplicationFlow : MonoBehaviour
    {
        ...
        {
            if (ui != null)
                throw new InvalidOperationException("应用流程被重复初始化。");
        ...
        public void Register(ApplicationSceneEntry entry)
        {
            ...
            if (Transitioning)
                return;
            if (entry.Role == ApplicationSceneRole.Boot)
                CurrentOperation = ObserveStartupAsync(BootAsync());
            else if (entry.Role == ApplicationSceneRole.Menu)
                CurrentOperation = ObserveStartupAsync(OpenMenuAsync());
            else
        ...
        {
            if (scenes.TryGetValue(entry.Role, out var current) && current == entry)
                scenes.Remove(entry.Role);
        }

        async Task ObserveStartupAsync(Task task)
        {
            try
            {
                await task;
        ...
        Task Begin(EcsSceneFlow.Request request)
        {
            if (stopping)
                throw new OperationCanceledException("应用正在退出。");
            cleanupRecovery.EnsureNewSessionAllowed();
            if (Transitioning)
                throw new InvalidOperationException("场景切换尚未结束。");
            transitioning = true;
            cancelled = false;
            return CurrentOperation = TransitionAsync(request, ++generation);
        }
        ...
        async Task TransitionAsync(EcsSceneFlow.Request request, long operation)
        {
            try
            {
                ...
                if (cancelled)
                {
                    await CancelToMenuAsync();
                    return;
                }

                var world = World.DefaultGameObjectInjectionWorld;
                if (world == null || !world.IsCreated)
                    throw new InvalidOperationException("ECS World 不可用。");
                ...
                Entity root;
                while ((root = WorldQueries.Root(em)) == Entity.Null || !em.HasComponent<SimulationReady>(root))
                {
                    if (cancelled)
                        break;
                    CheckDeadline(deadline, "初始化王朝");
                    await Frame();
                }
                ...
                host.BindLighting(em, root);
                ui.Audio.BindSession(em, root);
                gameView.WorldInteraction.WorldPresentation.BindSession(em, root, ui.Audio, host.gameObject.scene);
                ...
                loading.Stage("准备完成", 1);
                host.SetVisible(true);
                await ui.Manager.CloseAsync<UI_LoadingPanel>();
                loading = null;
                transitioning = false;
                EcsSceneFlow.EnterGame();
            }
            catch (Exception failure)
            {
                Debug.LogError($"场景切换失败 operation={operation} map={request?.MapId} run={request?.Archive?.RunId} slot={request?.Slot}: {failure}");
                if (loading != null)
                    loading.ShowError("加载失败：" + failure.Message + "\n正在释放地图…", false);
                try
                {
                ...
                if (loading != null)
                {
                    loading.ShowError("加载失败：" + failure.Message, true);
                    cancelled = false;
                    while (!cancelled)
                        await Frame();
                }
                ...
                try
                {
                    await LoadMenuAsync("上次加载未完成：" + failure.Message);
                }
                catch (Exception menuFailure)
                {
                    await RecoverCleanupToMenuAsync(failure, menuFailure);
                }
            }
            finally
            {
                if (operation == generation && !cleanupRecovery.BlocksNewSession)
                    transitioning = false;
            }
        }
        ...
        {
            return cleanupRecovery.RunAsync(cleanupFailure, CleanupGameAsync, () => LoadMenuAsync("上次加载未完成：" + initialFailure.Message), Frame, recovery =>
            {
                if (recovery.State == ApplicationCleanupState.Failed)
                {
                    Debug.LogException(recovery.LastFailure);
                    if (loading != null)
                        loading.ShowCleanupError("场景清理失败：" + recovery.LastFailure.Message + "\n新会话仍被锁定。可以重试清理，完成后返回主菜单。", () => recovery.RequestRetry());
                }
        ...
        void Restore(EcsSceneFlow.Request request, World world, Entity root)
        {
            ...
            if (request.Archive != null)
            {
                ...
                if (request.Store != null)
                    checkpoint.Store = request.Store;
                ...
                if (request.Slot != null)
                {
                    try
                    {
                        ...
                        if (latest.Recovery.Turn == data.Recovery.Turn)
                        {
                            ...
                            if (latest.Recovery.LossCount > data.Recovery.LossCount)
                                data.Recovery = latest.Recovery;
                            data.Recovery.KnownIntel = known;
                            data.Recovery.AwaitingDecision = 0;
                        }
                    }
                    catch (Exception error)when (error is IOException || error is InvalidDataException)
                    {
                        Debug.LogWarning($"王朝恢复使用所选记录 operation={generation} run={data.RunId} slot={request.Slot} source={(request.RecoveredBackup ? "backup" : "slot")}: {error}");
                    }
                }

                checkpoint.Import(root, data, request.Persist);
                if (request.Persist)
                    (checkpoint.Store ?? CheckpointSystem.DefaultStore).Activate(data.RunId, request.Slot);
                if (request.RecoveredBackup)
                    SimulationEvents.Emit(em, root, EventKind.Message, "正式记录损坏，已从备份恢复；损坏原件尚未修改。");
            }
            else if (request.Snapshot != null)
            {
                var snapshot = SnapshotCodec.Decode(em, root, request.Snapshot);
                if (snapshot.Session.Phase != Phase.Day)
                    throw new InvalidOperationException("只能从白天存档继续游戏。");
            ...
            else
            {
                ...
                {
                    DynastyIdentity stateDynasty = em.GetComponentData<DynastyIdentity>(root);
                    stateDynasty.Name = RunArchiveStore.DisplayName(request.DynastyName);
                    {
                        em.SetComponentData(root, stateDynasty);
                    }
                }
        ...
        async Task CloseSessionAsync()
        {
            ...
            ui.Audio.UnbindSession();
            if (gameView != null)
            {
                gameView.WorldInteraction.WorldPresentation.UnbindSession();
                gameView.PauseMenu.Unbind();
                gameView.UnbindSession();
                gameView = null;
            }

            if (gameScope != null)
            {
                await ui.Manager.EndScopeAsync(gameScope);
                gameScope = null;
            }
        }
        ...
        async Task CleanupGameAsync()
        {
            ...
            foreach (var pending in pendingSceneOperations.ToArray())
                await Wait(pending, "等待未完成的场景操作清理");
            var game = SceneManager.GetSceneByPath(EcsSceneFlow.Game);
            if (game.IsValid() && game.isLoaded)
                await Wait(SceneManager.UnloadSceneAsync(game), "释放运行场景");
            await WaitReleased();
        }

        async Task CancelToMenuAsync()
        {
            await CleanupGameAsync();
            await LoadMenuAsync();
        }
        ...
        async Task WaitReleased()
        {
            ...
            while (!Released())
            {
                CheckDeadline(deadline, "释放地图所有权");
                await Frame();
            }
        }
        ...
        {
            if (operation == null)
                throw new InvalidOperationException(stage + "没有创建异步操作。");
            if (!pendingSceneOperations.Contains(operation))
                pendingSceneOperations.Add(operation);
            try
            {
                float deadline = Deadline();
            ...
            {
                if (operation.isDone)
                    pendingSceneOperations.Remove(operation);
            }
        }
        ...
        static void CheckDeadline(float deadline, string stage)
        {
            if (Time.realtimeSinceStartup > deadline)
                throw new TimeoutException(stage + "超时，请检查场景烘焙与配置。");
        }

        async Task Frame()
        {
            await Task.Yield();
            if (stopping || this == null || !Application.isPlaying)
                throw new OperationCanceledException("应用流程已停止。");
        }
        ...
        public async Task ShutdownAsync()
        {
            ...
            {
                Debug.LogWarning("退出时结束场景操作：" + error.Message);
            }
        ...
        void OnDestroy()
        {
            EcsSceneFlow.Unbind(Begin);
        }

        // If the loading prefab itself failed, cleanup still has a reachable retry control.
        void OnGUI()
        {
            if (loading != null || cleanupRecovery.State != ApplicationCleanupState.Failed)
                return;
            GUILayout.BeginArea(new Rect(20, 20, 520, 160), GUI.skin.box);
            GUILayout.Label("场景释放未完成：" + cleanupRecovery.LastFailure?.Message);
            if (GUILayout.Button("重试释放并返回主菜单"))
                cleanupRecovery.RequestRetry();
            GUILayout.EndArea();
        }
    }
}
