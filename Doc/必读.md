...
Unity **6000.3.24f1**；Entities **1.4.8**；Entities Graphics **1.4.21**；Addressables **2.9.1**；A* Pathfinding Project Pro **5.4.7**；Easy Save 3 **3.5.26**。游戏使用 ECS 权威状态，导航搜索与局部避让由 A* Pro 执行，存档封装、压缩、文件、备份与设置持久化由 ES3 执行。当前 ECS 快照 **v37**，ES3 王朝封装 **v4**；旧快照即使可解码，仍须通过内容指纹校验。

## 项目规范

项目已集成Addressable、Easy Save 3、Odin Inspector、Behavior Designer Pro 3 和 A* Pathfinding Project Pro 5.4.7、DoTween。已有能力优先使用插件 API，不在项目内重建并行实现。
