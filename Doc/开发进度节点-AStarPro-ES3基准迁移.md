...
- `CombatVerification` 的 RVO 注册断言现使用显式 A* 服务：原独立编辑器夹具未创建服务，导致导航走无服务回退路径，断言前提不成立。夹具负责清理自建宿主，保留注册断言并补充友军注册、未部署单位退出检查。本轮战斗 41 条断言通过；该编辑器检查验证组件注册生命周期，不代替 Play 中的实际避障分离测试。
