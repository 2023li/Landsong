...
> 最后核对：2026-07-17
> 状态：本文是维护地图、格子规则、地图加载、初始建筑、放置预览、资源连接和空间范围效果的唯一架构入口。

本文只描述当前仓库的真实实现。策划创建地图的操作步骤见 [地图创建与校验](地图创建与校验.md)，建筑能力配置见 [新增建筑与编辑器](../建筑系统/新增建筑与编辑器.md)。历史重构过程已经清理，所有仍有效的决策均已并入本文。
...
- Buff/美化/医疗/治安范围已经有通用 Definition、模块、预览和运行时查询框架。
- 建筑根对象不存在旋转放置或旋转存档语义。
...
| Overlay | `GridOverlayChannelDefinition.cs`、`GridOverlayChannelCatalog.cs`、`GridOverlayService.cs` |
...
| Buff/美化/医疗/治安 | `BuildingSpatialEffects.cs` |
...
- 建筑 Runtime Prefab：`Assets/Landsong/Objects/Prefabs/BuildingsRuntime`
...
`GridOverlayService` 必须绑定唯一 `GridOverlayChannelCatalog`。C1～C11 的具体资产只在 Catalog 中登记一次；`BuildingPlacementController`、`BuildingSelectionController` 等消费者不再各自保存 Channel 资产引用。
...
并自动登记水域、障碍和不替换主地形的 `石矿 Tilemap`。石矿是附加 Terrain Key，不替换默认陆地。

如果 Scene 中存在多个 Grid，按钮拒绝猜测，需要先手动绑定目标 Grid。

### 7.5 石矿与采石场

- 地图作者在 `石矿 Tilemap` 绘制矿脉；每个有 Tile 的格子获得 `石矿` Key，并保留 `陆地` Key。
- 采石场的 `RequiredTerrainKeys` 为 `陆地 + 石矿`。完整 3×3 footprint 的 9 个格子都必须同时满足这两个 Key，任意一格缺少石矿都会返回 TerrainMismatch。
- 这属于地图静态地形查询，不通过建筑脚本、碰撞体、View Prefab 或“占地内至少一格”特判。
...
- 稳定 Effect ID
- 显示名
- Kind：`ProductionPercent` / `Beauty` / `Medical` / `Security`
- TargetFilter：`AnyBuilding` / `Farmland` / `Cell`
- OperationalLevel：`0` 表示全部等级，正数表示只在该运营等级生效
- MinimumWorkers：`0` 表示无岗位条件，正数表示当前工人至少达到该值
- 曼哈顿 Range
- Value
...
- 美化值属于格子，同格取最高值。
- 医疗值属于格子，通过 `GetMedicalAtCell(...)` 查询；医院正式规则使用 `HighestValue`，因此多个医院或同一医院多个工人档位覆盖同格时只取最高医疗值。
- 治安值属于格子，通过 `GetSecurityAtCell(...)` 查询；警局正式规则同样使用 `HighestValue`，因此多个警局或同一警局多个工人档位覆盖同格时只取最高治安值。
- 多格建筑取所有占地格美化值的算术平均并向下取整。

当前树木、雕塑、医院和警局已经挂载正式范围效果。医院与警局各有六条 Definition，以运营等级和最低工人数分档；范围从完整 2×2 footprint 向外计算并排除自身占地。若未来增加医疗或治安 Overlay，应分别读取 `GetMedicalAtCell(...)`、`GetSecurityAtCell(...)`，不要在地图系统复制家族判定。
...
- 必填且稳定的 `ChannelId`
- 必填 `TileBase`
- 相对于 Overlay 基础层级的 `sortingOrder`

`GridOverlayStyle` 已删除。没有 ChannelId 或 TileBase 的 Definition 无效，`AcquireOwner(...)` 会返回 null。

`GridOverlayChannelDefinition` 仍然是必需资产，但它不是由各业务 Controller 分散绑定。当前唯一注册入口是：

```text
Game Scene
└─ GridOverlayService
   └─ GridOverlayChannelCatalog
      ├─ C1 ...
      └─ C11
```

Catalog 提供 ChannelId 到 Definition 的解析和重复/缺失校验。消费者只使用 `GridOverlayChannelIds` 的语义常量：

```csharp
var handle = overlayService.AcquireOwner(
    GridOverlayChannelIds.SelectedBuildingFootprint,
    "building-selection");
handle?.SetFootprint(building.Footprint);
```

因此放置占地和点击建筑占地现在共用同一条完整显示路径：`GridOverlayService.AcquireOwner(...) -> GridOverlayOwnerHandle.SetFootprint(...)`。它们只因语义不同而使用不同 Channel：放置合法/非法使用 C1/C2，点击选择使用 C10；不再各自实现 footprint 转格子和 Tilemap 操作。

以后点击建筑显示 Buff、资源范围或其他格子效果时，也应从对应系统取得 cells，再通过同一个 `GridOverlayService` 和 Catalog Channel 提交，不允许新建专用 Tilemap 或在业务 Controller 中重新序列化一套 Channel 资产。
...
- 建筑表现资源仍待按 [建筑表现资源](../建筑系统/表现资源.md) 回填；缺 View 不影响地图占格和连接规则。
...
- Runtime 与 Editor 程序集必须编译通过。
- `Landsong/地图/地图目录与校验` 必须通过 MapDefinition、Content Scene、Grid/Base Tilemap 与初始建筑检查。
- 使用“从当前地图开始 Play”走完整启动链，不能用直接运行 Content Scene 代替。
- Play Mode 必须覆盖 Additive 加载、初始建筑、完整占地、地形、Resource 路径、Overlay 清理和存读档。
