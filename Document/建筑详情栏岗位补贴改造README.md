...
6. 一键补满工人遇到可用人口不足时允许点击，只招到可用人口，并在消息栏添加通知。
7. 一键补满工人遇到金币不足时同样允许点击，只招到金币能支付的人数，并在消息栏添加通知。
8. 桌面端工人详情侧栏使用 hover 即显示。
9. 移动端长按 `0.6s` 显示工人详情侧栏。
10. 工人详情侧栏固定在详情面板侧边，不跟随鼠标或手指。
11. “满岗位需要的最少就业吸引力”只显示最终阈值。
12. `GamePanel_BuildingDetaiPopup.cs` 后续直接删除。
13. `sld_补贴滑动条` 不直接选择金币数，改为选择“目标稳定工人数”；系统反算达到该目标所需的最小有效金币，避免补贴没有跨过阈值时白白浪费。
...
- `BuildingDetailRow` 只有 `Label` 和 `Value`，不能表达按钮、开关、红绿颜色、hover 详情、长按详情。
...
这已经可以支撑“一键补满工人旁边显示需要消耗的金币”，但要注意“实际招工人数”还会受可用人口限制。
...
- “一键补满工人”只负责立即招工，把 `currentWorkers` 尽量补到 `maxWorkers`。
- 两者不是同一件事：补贴解决“能不能稳定留住这么多人”，一键招工解决“现在有没有人入职”。

### 通用侧边栏

鼠标移动到“工人 0/3（2）”时，显示侧栏。

移动端长按“工人 0/3（2）”时，显示侧栏。
...
```text
sld_补贴滑动条.value = 目标稳定工人
sld_补贴滑动条.minValue = 不含补贴时的稳定工人
sld_补贴滑动条.maxValue = 最大岗位
sld_补贴滑动条.wholeNumbers = true
```

这样滑条不会暴露无效金币数，只会暴露有意义的稳定工人档位。金币消耗永远由当前环境反算最小有效值。
...
当前 `LumberCabinLV1.TryRecruitImmediately()` 已经接近这个需求：
...
- 在 UI 上显示本次点击预计消耗的金币。
- 按钮文字建议为“一键补满工人”。
- 如果当前已满员，按钮 disabled，消耗显示 `金币 x0`。
- 如果可用人口不足，按钮仍然可点，点击后只招到可用人口，并在消息栏添加通知。
- 如果金币不足，按钮仍然可点，点击后只招到金币能支付的人数，并在消息栏添加通知。
...
如果可用人口只够招 1 人，但缺 3 人，建议显示：

```text
[一键招工] 最多 1 人，需要 金币 x12
```

如果金币只够招 1 人，也按相同规则显示最多可招人数。点击后的消息示例：

```text
伐木小屋只招到 1 名工人：可用人口不足。
伐木小屋只招到 1 名工人：金币不足。
```
...
```text
实际招工人数 = Min(缺少工人数, 可用人口, 当前金币可支付人数)
```
...
- 目标稳定工人数 Slider，即复用 `sld_补贴滑动条`。
- 当前补贴金额文本：`txt_当前补贴金币`。
- 一键补满工人 Button：`btn_一键补满工人`。
- 一键补满消耗金币文本：`txt_一键补满消耗`。
- 工人概览行容器：`go_工人详情触发区`。
- 通用详情侧边栏外壳 GO：`go_详情侧边栏`，只负责显示/隐藏整个侧栏。
- 通用详情侧边栏内容 root：`root_详情侧边栏内容`，只作为动态文本 item 的父对象。
- 通用详情侧边栏文本 item prefab：`prefab_详情侧边栏文本`。

`go_工人详情触发区` 必须是 Canvas 下的 UI 区域，并且它自己或子物体需要能被 UGUI raycast 命中。当前脚本会在运行时检测这个对象；如果它是 `RectTransform` 且没有任何 `raycastTarget = true` 的 `Graphic`，脚本会自动加一个透明 `Image` 作为 hover/长按命中区域。仍需注意不要被其他更上层的 Raycast Target 完全挡住。
...
保留 `sld_补贴滑动条`，但不要让它表示金币数量。它应该表示“目标稳定工人数”：

- Toggle 控制自动补贴。
- 自动补贴打开时，滑条值固定为 `maxWorkers`，并禁用交互。
- 自动补贴关闭时，滑条可选范围为“不含补贴稳定工人”到 `maxWorkers`。
- 滑条只使用整数档位。
...
9. 进 Unity 验证 prefab 绑定、hover、长按、事件消息、存档恢复。
