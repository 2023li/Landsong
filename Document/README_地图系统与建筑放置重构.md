...
> 状态：**A～AB 规则已全部实现到代码；运行时与 Editor 程序集编译通过。尚需用户完成本文第 16 节的 Inspector/Scene/Addressables/瓦片绑定，并在 Unity Editor / Play Mode 验收。**
>
...
- 两个预制体都包含同样的公共骨架：`GridMapBehaviour`、`GridRenderer`、Base/陆地/水域/占用/高亮 Tilemap，以及 `InitialBuildingPlacement`。
...
初始建筑采用 `InitialBuildingMarker` 协议：

- Marker 只保存建筑 Prefab、grid origin 和策划可见的编辑器预览；不保存 rotation。
- 项目中没有“旋转建筑”概念：建筑根 Transform 始终使用单位旋转，footprint 始终使用未旋转的 `Definition.Size`。美术朝向如有需要，应在建筑 Prefab 内部子对象中完成，不成为放置或存档数据。
- Runtime Host 完成地图绑定后，仅在新游戏流程中把 Marker 实例化为真实 `BuildingBase` 并占格。
- 加载新存档时完全跳过 Marker，只恢复快照中的建筑实例。
- 删除 `InitialBuildingPlacement.Awake()` 自动占格路径，避免 Additive Scene 加载与快照恢复竞争。

当前 `GridMapBehaviour` 强制要求同对象上存在 Unity `Grid`，并序列化引用各 Tilemap。采用此方案时，需要把它改成由已加载 Content Scene 中的 `MapContentAuthoring` 显式绑定 Unity Grid 和静态层，而不是靠“必须在同一预制体同一根对象上”取得引用。
...
该能力只决定是否执行和显示对应类型的连接查询，不会把连接结果变成放置限制。`ResidentialHousingLV1` 继续需要 Resource 连接，但迁移为显式连接能力，不再由具体类型写死。市场继续要求岗位满员才可运行，并保留按经手资源总价值 10% 结算金币的规则。PlayerHome 继续作为低优先级基础 Resource 提供点；市场保持更高优先级，运行时优先选择可用市场，市场不可用时回退 PlayerHome。
...
- `InitialBuildingPlacement` 的新游戏初始占格流程，以及新存档加载时的建筑恢复顺序。
- 道路双端点、道路交叉复用和批量扣费。
- 拆除模式与普通选择互斥。
- 建筑升级/替换时忽略旧占用的事务流程。
...
- [x] **G｜重叠视觉表现**：**已决定：采用可动态注册的分层 Overlay；新增效果通过 `GridOverlayChannelDefinition` / `GridOverlayStyle` 扩展，Runtime Host 动态创建或复用 Tilemap 层，不修改地图 Content Scene 或固定通道枚举。**
...
- [x] **L｜初始建筑的编辑与生成协议**：**已决定：Content Scene 保存 `InitialBuildingMarker`（Prefab 引用、grid origin 和编辑器预览）；不保存旋转；Runtime Host 绑定地图后仅在新游戏实例化，加载存档时跳过 Marker、只恢复快照；删除 Awake 自动占格路径。**
...
- [x] **W｜新游戏初始建筑与首次存档时机**：**已决定：Runtime Host 完成地图绑定、Marker 实例化和占格后，立即覆盖保存当前新游戏槽位一次；只有保存成功后才完成进入游戏流程。读档流程跳过 Marker 生成和这次初始快照写入。**
...
- [x] **AA｜新游戏初始化或首次覆盖保存失败时的回滚**：**已决定：在实例化前一次性验证全部 Marker 的 Prefab、地形、边界和相互占用；任何 Marker 失败或首次快照保存失败时，撤销本次已生成建筑，删除本次刚创建的新游戏槽位，显示明确错误并返回主菜单；不留下可读取的半初始化存档。**

- [x] **AB｜建筑旋转语义**：**已决定：游戏中没有旋转建筑物的概念。`InitialBuildingMarker`、玩家放置请求、建筑升级/替换和新存档快照都不保存、传递或恢复 rotation；建筑根 Transform 使用单位旋转，footprint 始终使用原始 `Definition.Size`。旧存档已放弃，因此直接从新建筑快照模型中删除 rotation 字段，不保留兼容分支。美术朝向只能在 Prefab 内部子对象中设置，不影响根对象或占地。**

#### 代码审查发现的直接依据

- `LSScenes.OnTargetSceneLoaded()` 当前同步实例化地图后立即恢复快照，Additive Addressables Scene 必须进入同一加载进度和失败处理链。
- `InitialBuildingPlacement.Awake()` 当前自动占格；`GameRuntimeSnapshotService` 恢复时会清空所有场景 `BuildingBase`，所以初始建筑必须改成明确的新游戏协议。
- `GameData.Validate()` 当前会把旧版本直接提升到最新版，存档索引扫描也会读取旧槽位；“放弃旧存档”不能只删除 `MapName` 字段。
- 当前实际资源消费者是 `ResidentialHousingLV1`；`BM_资源产出` 只有输出，没有“输入 -> 输出”加工能力。
...
`ResidentialHousingLV1` 的旧逻辑迁移为显式 Resource 消费能力；本轮新增通用输入/输出加工模块。市场继续保留满员运行条件和 10% 经手收益。PlayerHome 保留为低优先级基础 Resource 提供点，市场保持更高优先级。
...
- 每种效果通过 `GridOverlayChannelDefinition` ScriptableObject 注册稳定 `ChannelId`、`GridOverlayStyle` 和排序。
...
- `InitialBuildingMarker` 只保存 Prefab + origin；新游戏预检全部 Marker 后生成，立即写入首次快照；失败时回滚建筑和新存档。
- 建筑放置请求、升级/替换和建筑快照已删除 rotation 语义。
...
6. `GridMapBehaviour` 只保留运行时 Occupancy Tilemap + occupied tile；Base/Terrain 由 Content Scene 绑定时注入。
...
4. 把旧 `InitialBuildingPlacement` 子建筑转换为 `InitialBuildingMarker`，逐个绑定 `buildingPrefab` 和 `origin`。Marker 根 Transform 不允许旋转。
...
每种视觉先创建 `GridOverlayStyle`（Tile + Color），再创建唯一 `GridOverlayChannelDefinition`（稳定 ChannelId + Style + sortingOrder）。建议创建以下独立通道：
...
- PlayerHome Prefab：`isResourceProviderPoint = true`，`resourceProviderPriority` 设为低于市场的数值。
- Market Prefab：`isResourceProviderPoint = true`，优先级高于 PlayerHome；市场仍只在满员时可提供。
- 居民房：类型已显式实现 Resource 消费能力，无需额外模块绑定。
- 加工建筑：在 `buildingModules` 添加 `BM_资源加工`，填写生产周期、最低工人、全部输入 ItemAmount 和输出 ItemAmount。
- 灌溉坊：创建 `BuildingSpatialEffectDefinition`，`Kind=ProductionPercent`、`TargetFilter=Farmland`、`Range=10`、`Value=30`、`StackingRule=NoStack`；在 Prefab 的 `buildingModules` 添加 `BM_空间效果源` 并绑定该定义。
- 第一个装饰物：定义使用 `Kind=Beauty`、`TargetFilter=Cell`、`Range=1`、`Value=1`、`StackingRule=HighestValue`；同样通过 `BM_空间效果源` 绑定。
...
- 仍需 Unity 完成脚本 meta 生成、Inspector 绑定、Addressables Scene 加载、Tilemap 排序与 Play Mode 实际视觉/交互验收。
