...
- `BM_施工材料消耗` 现在也是 `Resource` 消费者模块；施工态建筑没有提供点时仍可放置，但不会扣料或增加经验，成功消费会记录最终提供点。
...
- 居民房 LV1：类型显式实现 Resource 消费能力，无需额外模块绑定。
- 居民房（在建）：通过启用的 `BM_施工材料消耗` 自动声明 Resource 连接；`buildingActionPower` 决定放置预览和运行时搜索预算。
- 居民房 LV2～LV4：当前仍是简化实现，尚未声明 Resource 消费能力；后续完善住宅等级时必须一并统一。
...
- 2026-07-13 为 `BM_施工材料消耗` 增加 Resource 消费者声明后再次验证：Runtime 0 error（2 个原有 `UIPanel_Setting` CS0649 警告），Editor 0 error / 0 warning。
...
- `GridOverlayChannelDefinition`、`BM_资源加工` 和空间效果框架已经可用，但当前建筑 Prefab 中尚未配置实际 `BM_资源加工` 或 `BM_空间效果源` 实例，也没有灌溉坊/美化 Definition 资产。后续新增内容时按 [AI_添加建筑规则.md](AI_添加建筑规则.md) 和 [AI_地图系统.md](AI_地图系统.md) 配置。
