...
> 当前事实版本：2026-07-18
> 当前规模：16 个 Family、16 个 ModuleSet、16 个 Presentation、16 个唯一 Runtime Prefab。
...
| 层 | 拥有内容 | 不拥有 |
| --- | --- | --- |
| `BuildingDefinition` | FamilyId、名称、分类、图标、占地、地形、放置费、蓝图、菜单规则与建筑概览显隐 | 实例状态、等级美术 |
...
6. 拥有岗位模块的玩家新建建筑竣工时，立即使用当前闲置人口补充岗位（最多补到岗位上限），并在随后两个运营回合内禁止工人自然流失；保护结束后仍按岗位吸引力正常流动。
...
| 工人、岗位吸引力、补贴、竣工岗位保护剩余回合 | `BM_岗位运营` |
| 生产周期与进度 | 生产 Module |
...
建筑概览是否列出某个家族由 `BuildingDefinition.ShowInOverview` 单独控制，默认值为 `true`。建筑概览只读取该字段过滤条目，不按名称、FamilyId、分类或模块类型硬编码；关闭显示不影响经济预测、生产结算、任务统计和存档。当前仅树木、小土堆和小石堆设为 `false`。

建筑模块向详情 UI 提供结构化功能块和侧栏行，不直接创建 UI。`Popup_BuildingDetails` 负责将侧栏行绑定到可复用文本项；Prefab 中“侧栏容器”的预放子对象是对象池预热项，`Awake` 必须先全部停用，后续刷新只做获取、绑定、停用和回收。侧栏宽度由 Prefab 布局组件根据激活内容计算，模块文案变化不需要修改面板宽度常量。
...
4. Play Mode 验证放置、逐回合施工、升级、拆除、主要 Module 回合行为和存读档；岗位建筑还需验证竣工按闲置人口补岗、连续两个运营回合不自然离职、第三个运营回合恢复吸引力流失，以及保护剩余回合存读档一致；建筑详情需反复打开侧栏并切换不同行数/长度内容，确认对象池复用和宽度自适应。
