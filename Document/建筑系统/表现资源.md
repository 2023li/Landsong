...
同一个运营等级需要随运行时状态切换局部表现时，把实现 `IBuildingViewStateConsumer` 的组件放在对应的 View Prefab 上。`BuildingPresentationController` 在实例化 View 后自动发现这些组件，并在建筑 `StateChanged` 时把当前 `BuildingBase` 交给它们刷新；不会因为局部状态改变而重新加载整个 View Prefab。

- View 组件只能通过能力接口读取状态，不保存玩法真相，也不能修改建筑、库存或回合数据；
- 通用状态能力放在玩法模块接口中，家族专属的节点显隐、Animator 参数或粒子切换留在 View 组件中；
- 需要强制检查序列化引用时，同时实现 `IBuildingViewStateConfiguration`，最终架构校验会报告配置错误；
- 存档恢复完成后建筑会发送 `StateChanged`，因此表现从模块存档状态重新计算，不需要单独保存 View 状态。

小土堆和小石堆使用 `FiniteHarvestPileView` 读取 `IBuildingFiniteHarvestState.RemainingHarvests`，分别切换 `Remaining3Root`、`Remaining2Root`、`Remaining1Root`。当前已有 Sprite 放在 `Remaining3Root`；缺少的 2 次、1 次美术先使用空节点占位，后续只需在对应节点下补表现资源。
...
| 粮仓 | 缺 | LV1～LV3 缺，可先只回填 LV1 并让高等级回退 | 家族图标缺 | P1 |
| 泥路 | 缺 | 默认 LV1 缺 | 家族图标已有 | P1 |
| 树木 | 缺 | `tree_01`～`tree_08` 的 LV1 均已配置 | 8 个样式图标均已配置 | P1 |
| 小土堆 | 复用运营 View | 剩余 3 次已配置；剩余 2/1 次为空节点占位 | 复用泥土物品图标 | P1 |
| 小石堆 | 复用运营 View | 剩余 3 次已配置；剩余 2/1 次为空节点占位 | 复用石头物品图标 | P1 |
...
校验器按模式检查施工表现：单一模式缺图只产生一条家族级提醒；逐回合模式按“家族/样式/施工回合”列出缺口。美术不完整时应明确使用单一模式，不能依赖逐回合模式回退到单一字段。树木、小土堆、小石堆与雕塑的 LV1 View 已完成，不计入运营缺口。
