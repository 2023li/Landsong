...
| `BuildingPresentationDefinition` | Stage/Level/Style 到 View、过渡效果 | 玩法数值、占地、任务身份 |
...
- 王宫、居民房、市场、农田、捕鱼、伐木、道路和树木均已删除家族专属 `BuildingBase` 派生类，改由统一运行时与模块组合。
...
- 生成 8 个 Family、8 个 ModuleSet、8 个 Presentation 和 8 个 Runtime Prefab。
- 删除旧等级 Prefab 目录、旧等级/施工/云中城脚本和失效科技解锁引用。
- 新增终态架构校验器；当前结果为 8 家族、8 Runtime Prefab、结构零错误。

## 7. 当前不是架构问题的内容缺口

以下不会再通过创建等级 Prefab 解决：

- 23 个表现回填提醒：8 个施工 View、7 个无样式家族 LV1 View、8 个树样式 LV1 View。
- 王宫和居民房 LV2～LV4、捕鱼 LV2 尚未设计，因此保持 `configured=false`。
- 农田作物数组为空；捕鱼特殊鱼关闭；部分建筑缺图标/成本。
- 仓库与云中城需要未来按完整新家族流程实现，不能恢复旧壳资源。

具体数值见《建筑数值策划表》，具体美术项见《建筑Prefab与表现资源回填指南》。
...
10. 任何新增/修改必须通过 `Landsong/Building/Validate Final Architecture`，并同步策划数值文档。
