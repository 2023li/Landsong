...
- 仓库通过 `workforce + warehouse.operation` 组合实现正常仓储工人要求、固定物理槽位、运行异常倍率、维护费、累计经验、升级门槛和满员奖励；外部库存系统只读取槽位来源、类型、数量和运行时倍率。
...
- 当前共有 13 个 Family、13 个 ModuleSet、13 个 Presentation 和 13 个 Runtime Prefab。
- 删除旧等级 Prefab 目录、旧等级/施工/云中城脚本和失效科技解锁引用。
- 新增终态架构校验器；当前目录包含 13 家族和 13 个唯一 Runtime Prefab，提交前仍必须在 Unity 执行最终架构校验。
