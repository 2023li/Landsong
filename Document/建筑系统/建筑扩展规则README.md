...
> 本文定义建筑系统的终态领域边界。具体接入步骤见《AI_添加建筑规则》，当前资产事实见《建筑说明》与《建筑数值策划表》；美术资源见《建筑Prefab与表现资源回填指南》。
...
只负责 Stage/Level/Style 到纯 View Prefab 的映射以及施工完成/升级过渡。美术资源可以晚于玩法资源交付，缺失时使用占位或同样式低等级回退。
...
2. 校验目标等级已配置、科技条件成立、金币/物品足够。
3. 原子扣费。
4. 修改同一实例的 `CurrentLevel`。
5. 应用目标等级配置，保留并钳制模块/家族状态。
6. 播放过渡并按相同 `StyleId` 刷新 View。
...
ModuleSet 顺序就是执行顺序。依赖模块必须排在使用者之前，例如岗位必须位于生产、作物、市场结算之前。等级配置引用的目标模块必须存在且启用，架构校验器会阻止缺失依赖。

### 新样式

样式只改变表现。先在 Presentation 声明 StyleId，再补各级 ViewMapping。样式不得改变 footprint、模块、费用、任务身份或升级树；若这些规则不同，应评估是否是真正的新家族。

### 新动画

动画、特效和音效放在 View/Transition 侧。玩法先提交等级状态，过渡控制换图时点；动画中断时最终 View 必须与真实状态一致。
