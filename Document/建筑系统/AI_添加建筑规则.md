...
- `processing`：资源加工；
- `inventory.capacity`：固定库存槽位，等级配置必须同时填写数量与 `InventorySlotType`；
- `warehouse.operation`：仓库的固定物理槽位、槽位类型、运行异常倍率、维护费、经验升级和维护失败吸引力惩罚；依赖 `workforce` 且排在其后；
...
建筑库存模块只配置槽位数量与 `InventorySlotType`。固定基础损耗倍率、物品组修正和自动存放优先级由库存系统的 `InventorySlotTypeCatalog` 管理，视觉由 UI 代码管理。缺工或维护失败可以通过 `InventorySlotProvision.RuntimeLossRateMultiplier` 提供临时倍率，但不得把冻库、粮仓等固定槽位规则复制到建筑等级配置。
